...
- **두 버그가 서로를 상쇄하고 있을 때, 하나만 고치는 것은 고치기 전보다 나쁠 수 있다.** — 실제 사고: 색공간 오류가 하늘을 3배 밝혀 팔레트 대비 부족(3.5%)을 가리고 있었다. 한쪽만 고치면 하늘이 도로 안 보이거나 한낮 하늘이 된다. 증상이 하나여도 원인이 하나라고 가정하지 않는다.
