// world-glb/features/ocean.ts — 바다와 강. 감독 확정 "섬. 강", 표현은 "복셀스처럼".
//
// ── 물을 그리지 않고 물을 만든다 ────────────────────────────────────────────
// 이 파일이 하는 일은 **큰 판 셋을 지면보다 낮게 깔아 두는 것**이 전부다(해저·바다·강).
// `decide/water.ts` 가 물이라 판정한 파셀을 스트리밍이 아예 로드하지 않고 그 구멍으로 이
// 판들이 비친다. 그래서 강의 경로를 바꿔도 이 파일은 손댈 것이 없다 — **강 모양을 아는
// 곳은 판정 하나뿐**이고 여기는 "물의 높이" 만 안다.
//
// ── 왜 판이 둘인가 (감독 *"복셀스처럼 바다, 강을 표현하자고"*) ──────────────
// Voxels 물의 성격은 **밝은 반투명**이다 — 물 너머로 바닥이 비치고 깊어질수록 물빛이
// 바닥을 이긴다. 그 "비침" 이 물을 물로 읽히게 한다. 그런데 반투명 판만 깔면 아래에
// 아무것도 없어 **하늘이 비친다**(유리 구멍이 된다). 그래서 해저 판을 함께 깐다 —
// 처음에 불투명 짙은 물색 판 하나로 만든 것을 감독이 잡은 지점이 정확히 여기다.
//
// ── 왜 셰이더가 아니라 UV 스크롤인가 ────────────────────────────────────────
// 감독: *"복셀은 물이 살랑살랑 진짜 물 느낌이 나. 쉐이더를 기가막히게 쓴듯."* 맞다.
// 다만 **GLSL `ShaderMaterial` 은 못 쓴다** — `three/webgpu` 빌드에 그 렌더 경로가 아예
// 없고 감독 실기기가 WebGPU 다. 헤드리스(WebGL/swiftshader)에서 통과해도 실기기에서 안
// 뜨는 형태의 사각이라 검증할 방법 자체가 없어진다. 그래서 두 백엔드에서 **똑같이 도는
// 수단**을 고른다: 노멀맵 두 장을 다른 속도로 흘린다. 두 파동의 간섭 무늬는 되풀이
// 주기가 길어 눈에 안 띄고, 빛이 기울기를 따라 미끄러지며 반짝인다 — 일렁임의 정체는
// 색이 아니라 **법선**이다. 매 프레임 바뀌는 것은 `offset` 두 쌍뿐이라 재질·지오·
// 파이프라인이 그대로다.
//
// ── 개수 불변식 ──────────────────────────────────────────────────────────────
// 메시 **셋**(해저·바다·강)·재질 둘(바다와 강이 공유)·지오 하나(셋이 공유)·텍스처
// **넷**(노멀·색조·윤슬·환경맵)을 부팅 시 만들고 세션 내내 그대로 둔다. 드로우콜 **+3**
// 고정이다(그림자 패스 없음 — `castShadow`/`receiveShadow` 셋 다 끈다).
// ⚠ 넷째(환경맵)는 2026-08-20 에 늘었고 **시간대가 바뀌어도 안 는다** — 같은 버퍼를
// 다시 채운다(`decide/water-env.ts`). 그것이 PMREM 을 안 쓴 이유 중 하나다.
// ⚠ 여기에 안 세는 것이 둘 있다: 층2 노멀맵(`normB`, `?wlayer2`)과 층2 패치 지오다.
//
// `transparent: true` 는 파이프라인 캐시키 축이라 세션 중에 켜고 끄면 전량 재컴파일을
// 부른다. **부팅 시 한 번 정하고 다시 만지지 않는다.**

// ── 포크 추적: world3 와의 쌍둥이 (2026-08-16 팀장 조건 C7) ──────────────────
// 이 파일은 **world3/features/ocean.ts 와 사실상 쌍둥이**였다 — 여기를 고치면 저쪽도
// 봐야 한다. ⚠ 2026-08-20 현재 **갈라져 있다**: 감독의 world2 물 판정(흐름 방향 통일 ·
// 잔물결 속도 · 환경맵)이 world3/5 에 전파되지 않았고, 그쪽은 *"도시 안 물이라 잔잔한
// 쪽"* 이라는 **다른 판정** 위에 서 있어 기계적 전파가 옳은지가 열린 물음이다(백로그).
//
...
import { createWaterEnv, type WaterEnv } from './water-env.js';
...
// **내가 볼 수 없는 것을 감독이 보기 때문이다** — 헤드리스는 SwiftShader(WebGL)이고
// 감독 실기기는 WebGPU 라 수면 반사가 도는 경로가 다르다. 값 하나 바꾸는 데 코드 수정 →
// 스모크 → 배포 왕복이면 한 번 맞추는 데 하루가 든다. 노브는 디버그 전용이 아니다 —
// 기본값이 곧 배포값이고, 노브는 그 기본값을 감독이 실기기에서 고르는 수단이다.
//
...
//   ?wripv=  무늬 흐름 속도 배율        0~40   기본: 유도 — `RIPPLE_SPEED` (`1`=예전)
//   ?wenv=   수면 환경맵 세기           0~3    기본: 1        (`0`=예전, 반사 없음)
//
// 앞 둘은 `readNumOpt` 다 — **기본값이 시간대마다 다르기 때문이다.** `readNum` 은
// fallback 을 강제해 "지정 안 됨" 과 "기본값과 같은 값을 지정함" 을 구별하지 못하고,
// 그러면 낮 기본값이 밤에도 걸려 **시간대 분기가 통째로 죽는다**(이 브랜치가 고친 전역
// 하드코딩 버그의 재발이다). 나머지는 기본값이 하나뿐이라 `readNum` 으로 족하다.
//
// **검은 점 진단 도구이기도 하다**(감독 보고, 헤드리스에서 재현 실패). `?wrough=0.45`
// 로 사라지면 원인은 근-거울면 쪽이고 `?wns=0.35` 로 사라지면 물결 기울기 쪽이다.
// ⚠ 이 진단문은 오래 *"근-거울면인데 반사할 환경맵이 없는 것"* 이라고 적고 있었다 —
// **2026-08-20 에 환경맵이 생겨(`?wenv`) 그 절반이 더는 참이 아니다.** 지금 그 증상이
// 다시 나면 `?wenv=0` 과 비교하는 것이 첫 갈래다.
//
 ...
 *
 * ⚠ **이 값은 「호수」다** (실측 2026-08-20). `0.0408` 은 옛 `FLOW_A = {0.035, 0.021}`
 * 의 **크기**를 그대로 옮긴 것이고, 그 벡터 옆에는 *"0.05 m/s 언저리는 잔잔한 호수,
 * 0.2 를 넘으면 강물처럼 흐른다. 도시 안 물이라 잔잔한 쪽으로 잡았다"* 는 판정이 적혀
 * 있었다(지금도 `world3`·`world5` 의 같은 자리에 남아 있다 — 그쪽은 **도시 물이라 그
 * 판정이 여전히 맞다**). world2 는 강이고 감독이 강을 요구했는데 값은 호수로 남았다.
 * 아래 `RIPPLE_SPEED` 가 그 어긋남을 닫는다 — **이 상수는 안 바꾼다**(층 사이 속도비를
 * 쥔 자리라, 여기를 만지면 황금비가 깨진다).
 */
...
// 정확한 지적이다 — 그때까지 **법선을 만드는 층은 하나뿐**이었다(`normA`). `tint`·
// `sparkle` 도 흐르지만 색·반짝임이라 **형상에 기여하지 않는다.** 그래서 무늬가 모양을
// 유지한 채 통째로 미끄러졌다("천이 흘러가는" 느낌의 정체). 실제 수면은 파장·속도가
// 다른 파동이 **겹쳐 간섭**해 무늬 모양 자체가 시시각각 변한다.
//
// ── ⚠ 사인파로 돌아가지 않는다 ──────────────────────────────────────────────
// "복합적인 사인파" 를 **공간 무늬**로 읽으면 안 된다. 2026-07-31 에 사인파 넷을 겹친
// 판본을 감독이 *"ㅈ런. 일자형 말고"* 로 기각했다 — 사인파는 마루가 직선이라 몇 개를
// 겹쳐도 격자·빗금이 남는다. 높이장은 값 노이즈 그대로 두고, 되살리는 것은 무늬가 아니라
// **여러 파동이 다른 속도로 지나가며 간섭한다는 성질**이다.
//
// ── 비율을 정수비로 두지 않는다 (팀장 조건) ────────────────────────────────
// 두 층이 **같은 텍스처**를 쓰므로 스케일·속도가 정수비면 주기적으로 정확히 정렬되고,
// 그 순간 간섭이 사라져 "한 장" 으로 보인다(물이 규칙적으로 박동한다). 그래서 **황금비**를
// 쓴다 — 연분수 전개가 `[1;1,1,1,…]` 라 유리수로 가장 느리게 근사되는 수, 즉 **정수비에서
// 가장 먼 수**다. "적당히 안 맞는 값" 을 손으로 고르는 대신 성질에서 유도한다.
 ...
 * 반투명 판이 겹치면 실효 불투명도가 곱으로 올라간다. 이 파일은 그것을 이미 알고 있었고
 * (강 판을 파셀 위에만 깐 이유가 그것이다) 주석이 *"`WATER_DEPTH` 는 단일 반투명 층을
 * 전제로 고른 값이라 캘리브레이션이 무효가 된다 — 값이 아니라 전제가 깨진 형태"* 라고
 * 적고 있었다. **그런데 그 위에 또 한 겹을 얹었다.** 검수관 실측: 바다 투과 0.300 →
 * 0.174(−42%) · 강 0.090 → 0.030(−66%, 실효 불투명도 0.910 → 0.970). 물가에서 바닥이
 * 어렴풋이 비치는 정도는 감독이 `?wopa=` 로 고른 축인데 기록 없이 바뀌었고 검사도
 * 없었다(`LAYER2_OPACITY` 를 1.0 으로 바꿔도 78건 전부 통과).
 *
 * ── 균등 배분으로 간다 ──────────────────────────────────────────────────────
 * 처음엔 위 층을 상수(0.42)로 고정했는데 **위 층 값이 총량보다 크면 배분이 성립하지
 * 않는다**(아래를 0 으로 해도 총량을 못 맞춘다 — G-2b 검사가 `total=0.3, second=0.42`
 * 에서 바로 잡았다). 그래서 층 수에서 유도한다: `(1−o)ⁿ = 1−total`. 상수 하나가
 * 사라지고 총량 보존이 **어떤 값에서도** 성립한다 — 둘 다 같은 물이니 대등한 것이 맞다.
 *
...
//
// ⚠ **왜 오래 안 보였나** — 판 생성부 주석이 *"정점을 쪼개면 삼각형만 늘고, **실제 파도
// 진폭이 없는 지금은** 보이는 차이가 0"* 이라 적고 있었다. **쓰인 시점에 참이었고** 그 뒤
// `surfaceLift` 가 생겨 전제(진폭 0)가 무너졌는데 주석도 코드도 남았다 — CLAUDE.md 가
// 이름 붙인 *"참인 문장에서 성립하지 않는 결론"* 이다.
//
// ── 왜 바다 판 전체를 세분하지 않는가 (팀장 판정 A) ──────────────────────────
// `PLANE` 은 1,920m 다. λ/4 = 4m 로 전면 세분하면 정점 **231,361** 로 매 프레임 CPU
// 갱신이 불가능하고, 8m 로 낮춰도 58,081 인 데다 λ=16m 를 λ/2 로 샘플링하는 셈이라 강이
// 세운 유도 규율(`RIVER_SEG`)을 바다에서만 깨게 된다. **실제로 보이는 바다는 안개 far
// 까지뿐이므로**(바깥은 안개가 100% 덮는다) 그만큼만 세분한 패치를 플레이어 아래에 깔고
// 따라 움직인다. `surfaceLift` 가 **월드 좌표만의 함수**라 패치를 옮겨도 파형이 제자리에
// 있다 — 강이 파셀 경계 이음새를 막는 데 이미 쓰는 바로 그 성질이다.
 ...
 * 처음엔 `span = 2·far` 로 유도하고 *"보이는 바다는 far 까지뿐"* 이라 적었다. **그 문장은
 * 패치가 항상 플레이어 중심일 때만 참이다** — 패치는 `SNAP` 단위로만 움직이므로 플레이어가
 * 중심에서 최대 `SNAP/2` 어긋나고, 보장 반경은 `SPAN/2 − SNAP/2` 다. 검수관 실측:
 * 최악 축 커버 `155.33/2 − 25.889/2 = 64.72m`(far 76.8 에 **12.08m 미달**), 그 지점 안개
 * 52.8% → **가시 47.2%** 에서 패치 경계가 끊긴다. 즉 울렁이는 물과 평평한 물이 눈에
 * 보이는 거리에서 맞닿았다. 이 커밋 주석이 *"참인 문장에서 성립하지 않는 결론"* 을
 * 인용하며 들어왔는데 유도가 **같은 형태**였다 — 전제(중심 고정)를 스스로 깨는 구현을
 * 옆에 두고도 유도를 안 고쳤다. 그래서 한 타일을 더 얹는다: `SPAN ≥ 2·far + SNAP`.
 */
...
/**
 * 수면이 반사할 환경맵의 세기(`?wenv`). **`0` 이 정확히 2026-08-20 이전 상태**다 —
 * 그때까지 반사할 것이 없어 «반짝임» 이 광원과 무관한 `emissiveMap` 스티커였다(감독:
 * *"왜 밑 반사는 움직이지 않을까"*). 무엇을 굽는지는 `decide/water-env.ts` 한 곳이다.
 *
 * ⚠ 이 값은 **거울면 쪽 밝기를 통째로 쥔다** — 낮 `roughness` 가 0.18 이라 수면이 거울에
 * 가깝고, 올리면 점이 아니라 **면**이 밝아져 감독이 반려한 *"흰 덩어리"* 로 되돌아간다.
 *
 * ✅ **감독 승인 2026-08-21**(바다 앞·낮, `?wenv=0` 과 대조). 세기 1 은 **화면으로 판정된
 * 값**이라 올리는 것도 내리는 것도 판정을 뒤집는 일이다 — 그러려면 화면으로 다시 묻는다.
 */
const WATER_ENV = readNum('wenv', 1, 0, 3);
...
/**
 * 세 무늬 층이 흐르는 속도의 **공통 배율**(`?wripv`). `1` 이 정확히 예전 상태다.
 *
 * ── 감독 판정 2026-08-20 *"강에 가면 잔물결들이 막 이동하잖아"* ─────────────
 * 첫 진단은 «무늬가 16m 라 잔 것이 없다» 였고 처방이 `RIPPLE_M` 축소였다. **그 축은 이미
 * 기각돼 있다** — 2026-07-31 에 옥타브를 61·127 로 촘촘히 했다가 *"이단물결은 아까가 나아.
 * 되돌려"* 로 반려됐다. 남은 축이 **속력**이고 그것은 한 번도 안 열렸다.
 *
 * **실측**(유속 1.1 m/s, 배율 1 일 때 무늬의 월드 속도):
 *   normA 0.045 · tint 0.028 · normB 0.118 m/s   ←  `FLOW_SPEED_A` 가 스스로 «호수» 라
 *   강 UV 스크롤 **1.1 m/s**(유속 그대로 — 아래 `phase`)  적어 둔 대역이다
 *
 * 즉 **같은 화면에서 강만 25배 빠르게 흐르고 있었다.** 감독이 *"윗물은 자연스러워"* 라고
 * 판정한 것이 그 강이다 — 맞출 기준을 지어낼 필요가 없다. **이미 감독 판정을 받은 속도가
 * 저장소 안에 있다.** 그래서 기본 배율을 «가장 빠른 층(normB)이 그 강 유속과 같아지는 값»
 * 으로 **유도한다**(≈9.36). 수가 아니라 식으로 두면 `FLOW_SPEED_A`·황금비 중 무엇을 바꿔도
 * 관계가 따라온다 — 실측에 여유를 얹은 값은 근거가 아니다.
 *
 * ⚠ 가장 빠른 층을 기준으로 잡는 것이 **보수적인 쪽**이다. 층1 을 강에 맞추면 층2 가
 * 2.9 m/s 로 흘러 «급류» 가 된다 — 되돌리기가 비싼 쪽을 기본값으로 삼지 않는다.
 * ✅ **감독 승인 2026-08-21**(강가·낮, `?wripv=1` 대조). **여기서 멈춘다** — 더 빠른 축이 열려 있으나 감독이 화면으로 이 속도를 골랐다.
 */
const RIPPLE_SPEED_DEFAULT = 1 / (FLOW_SPEED_C * LAYER2_WORLD_SPEED);
const RIPPLE_SPEED = readNum('wripv', RIPPLE_SPEED_DEFAULT, 0, 40);
// 파동 성분과 세 층의 흐름 벡터. **방향은 하나, 속력만 다르다**(감독 처방)
const LIFT_DIRS = liftWaves(WAVE_SPREAD);
const FA = flowVec(FLOW_SPEED_A * RIPPLE_SPEED, FLOW_DIR);
const FB = flowVec(FLOW_SPEED_B * RIPPLE_SPEED, FLOW_DIR);
const FC = flowVec(FLOW_SPEED_C * RIPPLE_SPEED, FLOW_DIR);
...
// `three/webgpu` 에는 GLSL `ShaderMaterial` 렌더 경로가 없다. TSL 노드 재질은 world2 최초
// 도입이라 **헤드리스로 검증할 수 없는 축**이 늘어난다(감독 실기기가 유일 판정). 팀장이
// 그 위험으로 기각하고 현행 방식 확장을 지시했다: *"반짝임의 정체는 법선 위의 스페큘러
// 이고, 이것은 실제 광원 방향을 따른다."* **`emissive` 로 내지 않는다**(조건 4) — 스스로
// 빛나는 물은 광원과 무관해져 밤 지적과 정면 충돌한다.
//
// ⚠ **그 조건은 지켜지지 않고 있었다**(2026-08-20 실측). 아래 `seaMat` 이 흰 점 텍스처를
// `emissiveMap` 으로 달고 있다 — 광원 의존 경로가 실기기에서 반짝임 0 이라 교체한 것이고
// 기록도 있다. 그런데 **발광은 물결이 기울어도 안 변한다.** 감독이 *"왜 밑 반사는 움직이지
// 않을까"* 라고 물은 것이 정확히 그 성질이다. 환경맵(`WATER_ENV`)이 조건 4 의 **목적으로
// 되돌아가는 길**이다 — 거울상은 시점·광원·물결에 전부 반응하고, 밤에는 씬 조명이 이미
// 어두우니 물도 저절로 어두워진다. 스티커는 아직 안 걷었다(`?wenv=0` 이 대조군 — 걷는
// 판단은 감독이 두 화면을 본 뒤다).
//
// ── 왜 거칠기맵 슬롯을 버렸나 (감독 실기기 실증 2026-07-31) ──────────────────
// 예전 판본은 *"슬롯을 추가하지 않는다"* 를 내걸고 **층 B 노멀맵을 `roughnessMap` 에
// 태웠다** — 표준 재질이 G채널만 읽으니 텍스처 한 장이 두 역할을 한다는 절약이었다.
// **그 절약이 화면을 망가뜨렸다.** G 는 법선의 Y성분이라 0~1 을 오가고, 실효 거칠기가
// `스칼라 × G` 이므로 G≈0 인 자리가 **완전 거울**이 된다. 그때 이 씬에는 반사할
// 환경맵이 없었으므로 그 자리가 **검었고**, 무늬를 따라 띠를 이뤄 **검은 줄기가 물
// 위를 흘러갔다.** 감독 스크린샷이 확정했다 — 거칠기를 **1.00** 까지 올려도 그대로였다
// (곱셈이라 G=0 은 어떤 스칼라로도 0 이다). 값으로 못 고친다.
// 내가 처음 짚은 원인 *"환경맵 부재"* 는 **절반만 맞았다** — 진짜 원인은 *어디가 거울이
// 되는지를 노멀맵이 정하고 있던 것*이다. 그래서 슬롯을 **비운다**(거칠기는 시간대·노브가
// 정하는 스칼라 하나). ⚠ **환경맵은 그 뒤 2026-08-20 에 생겼다**(`WATER_ENV`) — 그러니
// 위 «검다» 는 **그때의 상태**이고 지금 이 슬롯을 되살려도 그 증상은 안 난다. 되살리지
// 않는 이유는 여전히 유효하다: 한 채널이 두 역할을 맡으면 어느 쪽도 따로 못 고친다.
...
export const oceanFeature: Feature = {
  ...
  create(env: FeatureEnv): FeatureInstance | null {
    ...
    // 윤슬의 원천. `null` 이면 `?wenv=0`(옛 상태)이고 그때 재질에도 안 걸린다.
    const waterEnv: WaterEnv | null = WATER_ENV > 0 ? createWaterEnv(env) : null;
    ...
    const layer2Mat = normB
      ? new THREE.MeshStandardMaterial({
        color: WATER,
        normalMap: normB,
        // 층1 과 **같은 텍스처**를 건다(개수 불변식 `[7]`). 법선이 다르므로 같은 하늘을
        // 봐도 상은 갈리고, 그 어긋남이 간섭을 눈에 보이게 한다.
        envMap: waterEnv?.texture ?? null,
    ...
    if (patchGeo) {
      ...
      // `layer2Mat` 은 `sea2`·`river2` 가 **공유**하고 `normB.repeat` 는 UV 0..1 이
      // **`PLANE`(1,920m)** 을 덮는 지오를 전제로 유도돼 있다. 그런데 `sea2` 의 지오만
      // 181m 패치로 바뀌고 `repeat` 는 그대로였다 — **같은 재질인데 월드 무늬 크기가
      // 달라진다**(실측: river2 25.889m vs sea2 2.443m, **10.59× 어긋남**). 셋이 깨진다:
      //   ① `LAYER2_SCALE = PHI` 의 근거("층2 = 층1 × 1.618")가 무효 — 실제로는 1/6.5
      //   ② 흐름이 10.59× 느려진다 — 아래 `b` 의 UV 환산이 타일 `RIPPLE_M × LAYER2_SCALE`
      //      를 전제하므로 **검수관 R-3 결함이 되살아난다**(이 작업을 연 감독 판정이
      //      *"`?wflow=8` 이것도 약해"* 였다)
      //   ③ 월드 고정이 깨진다 — 스냅당 위상 10.5949 타일(소수부 0.405)이라 25.9m 마다
      //      무늬가 0.99m 튄다. 스냅이 막는다고 적은 바로 그 실패다.
      // **재질·텍스처는 그대로 두고 지오의 UV 만 줄인다** — `normB` 를 고치면 `river2` 가
      // 깨지고(공유), 재질을 새로 만들면 `layerOpacity` 캘리브레이션과 겹쳐 팀장 사안이
      // 된다. UV 를 `SPAN/PLANE` 배로 두면 `PLANE` 지오와 **같은 규칙**이 되어 월드 타일이
      // 저절로 맞는다. ⚠ 이동해도 무늬가 안 튀는 근거: `offset` 은 공유라 보정할 수 없지만
      // 스냅 보폭이 월드 타일과 같으므로(`SEA_PATCH_SNAP = LAYER2_TILE_M`) 한 칸이 정수
      // 타일이고 `RepeatWrapping` 에서 정수 위상차는 같은 무늬다. G5 가 이것을 검사한다.
    ...
    // ── 강 판 (감독 지시 2026-07-30 *"강은 땅보다 50cm 밑, 바다는 1미터 밑"*) ──
    // 두 높이를 요구하므로 판이 하나일 수 없다. **재질은 바다와 공유한다** — 같은 물이고,
    // 따로 만들면 물빛·윤슬·불투명도가 두 곳에 적히는 미러링이 된다.
    //
    // ── 지오는 공유하지 않는다 (검수관 블로커) ──────────────────────────────
    // 처음에는 지오도 공유했다(둘 다 `PLANE` 판). 그러면 **강이 보이는 모든 곳에서 두 판이
    // 겹쳐 그려진다** — 격자 안에서 물인 곳은 강뿐이라 물이 보이는 전 구간이 그 상태였다.
    // 반투명 두 장이 겹치면 실효 불투명도가 `1 − (1 − 0.7)² = 0.91` 로 오르는데,
    // `WATER_DEPTH` 는 **단일 반투명 층**을 전제로 고른 값이라 캘리브레이션이 무효가 된다
    // (값이 아니라 전제가 깨진 형태다).
    //
    ...
    return {
      system: {
        ...
        update(ctx) {
          ...
          // ⚠ **여기에 `flowMps()` 가 빠져 있었다** (감독 실기기 2026-08-05: *"바다는
          // 안 움직여… 옵션으로 조정해봐도 안움직여"*). 위 주석은 *"초당 미터 /
          // RIPPLE_M"* 이라 적는데 코드는 `t` 만 썼다 — **주석과 코드가 다른 것을
          // 말했다.** 층2 는 검수관 R-3 으로 UV 환산을 고쳤는데 **층1 은 그때 아무도
          // 안 봤다**; 층2 에만 속도 상수가 있는 비대칭이 그 흔적이다.
          ...
          //
          // ── 되감지 않는다 (감독 실기기 실증 2026-07-31 *"심리스가 아니여서
          // 이동하다가 끊기네"*) ────────────────────────────────────────────
          // 예전에는 `% RIPPLE_M` 으로 타일마다 되감으며 *"모든 정점이 같은 순간에
          // 되감기니 무늬가 경계에서 겹친다"* 고 적었다. **틀렸다** — 같은 순간인 것은
          // 맞지만 되감긴 **위치**가 타일 격자와 안 맞는다. UV 이동량이
          // `riverFlow × phase` 인데 흐름이 단위벡터라 `|fx| < 1` 이고, 그래서 한 타일을
          // 채운 순간의 실제 u 이동량이 타일 주기의 정수배가 아니다. 게다가 `fx` 는
          // **정점마다 다르므로**(굽이의 접선) 튀는 양도 달라 파셀 경계에서 찢어진다.
          // 안 되감으면 그만이다 — `RepeatWrapping` 이라 UV 가 1 을 넘어도 이어지고,
          // float32 도 여유롭다(10시간을 흘려도 `phase` 는 수십 수준).
