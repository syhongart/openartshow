...
export interface FeatureEnv {
  ...
  // ── 🔴 **늦게 읽는 클로저 계약** — `create` 에서 값을 꺼내 캐시하지 마라 ──────
  //
  // 아래 넷(`glbMap` · `walkGrid` · `blockWalk` · `glbCityRoot`)은 **전부 같은 성질**이다:
  // 기능 조립(`main.ts` 의 `pools` 단계)이 그 값을 만드는 단계(`stream`, 또는 13.5MB
  // 자산의 비동기 로드)보다 **먼저** 돈다. 그래서 **켠 페이지에서도** mount 시점의
  // 호출은 `null` 이고, 값은 그 **뒤에** 채워진다.
  //
  // 즉 계약은 「호출할 수 있다」가 아니라 **「매번 다시 호출한다」** 다. `create` 안에서
  // 한 번 읽어 파생값을 굳히면, 그 기능은 **영영 `null` 만 본 채로 돈다.**
  //
  // ⚠ **이것은 가정이 아니라 실제로 일어난 사고다**(부팀장 자기신고 2026-09-19).
  // `features/npc.ts` 가 `walkGrid()` 를 `create` 에서 한 번 읽고 캐시했고, 그래서
  // 배포된 걷기 격자가 world11 에서 **한 번도 동작하지 않았다** — 치비가 맨해튼 건물을
  // 관통했다(감독 신고 *"벽사이를 걸어가네"*). 같은 자리의 `blockWalk` 는 **매 프레임
  // 부르는 형태**라 멀쩡했다 — 두 값이 나란히 배선돼 있는데 **소비 방식만 갈렸다.**
  //
  // ⚠⚠ **그 결함을 어떤 테스트도 못 봤다.** 검사가 `create` 에 값을 **직접 주입**해
  // 돌리면 「그 값이 소비되는가」는 보지만 「그 값이 **손에 들어오는가**」는 원리적으로
  // 재는 축에 없다. 그러므로 이 계약을 지키는지는 **부팅을 실제로 돌리는 검사**로만
  // 확인된다 — `tests/world-glb-walkgrid-boot.test.ts` 가 그 자리다.
  /**
   ...
   * 가 이 트리의 불변 보장 형태다.
   *
   * 🔴 **`create` 에서 캐시하지 마라 — mount 시점엔 켠 페이지에서도 `null` 이다**(위
   * 「늦게 읽는 클로저 계약」 절). 값은 `main.ts` 의 `stream` 단계에서 채워진다. 소비자는
   * **프레임마다 다시 호출**하고, 바뀌었는지는 **참조 동일성**으로 판정한다(파생값을 매
   * 프레임 다시 계산하지 않기 위해서다 — `decide/npc-grid.ts` 의 `walkBinding`).
   */
