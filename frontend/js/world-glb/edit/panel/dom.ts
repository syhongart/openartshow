// world-glb/edit/panel/dom.ts — 패널을 짓고, **화면에 말한다**.
//
// ── 이 파일이 소유하는 것 ───────────────────────────────────────────────────
...
export interface PanelHandlers {
   ...
   * 겹쳤었다. 패널 버튼은 그 충돌 밖에 있다.
   *
   * ⚠ **2026-08-21 에 그 충돌을 고쳤다**(`G-EDIT14`, 팀장 판정 (나)). 이동 조이스틱은
   * 전용 원 안에서만 판정하고, 편집 중에는 시선 드래그만 끊는다(`ui/touch-controls.ts`).
   * **그래서 이제 캔버스도 투입구가 될 수 있다** — 다만 「고르기 → 벽 탭」 2단계는
   * 상호배타 불변식과 `input.ts` 여유가 따로 걸려 있어(`edit/pick.ts` 의 팀장 판정 (가))
   * 이 버튼이 여전히 폰의 주 경로다.
   *
   * ⚠⚠ **이 줄은 두 번 거짓이었다.** ① 오래 **번호 없이** *"백로그"* 라고만 적었는데
   * 그 항목이 실재하지 않았다(2026-08-21 실측) ② 항목을 만들어 참으로 되돌린 **바로 그
   * 회차에 충돌을 고쳐 놓고 이 문장은 안 고쳤다**(검수관 조건 B) — 「가리키는 대상 없는
   * 참조」를 지적한 자리에서 「사실과 다른 서술」을 새로 남긴 것이다.
   * **구조를 바꾸면 그것을 서술하는 문장을 다시 읽는다**(태스크 #110 이 그 트리거를
   * 조건문으로 만드는 일이다 — 지금은 형용사라 이렇게 새어 나간다).
   */
