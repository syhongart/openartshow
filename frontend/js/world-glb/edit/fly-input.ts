// world-glb/edit/fly-input.ts — **편집 비행의 키 입력.** 산술은 안 한다.
//
...
const FLY_KEYS: Readonly<Record<string, keyof FlyInput>> = {
  ...
  //
  // 🔴 **그 「금지」에 붙는 단서**(검수관 실증 + 재실측, 2026-08-19):
  //   ① GS-F5 는 **문자열 리터럴 안의 `//`** 에 눈이 먼다 — 주석 제거가 문자열을 구분하지
  //      않아 `const _t = "note//"; Object.assign(…);` 한 줄이 **통째로 지워진 채** 검사에
  //      도달하지 않는다. **다만 표본 안 키라면 GS-F6 이 구제한다**(은닉+`F1` 1 failed).
  //   ② GS-F6 은 **표본**(91개)만 본다 — 전수가 아니다.
  //   ③ GS-F6 은 **판정 시점 밖**을 못 본다 — `setTimeout` 은 물론 **`onUp` 에서
  //      `held.add`** 하는 형태도 **표본 안 키라도** 0 failed 다.
  //   🔴 **남는 사각 셋**: ⓐ **표본 밖 키코드 — 은닉과 무관**(`onDown` 에 `IntlYen`·`F13`
  //   을 그냥 하드코딩해도 0 failed. 이 경로는 `FLY_KEYS` 를 안 만져 GS-F5 를 탈 일이
  //   없다 — 첫 판본이 *"은닉 + 표본 밖"* 으로 좁게 적었다, 검수관 P-a) ·
  //   ⓑ **판정 시점 밖**(비동기 지연 + `keyup` 경로) · ⓒ **동적 이름 접근**.
  //   ⚠ 거짓 FAIL 0 은 **jsdom 범위에서만** 참이다(한글 IME·macOS 레이아웃 미측정).
  //   재론 조건은 백로그 `G-FLY14`.
  //
  // ⚠ **이 목록은 세 번 거짓이었다.** ① 1판은 「리터럴 안」조차 못 지켰다 — 정규식이
  // 따옴표를 안 봐서 `'AltLeft': 'down'` **추가**가 통과했다(검수관 조건 1). ② 2판은 남은
  // 사각을 *"다른 파일에서 병합 — 정적 파싱 밖이라 못 본다"* 로 적었는데, 실제로 뚫린 것은
  // **이 파일 안, 선언 바로 밑 한 줄**이었다(`Object.assign(…)` → 0 failed). ③ 3판은
  // GS-F5·GS-F6 을 세워 놓고 *"잡는다"* 로 **단정**했다 — 위 단서 셋이 빠져 있었다.
  // **닫을 수 있는 것을 「원리적 한계」로 적으면 다음 사람이 확인을 생략한다.** 그리고
  // ③이 보여주듯 **닫은 뒤에 「어디까지 닫혔는지」를 안 적는 것도 같은 형태다.**
  Space: 'up', KeyC: 'down',
};
