...
export interface WalkGrid {
   ...
   *
   * ⚠⚠⚠ **씬에 나중에 붙는 물건도 이 값을 갱신한다**(`blockMesh`). 감독 신고
   * 2026-09-22 *"실내 갤러리에서 떠보이는 것 같아"* 가 그 구멍이었다 — 굽기는 GLB
   * 세계만 보는데 미술관은 `position.set(x, **0**, z)` 로 나중에 놓이고 맨해튼 지면은
   * +0.2m 라, 치비가 세계 지면 높이에 서서 **실내 바닥보다 20cm 떴다.** 그래서 덧칠에서
   * 「**나중에 놓인 것이 이긴다**」로 갱신한다(「높은 쪽」이 아니다 — 실내에 들어가면
   * 화면에 보이는 것은 그 실내 바닥이다).
   *
