// world-glb/decide/fly.ts — **편집 중 비행**의 판정. three 도 DOM 도 안 만진다.
//
...
//
// ── 🔴 이 파일은 **아직 배선이 0 이다** (검수관 B-3 · `docs/BACKLOG.md` 의 `G-FLY1`) ──
...
// 같은 것을 하나 더 만드는 셈이므로 **주소를 남긴다: `docs/BACKLOG.md` 의 `G-FLY1`.**
//
// ⚠⚠ 이 자리는 원래 *"태스크 #119 로 등록해 두었다"* 라고만 적었고, 검수관이 **저장소
// 안에서 그 진술을 확인할 수 없다**고 지적했다(3차 P-4). 세션 태스크 트래커는 저장소
// 밖이라 다음 사람이 못 본다 — **#109 가 잊힌 경로가 정확히 그것이다.** 확인할 수 없는
// 주소는 주소가 아니다.
//
