// world-glb/decide/parcel-layout.ts — 파셀 배치 계산. 순수 함수만.
//
// ── 왜 배치가 판정 계층인가 ──────────────────────────────────────────────────
...
//
// ── 두 가지 불변식 ───────────────────────────────────────────────────────────
// ① **좌표 결정론 — 단, 동결된 파셀은 예외다.** 같은 (px,pz)는 언제나 같은 배치를 낸다.
//    저장하지 않고 매번 다시 계산하므로("파라미터가 곧 공간"), 이게 깨지면 파셀이
//    언로드/재로드될 때마다 세상이 바뀐다.
//    ⚠ **예외가 2026-08-13 에 열렸다**(팀장 판정 2026-08-12 의 집행). 감독이 마을 건물을
//    손으로 옮긴 파셀은 «저장된 배치» 를 쓴다 — 감독 판정 «손본 구역은 슬라이더에서
//    빠진다». 근거·설계의 SSOT 는 **`decide/parcel-freeze.ts` 헤더 한 곳**이고 여기에
//    다시 적지 않는다. 소비 지점은 `systems/parcel-builder.ts` 의 `fill()` 하나다.
//    ⚠ **예외는 파셀 단위이고 전역이 아니다.** 동결하지 않은 파셀은 그대로 계산된다.
//    그리고 동결은 **이 함수 «바깥»에서** 적용된다 — 여기에 분기를 넣으면 골든 해시가
//    깨지고, 그때 고칠 것은 해시가 아니라 설계다. **이 함수는 동결을 모른다**(그래서
//    골든 해시가 이 개정 뒤에도 그대로다 — 테스트가 그것을 지킨다).
