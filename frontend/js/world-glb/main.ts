// world-glb/main.ts — Composition Root. **조립만 한다.**
//
...
import { blockWalkFor } from './systems/glb-walkblock.js';
...
export async function startGlbWorld(
  ...
  // ── 🔴 world8 의 갈림: 충돌이 **파셀 목록이 아니라 GLB** 를 본다 ──────────
  // world2 의 `createCollider` 는 파츠별 원형 footprint 를 배치 목록에서 읽는데 GLB 에는
  // 그 목록이 없다. 구현은 갈리지만 **계약(`Collider`)은 같다** — 그래서 플레이어·입력·
  // 조이스틱은 이 갈림을 모른다(포크 규율: 세계 소스 한 축).
  //
  // ⚠ **지연 프록시인 이유**: GLB 는 `stream` 단계에서야 도착하는데 `collider` 는 여기서
  // 플레이어에 주입된다. 프록시가 없으면 「로드 전에는 충돌 없음」을 호출부마다 분기해야
  // 하고, 그러면 그 분기가 세계 소스 축 **밖으로** 번진다.
  let glbCollider: Collider | null = null;
  const collider: Collider = {
    // 아직 GLB 가 없으면 **그냥 통과시킨다.** 부팅 중이라 화면도 로딩이고, 여기서 막으면
    // 「스폰 자리에서 안 움직인다」가 된다.
    resolve: (x, z, dx, dz) => glbCollider?.resolve(x, z, dx, dz) ?? { x: x + dx, z: z + dz },
    count: () => glbCollider?.count() ?? 0,
    invalidate: () => glbCollider?.invalidate(),
  };
  ...
  // ── 안개 거리·`?fogd=` 노브 ────────────────────────────────────────────────
  // **경위·실측·노브의 한계(무엇이 이 노브로 안 사라지는가)는 `decide/fog.ts` 의
  // 「`?fogd=` 노브」 절 한 곳**이다. 그 파일이 안개 거리 SSOT 이므로 기록도 거기 있다.
  ...
  /**
   * 얹힌 GLB 세계. **이 포크에서 `streaming`·`builder` 가 있던 자리다.**
   *
   * world2 는 여기에 `StreamingSystem`·`PooledParcelBuilder`·`ParcelFadeSystem`·
   * `ParcelGrowSystem`·`ShadowDecalSystem`·`SlotPool` 여섯을 들고 있다 — 전부 **파셀을
   * 세우고 뜨고 지게 하는** 물건이라, 세계가 GLB 한 덩어리인 이 페이지에는 대상이 없다.
      ...
      pools: () => {
        ...
        features = mountFeatures(
          FEATURES,
          {
            ...
            // ⚠ **이 세계에는 파츠 슬롯이 없다** — 세계가 GLB 한 덩어리라 옮길 슬롯이
            // 애초에 생기지 않는다. 계약을 지키려고 no-op 을 둔다(빼면 편집 기능이
            // `undefined` 를 부른다). world2 에서는 여기가 `slotPool.retarget` 이다.
            retargetSlot: () => {},
            setTouchEditing: (on) => { touch?.setEditing(on); },
            sun: sun!, hemi: hemi!, cell: CELL_X,
            // ⚠ **파셀 개념이 없으므로 언제나 `false` 다.** world2 에서는 스트리밍에
  ...
  const exportPanel = attachExportPanel(document, {
    ...
    applyOverlay: (o) => {
      glbHost.set(o);
      // ⚠ **`streaming.rebuildAll()` 이 없다** — 다시 세울 파셀이 없다. 충돌 캐시는
      // 그대로 버린다(GLB 를 다시 훑는다).
      collider.invalidate();
      kernel?.markDirty();
    },
  ...
  (window as unknown as Record<string, unknown>).__glbWorld = {   // 이름은 트리와 같다
    ...
    stats: () => ({
      ...
      // ⚠ **파셀 페이드가 없다** — 뜨고 지는 파셀이 없으므로 `pending` 은 개념상 `null`
      // 이다. 노브 값(`?lodfade=`·`?lodease=`)은 그대로 읽어 보고한다 — 그것은 이 세계에도
      // 있는 설정이고, 소비자가 없다는 사실이 값을 없앤 것은 아니다.
      fade: { pending: null, duration: lodFade, ease: lodEase, gated: false },
  ...
  return {
    ...
    dispose() {
      ...
      // 조명 타깃도 씬에서 뺀다. 지금은 `startWorld8` 가 페이지당 한 번뿐이라 **도달하지
