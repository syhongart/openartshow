...
//
// ── ✅ 마지막 열이 이 회차의 판정 축이었고, **감독이 ×1 로 정했다** (2026-09-19) ──
...
// 그래서 배포 후 `?walkcell=` 네 후보를 **링크로 동시에** 드리고 카드로 물었다.
//
//   **감독 카드 판정 2026-09-19 — 「×1 — 지금 기본」.** 기본값이 이미 ×1 이므로 값은
//   안 움직였다. 그러나 **판정이 없던 값과 판정을 받은 값은 다른 것**이다 — 앞엣것은
//   다음 사람이 「아직 안 정해졌네」로 읽고 또 한 바퀴 돌리지만, 뒤엣것은 닫혔다.
//
// ⚠ **내 유도는 뒤집히지 않았다 — 그러나 그것이 유도가 옳았다는 증거는 아니다.**
// 유도는 「좁은 통로가 살아남을 상한」만 말하고 벽 옆 틈은 셈에 없었다. 감독이 ×1 을
// 고른 것은 그 표의 두 대가(실내 누수 1.9% vs 골목 손실 25.3%)를 화면에서 견준 결과이지
// 유도식을 승인한 것이 아니다. **새 자산에서는 그 대가의 균형이 달라질 수 있다** —
// 도로망이 성기거나 실내가 많은 세계가 오면 이 판정은 그 자산에 대해 다시 열린다.
//
// ── 🔴 **여기서 멈춘다** ────────────────────────────────────────────────────
// 「칸을 몇으로 하느냐」로 실내 누수를 더 줄이려 들지 마라. 이 축은 **네 후보를 실측하고
// 감독이 고르는 것으로 끝났다.** 실내를 제대로 막으려면 칸 크기가 아니라 **「실내」의
// 정의**가 필요하고(지붕이 있는가 · 천창은 · 어느 높이까지), 그것은 이 회차의 두 축에서
// 유도되지 않는 **재론 회차**다 — 경계 문언은 `decide/walkable.ts` 의 `pruneUnreachable`
// 한 곳이다. 이 저장소는 밤 지면 밝기에서 같은 고리를 **네 바퀴** 돌았고, 그것을 끊은
// 것도 감독의 한 문장이었다.
//
