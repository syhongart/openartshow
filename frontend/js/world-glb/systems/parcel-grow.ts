// world-glb/systems/parcel-grow.ts — 새로 태어난 부품을 **땅에서 자라나게** 한다(스케일 인).
//
...
export interface ParcelGrowOptions {
  ...
  duration: number;
  /**
   * 수축 시간(초). 안 주면 `SHRINK_SECONDS`. **`?shrink=` 가 그대로 들어온다 — `0` 포함.**
   *
   * ── 🔴 검수관 블로커 D2 (2026-08-20) ──────────────────────────────────────
   * 그전에는 조립부(`main.ts`)가 값이 양수일 때만 넘기고 아니면 `undefined` 로 떨어뜨려
   * **0 을 흡수**했다. `0` 이 「노브 미지정」을 뜻하던 시절에는 맞는 코드였다 —
   * **퇴장 수축 승격이 그 전제를 뒤집었다**(`readParcelAnim()` 의 기본이 `SHRINK_SECONDS`
   * 가 되면서 `0` 은 **「명시적으로 끔」**이 됐다). 흡수가 남아 `?shrink=0` 이 여기 못
   * 닿았고, 그러면 **되돌려 보려는 사람이 반쪽 화면으로 판정**한다. 이 승격의 알려진
   * 위험이 「후진 중 그림자 깜빡임」이고 그때 `?shrink=0` 이 롤백 수단이라 중요하다.
   *
   * 폴백이 `??`(nullish)인 것이 요점이다: `0` 은 유효한 값이라 통과하고 「안 줌」만
   * 기본값으로 간다. `||` 로 바꾸면 0 이 삼켜진다 — `tests/world2-parcel-grow.test.ts`
   * 의 **GS-S2ⓐ** 가 그 형태를 잡는다(실측: `?? → ||` 로 1 failed).
   *
   * ⚠ 조립부가 다시 걸러내는지는 **GS-S2ⓑ**(`world2-lod-fade.test.ts`)가 본다 —
   * 「없어야 할 삼항이 없는가」와 「있어야 할 전달이 있는가」 **두 축**이다. 첫 판본은
   * 앞만 걸었고 전달을 끊는 형태를 못 잡았다(실측 0 failed).
   */
  shrinkSecs?: number;
  /**
