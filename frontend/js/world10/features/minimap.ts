// world10/features/minimap.ts — 미니맵. 감독 지시로 world1 것을 world2 에 되살린다.
//
...
// ── ⚠ **색 여덟을 지웠다** (검수관 P2, 2026-08-26) ──────────────────────────
// `land`·`road`·`water`·`shore`·`plaza`·`plazaEdge`·`fountain`·`clock` 여덟이
// **소비자 0** 인 채 남아 있었다. 절차적 파셀 지도(`mapCells`)와 랜드마크 표시를 걷어낸
// 자리인데 색만 남은 것이다.
//
// **왜 지우는가**: 각각에 *"물과 뭍의 대비가 지도에서 곧 「세계의 끝」이다"*·*"광장 —
// 감독 지시로 강조한다"* 같은 **판정 주석**이 붙어 있었다. 다음 사람이 `COLOR.water` 를
// 고치고 화면이 안 변하는 것을 보면 **자기 눈이나 빌드를 의심하지 코드가 죽었다고는
// 생각하지 않는다.** 그리고 `.ts` 는 ESLint 대상 밖이라 아무도 안 잡는다.
//
// 지금 지도 색은 **GLB 재질에서 온다**(`systems/glb-minimap.ts` 의 `colorOf`) — 세계가
// 무슨 색인지는 그 파일이 정하지 이 상수가 정하지 않는다.
const COLOR = {
  /** 지도 바탕 — 세계 밖이거나 아직 안 구워진 자리다 */
  bg: '#0d1016',
  /** 내 위치 마커 */
  me: '#ffffff',
} as const;
