...
export const CSS = `
#wg-edit{position:fixed;z-index:40;font:11px/1.35 system-ui,sans-serif;
...
#wg-edit[data-open="0"]{width:auto;padding:0;background:none;border:0;backdrop-filter:none;overflow:visible}
#wg-edit[data-open="0"] .body{display:none}
#wg-edit .head{display:flex;gap:6px;align-items:center}
#wg-edit h4{margin:0;font-size:11px;letter-spacing:.04em;color:#8B72FF;flex:1 1 auto}
#wg-edit[data-open="0"] h4{display:none}
#wg-edit .row{display:flex;gap:4px;flex-wrap:wrap;margin:4px 0}
...
#wg-edit .tabbar{display:flex;gap:3px;padding:4px 0 5px;margin:2px 0 0}
...
#wg-edit .toolbar{position:sticky;top:0;z-index:1;background:rgba(11,13,18,.96)}
#wg-edit .tabbar button{flex:1 1 0;min-width:0;padding:5px 2px;font-size:10px;
  white-space:nowrap;overflow:hidden}
/* 안 고른 탭은 테두리를 지운다 — 넷이 다 상자면 어느 것이 켜졌는지가 색 하나에만
   걸리고, 그 대비는 야외 화면에서 가장 먼저 사라지는 신호다. */
#wg-edit .tabbar button[data-on="0"]{background:none;border-color:transparent;color:#9A9EB1}
#wg-edit .tabbar button[data-on="0"]:hover{border-color:#3A3D4B}
/* 「지금 볼 게 없다」 — 흐리게만 한다. 못 누르게 막지 않는 근거는 tabHasContent 주석. */
#wg-edit .tabbar button[data-empty="1"][data-on="0"]{opacity:.45}
#wg-edit .pane[data-on="0"]{display:none}
/* 🔴 hidden 이 실제로 감추게 한다.
   ...
   *"#wg-edit .row 와 구체성이 (1,1,0) 으로 같으니 뒤에 오는 쪽이 이긴다 — 그래서 이 줄이
...
#wg-edit [hidden]{display:none!important}
/* 탭 안 첫 요소의 위 여백을 지운다 — 바 아래 간격이 두 겹이 된다 */
#wg-edit .pane > :first-child{margin-top:0}
#wg-edit button{flex:1 1 auto;min-width:30px;padding:4px 5px;font:11px/1 system-ui,sans-serif;
  color:#F5F5F2;background:#1A1D26;border:1px solid #3A3D4B;border-radius:6px;cursor:pointer}
#wg-edit button:hover{border-color:#8B72FF}
#wg-edit button[data-on="1"]{background:#8B72FF;border-color:#8B72FF;color:#0B0D12}
/* 토글 버튼은 접혔을 때 유일하게 보이는 것이라 손가락이 닿을 크기여야 한다 */
#wg-edit .toggle{flex:0 0 auto;padding:8px 12px;font-size:12px;
  background:rgba(11,13,18,.86);border-color:#3A3D4B;backdrop-filter:blur(6px)}
#wg-edit[data-mode="edit"] .toggle{background:#8B72FF;border-color:#8B72FF;color:#0B0D12}
/* ── 사진 걸기 (W8-5 · 폰 경로) ────────────────────────────────────────────
...
#wg-edit .photo-in{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}
/* 폰에서 감독이 가장 자주 누를 버튼이다. 위 .toggle 과 **같은 이유**로 키운다 —
   나머지 버튼의 4px 패딩은 손가락 타깃으로 작다. */
#wg-edit .photo button{padding:8px 10px;font-size:12px}
/* ── 조준 중에는 패널을 접는다 (W8-6) ──────────────────────────────────────
...
#wg-edit[data-aim="1"] .body{display:none}
#wg-edit[data-aim="1"]{width:auto;padding:0;background:none;border:0;backdrop-filter:none}
#wg-edit .note{color:#9A9EB1;margin:4px 0 0}
#wg-edit .warn{color:#FFC46B}
#wg-edit .lead{color:#F5F5F2;margin:6px 0 0;font-size:12px}
#wg-edit .sel{color:#72E6E1}
#wg-edit hr{border:0;border-top:1px solid #3A3D4B;margin:6px 0}
#wg-edit .pal button{flex:1 1 100%;text-align:left}
/* ── 에셋 라이브러리 (W6 E) ────────────────────────────────────────────────
   검색칸 · 카테고리 제목 · 색 스와치. 목록이 길어지므로 팔레트에 최대 높이를 주고
   그 안에서만 스크롤한다 — 안 주면 파츠 8개 + GLB 가 패널을 화면 밖까지 밀어낸다. */
#wg-edit .pal{max-height:34vh;overflow-y:auto}
#wg-edit .pal .find{width:100%;box-sizing:border-box;padding:4px 6px;margin:0 0 4px;
  font:11px/1.2 system-ui,sans-serif;color:#F5F5F2;background:#1A1D26;
  border:1px solid #3A3D4B;border-radius:5px}
#wg-edit .pal .find:focus{outline:none;border-color:#8B72FF}
#wg-edit .cat{display:flex;flex-wrap:wrap;gap:3px}
#wg-edit .cat-h{flex:1 1 100%;color:#9A9EB1;font-size:10px;margin:4px 0 1px}
/* 스와치는 라벨 앞 작은 사각. 파츠에만 붙는다 — 「어떤 계열인가」를 공짜로 말한다 */
#wg-edit .sw{display:inline-block;width:9px;height:9px;margin-right:5px;
  border-radius:2px;border:1px solid rgba(0,0,0,.35);vertical-align:-1px}
/* 수치 칸 — 다섯이 한 줄에 들어가야 패널이 안 길어진다 */
#wg-edit .insp{gap:3px}
#wg-edit .fld{flex:1 1 0;min-width:0;display:flex;flex-direction:column;gap:2px}
#wg-edit .fld span{color:#9A9EB1;font-size:10px;text-align:center}
#wg-edit .fld input{width:100%;min-width:0;box-sizing:border-box;padding:3px 4px;
  font:11px/1 system-ui,sans-serif;color:#F5F5F2;background:#1A1D26;
  border:1px solid #3A3D4B;border-radius:5px}
#wg-edit .fld input:focus{outline:none;border-color:#8B72FF}
#wg-edit .fld input:disabled{opacity:.4}
...
#wg-badge{position:fixed;z-index:45;pointer-events:none;
  left:50%;transform:translateX(-50%);
  top:calc(10px + env(safe-area-inset-top,0px));
  max-width:min(60vw,520px)}
/* 안 골랐거나 주행 중이면 없다 — 「선택: 없음」을 크게 띄우지 않는다 */
#wg-badge[data-on="0"],#wg-badge[data-mode="drive"]{display:none}
#wg-badge .name{
...
/* ── 조준 화면 (W8-6) ──────────────────────────────────────────────────────
   ...
   **조준 대상을 볼 수 없다** 였고, 이 화면이 그것을 보여준다.
   ...
   ⚠ 뿌리(#wg-aim)는 화면 전체를 덮되 **클릭을 안 먹는다**(pointer-events:none).
...
#wg-aim{position:fixed;inset:0;z-index:46;pointer-events:none;display:none}
#wg-aim[data-on="1"]{display:block}
/* 조준선 — 화면 **정중앙**. castCenter() 가 실제로 쏘는 그 지점이다.
   ⚠ 이 위치가 광선과 어긋나면 화면이 거짓말을 한다 — 둘 다 「한가운데」로 정의돼야 한다. */
#wg-aim .cross{position:absolute;left:50%;top:50%;width:42px;height:42px;
...
#wg-aim[data-hit="1"] .cross{border-color:#72E6E1;width:54px;height:54px;margin:-27px 0 0 -27px;
...
#wg-aim .bar{position:absolute;left:50%;transform:translateX(-50%);
...
#wg-aim .msg{flex:1 1 100%;text-align:center;color:#9A9EB1;
  font:11px/1.4 system-ui,sans-serif;text-shadow:0 1px 3px rgba(0,0,0,.7)}
#wg-aim[data-hit="1"] .msg{color:#72E6E1}
/* 손가락 타깃 — 패널 버튼(4px)보다 크다. 폰에서 누르는 것이 이 둘뿐이다. */
#wg-aim .bar button{padding:9px 15px;font:12px/1 system-ui,sans-serif;color:#F5F5F2;
  background:#1A1D26;border:1px solid #3A3D4B;border-radius:8px;cursor:pointer}
#wg-aim .bar button[data-on="1"]{background:#8B72FF;border-color:#8B72FF;color:#0B0D12}
...
#wg-edit .surf{display:flex;flex-direction:column;gap:5px}
#wg-edit .surf .lbl{color:#9A9EB1;font-size:10px;min-width:44px}
#wg-edit .surf .num{color:#F5F5F2;font-size:10px;min-width:34px;text-align:right}
#wg-edit .surf-pick{display:flex;flex-wrap:wrap;gap:3px}
#wg-edit .surf-pick button[data-on="1"]{background:#8B72FF;border-color:#8B72FF;color:#0B0D12}
#wg-edit .surf-row{display:flex;align-items:center;gap:5px}
...
#wg-edit .size-row .lbl{color:#9A9EB1;font-size:10px;min-width:28px}
#wg-edit .size-row .num{color:#F5F5F2;font-size:10px;min-width:44px;text-align:right}
#wg-edit .tone-box{display:flex;flex-wrap:wrap;gap:4px;flex:1;min-width:0}
...
#wg-edit .tone-sw{width:18px;height:18px;padding:0;border-radius:3px;
  border:1px solid #2A2F3E;cursor:pointer}
#wg-edit .tone-sw[data-on="1"]{border-color:#8B72FF;box-shadow:0 0 0 1px #8B72FF}
#wg-edit .surf-row input[type="range"]{flex:1;min-width:0;accent-color:#8B72FF}
#wg-edit .surf-row button[data-on="1"]{background:#8B72FF;border-color:#8B72FF;color:#0B0D12}
#wg-edit .surf-slots{display:flex;flex-direction:column;gap:3px}
#wg-edit .surf-slot{display:flex;align-items:center;gap:5px;padding:3px 5px;
  border:1px dashed #3A3D4B;border-radius:6px}
#wg-edit .surf-slot[data-on="1"]{border-style:solid;border-color:#8B72FF}
/* 끌고 있는 동안 칸이 스스로 «여기» 라고 말한다 — 드롭 대상이 곧 «어디에 바르는가» 라서
   그 피드백이 없으면 감독이 엉뚱한 칸에 놓는다. */
#wg-edit .surf-slot[data-over="1"]{background:rgba(139,114,255,.22);border-color:#8B72FF}
#wg-edit .surf-slot select{flex:1;min-width:0;font:10px/1.3 system-ui,sans-serif;
...
#wg-edit .surf-slot .sname{font-size:10px;color:#8B72FF;white-space:nowrap}
#wg-edit .surf-slot button{padding:1px 6px;line-height:1.2}
#wg-outliner{display:none}
@media (min-width:1024px){
  #wg-outliner{display:flex;flex-direction:column;gap:4px;position:fixed;z-index:40;
  ...
  #wg-edit[data-mode="drive"] ~ #wg-outliner,
  #wg-outliner[data-mode="drive"]{display:none}
  #wg-outliner h4{margin:0;font:600 12px/1.2 system-ui,sans-serif;color:#8B72FF}
  /* 작품 칸은 구역 목록 **아래**에 이어 붙는다(W8-11). 두 번째 제목부터 위 여백을 줘
     두 목록이 한 덩어리로 읽히지 않게 한다 — 같은 상자 안이라 구분선 대신 여백이다. */
  #wg-outliner h4 ~ h4{margin-top:6px}
  #wg-outliner .items{display:flex;flex-direction:column;gap:2px}
  #wg-outliner .items button{text-align:left;padding:3px 6px;
  ...
  #wg-outliner .items button[data-on="1"]{background:#8B72FF;border-color:#8B72FF;color:#0B0D12}
  #wg-outliner .note{color:#9A9EB1;margin:0;font-size:10px}
}
`;
