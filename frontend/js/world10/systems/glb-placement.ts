// world10/systems/glb-placement.ts — **낱개 메시와 인스턴스를 같은 것으로 다룬다.**
//
 ...
 *
 * ⚠⚠ **못 보는 것: 음수(미러링) 스케일**(검수관 권고 P2). `hypot` 은 언제나 양수라
 * 부모가 y 를 뒤집어 놓은 GLB 에서는 「위」가 반대로 해석된다. 지금 막지 않는 이유는
 * 그런 자산을 만난 적이 없고 방향을 되살리려면 행렬식 부호까지 봐야 해서다 — **막지
 * 않기로 한 것이지 안전한 것이 아니다.** 그런 GLB 에서 그림자가 지면에 파묻히면
 * 이 자리를 먼저 본다.
 */
