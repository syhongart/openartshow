// world-glb/decide/adapt.ts — 적응 품질 판정. 순수 함수만, import 0.
//
...
/**
 * ⚠ **유도되지 않은 값 · 관측 전용** (팀장 조건 C2, 2026-09-05 명기).
 *
 * 60,000 에는 식도 실측 근거도 없다 — 위 문단의 실측(*"tri 를 84,526 에서 41k 로 절반 깎았는데
 * fps 는 25→26"*)은 오히려 **삼각형이 병목이 아니었다**는 관측이고, Decentraland 원칙은 「동시
 * 렌더 삼각형을 예산으로 삼는다」는 원리이지 이 숫자가 아니다. 2026-09-05 헤드리스 실측
 * (백로그 G-W8N 표): 라이브 world2 기본이 **1,339,882**(22배), 그중 90% 가 잔디다.
 * 이 상수가 만드는 판정은 `pressure()` 의 축소 계수뿐이고 그것도 `applyTierPressure` 가
 * 어느 트리에서도 주입되지 않아 **실제로 아무 화면도 바꾸지 않는다**(grep 0건). 즉 예산을
 * 어겨도 FAIL 이 나지 않는다 — 이것을 게이트로 읽지 마라. 값을 정하려면 실기기 프레임과
 * 함께 유도한다(검증 규율 *"실측에 여유를 얹은 값은 근거가 아니다"*).
 */
