// world-glb/decide/water.ts — 어디가 물인가. 순수 함수만.
//
// ── 이 파일이 SSOT 인 이유 ───────────────────────────────────────────────────
...
// 이 파일은 **world3/decide/water.ts 와 사실상 쌍둥이**다(diff **hunk 1개 · 4줄**).
// 여기를 고치면 저쪽도 고쳐야 한다 — 안 그러면 조용히 갈라진다.
//
// **통합은 재상정 대기 중이다**(팀장 판정 2026-08-16). 지금 막힌 이유는 이 파일이
// 세계 내부 모듈 7개에 의존하고 있어서다. 혼자 떼어낼 수 없으므로 통합은 선례
// (`frontend/js/world-shared/glb-city.ts` — 주입으로 세계 차이를 뺀 방식)를
// 따를 것이다.
...
/**
 * 물 판의 한 변(미터). **세계 지름의 여러 배**여야 한다.
 *
 * 가장자리에 서서 바깥을 볼 때 물이 끝나고 허공이 보이면 유한 세계가 아니라 부서진
 * 세계로 읽힌다. 안개가 60.8m 에서 모든 것을 덮으므로 실제로 필요한 건 `세계 반경 +
 * 안개 거리` 남짓이지만, 판 하나 늘린다고 드는 비용이 삼각형 두 개뿐이라 넉넉히 잡는다.
 *
 * ── 왜 `features/ocean.ts` 가 아니라 여기인가 ────────────────────────────────
 * 원래 이 식은 `ocean.ts` 의 `PLANE` 하나였다. GLB 내보내기(`export/collect.ts`)가
 * 물판을 함께 구우면서 **같은 식이 두 곳에 생겼고**, 그것이 곧 값 미러링이다 — 세계를
 * 넓힐 때 한쪽만 따라오면 내보낸 파일에서만 바다가 모자란다. 어느 쪽도 상대를 import
 * 할 수 없어서(수집기는 three import 0, ocean 은 three 의존) 공통 조상인 여기로 올린다.
 * 물의 치수는 원래 이 파일 소관이기도 하다(`SEA_Y`·`WATER_DEPTH`·`SEABED_Y`).
 */
export function waterPlaneSpan(cell: number): number {
  return worldHalfExtent(cell) * 4;
}
 ...
 * 스폰은 `SPAWN` 의 **z 가 양수**이고 기본 시선 `yaw = 0` 은 **-z** 를 본다(`grid.ts`).
 * (여기 원래 `{x:0, z:10}` 이라고 값을 적어 뒀는데, 충돌 도입으로 x 가 -3.5 로 옮겨진
 *  2026-08-08 에 **거짓이 됐다** — 이 논증에 필요한 것은 z 의 부호뿐이므로 값을 지운다.)
...
// ── 수면 광택 ───────────────────────────────────────────────────────────────
// `decide/water-gloss.ts` 로 떼어냈다(2026-08-20 — 축이 다르고 이 파일이 570줄이었다).
// **재수출로 배럴을 유지한다** — 소비자 import 경로를 안 건드리기 위해서다.
export { waterGloss } from './water-gloss.js';
export type { WaterGloss } from './water-gloss.js';
