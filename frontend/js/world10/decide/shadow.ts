 ...
 *
 * @param cell       파셀 한 변(m). **호출자가 자기 세계의 셀을 넘긴다.**
 * @param band       담을 범위를 셀의 몇 배로 볼 것인가. 호출자의 밴드에서 온다.
 *
 * ⚠ **이 두 줄은 오래 «`DEFAULT_LAYOUT` 에서 온다» · «`DEFAULT_BANDS.nearExit` 를 쓴다» 라고
 * 적고 있었고 world10 에서 둘 다 거짓이었다**(2026-09-11 실측). 밴드는 2026-08-10 에 이미
 * `nearExit` → `farExit` 로 바뀌어 있었고(바로 아래 절이 그 경위다), 셀은 world10 이
 * `DEFAULT_LAYOUT.cellX`(32m)를 넘기는 바람에 **세계의 셀(64m)과 달랐다.** 두 오기가 서로를
 * 가려 반폭이 «우연히» 76.8m 로 맞아 있었다 — 32×2.40 = 76.8. 이 저장소가 「두 버그가 서로를
 * 상쇄하고 있을 때 하나만 고치는 것은 고치기 전보다 나쁠 수 있다」고 적어 둔 그 형태다.
 *
 * B1 **사후 추인**으로 world10 호출부가 **밴드와 셀을 함께** 자기 것으로 바꿨다
 * (`NYC_BANDS.farExit × CELL_X` = 1.20×64 = 76.8m). **반폭은 한 밀리미터도 안 움직인다** —
 * 값이 아니라 출처를 옮긴 것이고, 그래야 셀이나 밴드를 다시 만질 때 그림자가 «파셀이
 * 사라지는 거리» 를 실제로 따라간다. 밴드만 바꾸고 셀을 두면 반폭이 절반(38.4m)이 되어
 * 이 절이 없앤 그 경계가 **화면 안쪽으로 되돌아온다.**
 *
 * ⚠ 위 문단 첫 줄은 «팀장 판정 2026-09-11 B1 로» 라고 적고 있었고 그렇게 읽으면 안 된다 —
 * **B1 원문이 지목한 것은 밴드뿐이고 셀 인자는 아니다.** 셀 교체는 B-1 을 넘은 편집이라
 * 집행 후 사후 통지로 추인받았다(게시판 2026-09-11 팀장 글 «B-1 을 넘은 편집 2건 추인»).
