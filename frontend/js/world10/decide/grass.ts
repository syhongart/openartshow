// world-glb/decide/grass.ts — 잔디 필드의 **판정**. 순수 함수만, three 무의존.
//
...
export const GRASS_RINGS: readonly GrassRing[] = [
  { radius: 14, density: 77, scale: 1.00 },
  { radius: 34, density: 11.2, scale: 1.35 },
  { radius: 70, density: 1.96, scale: 1.90 },
];
 ...
 *
 * ── 실기기 성능: 감독 승인으로 간주한다 (팀장 판정 2026-08-21) ───────────────
 * 밀도를 크게 올린 회차(#209)는 *"성능 미실측 — 감독 실기기가 유일한 판정 축"* 인 채
 * 나갔다. 2026-08-21 에 감독이 `?styl=1&at=sea&time=day`(잔디 포함) 화면을 직접 보고
 * **"응 승인"** 했으므로 그 축이 채워진 것으로 **간주한다.**
 * ⚠ **「간주」다** — 감독이 성능을 명시적으로 판정한 것은 아니다. 프레임률이 문제였다면
 * 승인이 안 나왔을 것이라는 추론이고, 그 추론이 이 자리에 적힌 근거의 전부다.
 * ⚠⚠ **헤드리스는 이 밀도를 못 돈다**(4코어·GPU 0). 그래서 스모크 세션이 잔디를 끈다 —
 * 근거·한계·재론 트리거는 `scripts/smoke/world2-ready.mjs` 의 `WORLD2_QUERY` 한 곳이다.
 */
export const MAX_BLADES = 155000;
...
// **폭과 끝은 노브로 열어 둔다** — 화면으로만 판정되는 값이고 기본값은 감독이 실기기에서
// 정한다(`url-knob.ts`). ⚠ 2026-09-05 전제 변화(2D 마디 1)와 재판정은 `grass-mode.ts` «BLADE_W 재판정» 절.
