// world-glb/decide/artwork.ts — **벽에 거는 작품.** 순수 함수만 (W8-4).
//
...
/**
 * 걸린 작품의 **자세**(W8-11) — 편집이 미는 것만 추린 부분집합.
 *
 * ⚠ **`ArtworkItem` 에서 유도한다**(`Pick`). 여기에 필드를 다시 적으면 그것이 값
 * 미러링이고, 계약에 필드가 하나 붙는 날 한쪽만 늘어난다 — 이 저장소가 색·고도·임계값
 * 에서 **세 번** 겪은 형태다.
 *
 * `src`·`ar` 이 빠진 것은 의도다: `src` 는 «어느 그림인가» 라 자세가 아니고, `ar` 은
 * 이미지에서 읽은 값이라 사람이 만질 것이 아니다(`w` 하나로 크기가 정해진다).
 */
export type ArtPose = Readonly<Pick<ArtworkItem, 'x' | 'y' | 'z' | 'ry' | 'w'>>;

/**
 * 작품을 화면에서 부르는 **짧은 이름** — `src` 의 마지막 조각.
 *
 * ⚠ **여기 한 곳이다.** 소비자가 셋이고(아웃라이너 줄·패널 한 줄·배지) 각자 자르면
 * 「같은 작품인데 화면마다 다르게 불린다」가 난다 — 오버레이가 `assets/models/` 접두를
 * 세 곳에서 각자 지우고 있는 것이 이미 그 형태다(그쪽은 이번 범위가 아니다).
 *
 * 접두 규약을 파싱하지 않고 `/` 로만 자른다 — 경로 형식은 `decide/upload-plan.ts` 가
 * 소유하고, 여기서 그것을 다시 알면 한쪽만 고쳐도 아무도 모른다.
 */
export function artName(src: string): string {
 ...
 *
 * ⚠ **이 판정은 「수직인가」만 본다 — 「벽인가」는 안 본다.** 나무 줄기의 옆면도 수직이라
 * 그대로 통과한다. 2026-08-18 라이브 실측으로 실물이 나왔다: 첫 배포된 작품 두 장 중
 * 한 장이 파셀(0,0)의 나무에 걸려 있었고(액자 중심에서 줄기까지 **0.28m**, 나무 반경
 * 안이다), 감독이 *"액자가 어딧어?"* 로 신고했다. 화면에서는 «허공에 뜬 액자» 로만
 * 보인다 — 줄기가 액자(w 2.4m)보다 훨씬 얇아 뒤에서 가려지지 않기 때문이다.
 *
 * **고치지 않았다.** 종류를 보려면 이 순수 함수가 파츠 종류(`owner.key`)를 알아야 하고,
 * 그것은 판정 계층이 마을 자산 계층에 붙는다는 뜻이다. 그리고 **막는 것이 옳은지도
 * 아직 모른다** — 나무에 그림을 거는 것을 사용자가 원할 수 있다(감독은 이 건을
 * *"3은 뭐 나중에 사용자가 설치하겠지"* 로 넘겼고, 즉 배치는 사용자 몫이라는 판정이다).
 * **재론 트리거**: 사용자가 «나무에 걸렸다» 를 결함으로 신고하면 그때 종류 판정을 연다.
 */
