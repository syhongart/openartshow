// world10/decide/fog.ts — **세계가 어디까지 보이는가.** 순수 함수만.
// ⚠ 이 줄은 오래 *"import 0"* 이라고 적고 있었고 **거짓이었다** — 값을 렌더 반경에서
//   유도하는 이상 밴드를 import 한다(2026-08-02 검수관 B4 가 그렇게 만들었다).
//
// ── 왜 생겼나 (팀장 판정 2026-08-02, 본편 ①) ────────────────────────────────
// 안개 밴드가 `main.ts` 안에 인라인으로 계산돼 있었다:
//
//     scene.fog = new THREE.Fog(0x0b0d12, CELL_X * 1.6, CELL_X * 2.4);
//
// **저장소에서 이 두 배수를 아는 곳은 그 한 줄뿐이었다.** 그래서 "안개 밖" 을 판단해야
// 하는 다른 모듈은 그것을 알 방법이 없었고, 실제로 NPC 가 자기 나름의 거리를 썼다 —
// 그 어긋남이 이번 회차가 찾아낸 결함이다:
//
//   `npc.ts` 의 은닉 임계  = RECYCLE_CELLS × cell = 102.4m   ← 재배치 거리
//   실제 안개 차단          = 2.4 × cell           =  76.8m
//
// 주석은 *"안개 밖이면 끈다"* 라고 적혀 있었다. **문장은 옳았고 값이 딴 것을 가리켰다.**
// 그 사이 25.6m 구간의 NPC 는 화면에 한 픽셀도 기여하지 못하면서 체당 45 드로우콜을
// 냈고, 그 수가 `shown` 진단으로 나가 **부팀장의 판정을 한 번 오도했다**(`shown=7` 을
// "안개 안" 으로 읽어 "멀어서 안 보인다는 배제됐다" 고 적었다 — 틀렸다).
//
// ── 그래서 배수를 여기 한 곳에 둔다 ─────────────────────────────────────────
// 소비자는 **거리 숫자를 자기 파일에 적지 않는다.** `fogBand(cell)` 을 부른다.
// 팀장 조건: *"51.2 / 76.8 을 NPC 쪽에 숫자로 적지 않는다. 주석에 거리 숫자를 적는
// 것도 미러링이다 — 이번 사고가 정확히 '주석이 안개를 말하는데 값은 102.4' 였다."*
//
// ── 무엇을 안 정하는가 ──────────────────────────────────────────────────────
// 안개 **색**은 여기 없다. 색은 시간대·날씨에 따라 `sky.js` 가 크로스페이드로 바꾸고,
// 밤 하한은 `decide/night.ts` 가 따로 본다. 여기가 정하는 것은 **거리**뿐이다 —
// 거리는 시간대와 무관하게 고정이고, 그래서 가시성 판단의 기준이 될 수 있다.

import { NYC_BANDS, NYC_BAND_SCALE } from './nyc-bands.js';

/** 안개 밴드. 셀 크기의 배수로 정의한다 — 세계가 커지면 시야도 같이 커진다 */
export interface FogBand {
  /** 여기서부터 대비가 깎이기 시작한다(m) */
  near: number;
  /** 여기서 완전히 닫힌다. **이 밖은 정의상 화면 기여 0**(m) */
  far: number;
}

/**
 * 안개 시작 배수의 **기준값** — 셀 `BAND_REF_CELL_M`(32m) 세계에서 정해진 수다.
 *
 * 감독 실기기 판정으로 `0.9` 에서 여기까지 왔다(아래 「`?fogd=` 노브」 절의 유도표).
 * 그 판정이 본 것은 배수가 아니라 **화면 위의 거리**였으므로, 셀이 바뀌면 배수 쪽이
 * 움직여야 그 판정이 보존된다 — 그것이 바로 아래 줄이다.
 */
const FOG_NEAR_REF_CELLS = 1.6;

/**
 * 안개가 시작되는 거리 = 셀의 몇 배인가.
 *
 * **가시성의 상한이 아니라 "온전히 보이는" 구간의 상한이다.** 이 밖에 놓인 것은
 * 대비가 깎인 채로 등장하고, 스스로 걸어서 회복할 수 없다면 영원히 그 상태다 —
 * NPC 스폰이 정확히 그 함정에 빠져 있었다(스폰 링의 절반이 이 밖이었다).
 *
 * ── ⚠ **밴드와 같은 배율을 탄다** (B1 **사후 추인**) ───────────────────────────
 * 기준 배수를 이 세계에 그대로 쓰면 안개 시작이 **렌더가 끊기는 거리보다 뒤로** 밀린다
 * (2026-09-11 실측: 렌더 76.8m · 안개 시작 102.4m — 세계의 끝이 안개 없이 드러났다).
 * `FOG_FAR_CELLS` 만 밴드에 물리면 이번에는 near > far 가 되어 안개가 **뒤집힌다** —
 * 즉 이 줄은 선택이 아니라 far 를 밴드에 묶는 순간 함께 따라와야 하는 짝이다.
 * 밴드에 건 것과 **같은** `NYC_BAND_SCALE` 을 걸어 절대 거리를 보존한다.
 *
 * ⚠ 위 제목의 괄호는 «(팀장 판정 2026-09-11 B1)» 이었고 그렇게 읽으면 안 된다 — **B1 원문이
 * 지목한 것은 `FOG_FAR_CELLS` 하나이고 이 줄은 아니다.** 바로 위 이유로(안 하면 near > far)
 * 함께 바꾼 뒤 사후 통지로 추인받았다(게시판 2026-09-11 팀장 글 «B-1 을 넘은 편집 2건 추인»).
 * 판정문에 없는 근거를 판정문으로 적으면 다음 사람이 원문 확인을 생략한다.
 */
export const FOG_NEAR_CELLS = FOG_NEAR_REF_CELLS * NYC_BAND_SCALE;

/**
 * 안개가 완전히 닫히는 거리 = 셀의 몇 배인가.
 *
 * **이 밖을 그리는 것은 낭비다.** 화면에 기여할 수 없다는 것이 안개의 정의이므로,
 * 은닉·컬링 임계를 여기보다 멀리 잡으면 그만큼이 그대로 버려진다.
 *
 * ── 렌더 반경에서 **유도한다** (검수관 B4, 2026-08-02) ──────────────────────
 * 처음엔 `2.4` 를 여기 적었다. 그런데 `decide/lod.ts` 의 `farExit` 도 **2.40** 이고
 * 둘은 같은 경계다 — `main.ts` 주석이 이미 *"끝을 far 렌더 반경에 정확히 맞춘다 …
 * 렌더 반경에서 유도한 값이라 밴드를 넓히면 함께 따라온다"* 고 적고 있었다.
 * **따라오지 않았다.** 같은 리터럴이 두 파일에 각각 있었을 뿐이다.
 *
 * 안개 SSOT 를 세우는 파일이 안개 미러를 옆에 두고 있었던 셈이라, 이제 실제로
 * 유도한다. LOD 가 안 그리기 시작하는 거리와 안개가 닫히는 거리가 **정의상 같아야**
 * 세계의 끝이 안 드러난다 — 안개가 더 멀면 그 사이가 텅 비어 보인다.
 */
export const FOG_FAR_CELLS = NYC_BANDS.farExit;

/**
 * 셀 크기에서 안개 밴드를 유도한다.
 *
 * @param cell 파셀 한 변(m). `DEFAULT_LAYOUT.cellX`
 */
export function fogBand(cell: number): FogBand {
  return { near: cell * FOG_NEAR_CELLS, far: cell * FOG_FAR_CELLS };
}


// ════════════════════════════════════════════════════════════════════════════
// 「`?fogd=` 노브」 — **`world-glb/main.ts` 에서 옮겨 왔다** (2026-08-28).
//
// 감독 지시: *"체크리스트를 다른 파일로 분리하면 되잖아. main을 조금 정리하던가."*
// `main.ts` 가 1,314줄이라 `check:filesize` 가 새 배선을 막았고, 줄을 압축해 통과시키는
// 대신 **기록을 SSOT 옆으로 옮겼다.** 옮긴 자리가 여기인 이유는 원문이 스스로
// *"안개 거리는 여전히 `decide/fog.ts` 가 SSOT 다"* 라고 적고 있었기 때문이다 —
// 값과 기록이 갈라져 있던 것을 붙인 것이지, 기록을 치운 것이 아니다.
//
// ⚠ 노브를 **읽는 코드**는 `main.ts` 에 남는다(URL 접근이라 `decide/` 의 순수성 밖이다).
// 거기에는 이 절을 가리키는 포인터 3줄만 있다.
// ════════════════════════════════════════════════════════════════════════════
//
// ── 안개 거리 (감독 실기기 판정) ──────────────────────────────────────────
// 예전 값은 `0.9 ~ 1.9` 셀 = **28.8m 부터 60.8m** 이었다. 파셀 하나 거리에서 안개가
// 시작되니, 화면이 이렇게 갈렸다:
//
//   가까운 건물 — 태양 반대 면이 검다(그림자 면 대비)
//   조금 먼 건물 — 흰 안개에 묻힌다
//   **중간이 없다**
//
// 스크린샷에서 눈앞 건물이 새까맣고 그 너머 건물이 새하얗게 보인 것이 이것이다.
// 원근이 뒤집힌 것처럼 읽힌다 — 보통은 먼 것이 흐릿하고 가까운 것이 또렷하다.
//
// `1.6 ~ 2.4` 셀 = **51.2m 부터 76.8m**. 시작을 늦춰 또렷한 범위를 1.8배로 넓히고,
// 끝을 far 렌더 반경(76.8m)에 정확히 맞춘다. 그 너머는 파셀이 로드되지 않으므로
// 안개가 딱 그 경계를 덮어야 **세계의 끝이 안 보인다** — 더 늘리면 빈 공간이 드러나고,
// 더 줄이면 지금처럼 답답해진다. 렌더 반경에서 유도한 값이라 밴드를 넓히면 함께 따라온다.
// 안개 거리는 `decide/fog.ts` 가 SSOT 다. 여기 배수를 다시 적으면 "안개 밖" 을
// 판단해야 하는 다른 모듈이 그것을 알 방법이 없어진다 — 실제로 `npc.ts` 가 자기
// 나름의 거리를 쓰다가 25.6m 를 헛그리고 있었다(본편 ①).
// ── `?fogd=` — 안개 거리 배수. **감독 지시 2026-08-07** ────────────────────
//
// 감독: *"lod 가리기 위한 연기 그 값이 문제인듯 그냥 띠로 보여"* →
//       *"안개 가리개가 아니라 **띠 가리개**야. 일단 가리개를 없애보자"*
//
// 위 문단이 *"안개가 딱 그 경계를 덮어야 세계의 끝이 안 보인다"* 고 적고 있는데,
// **바다에서는 그 덮개 자체가 보인다.** 76.8m 는 트인 수면에서 화면상 코앞이라
// 그 밖이 전부 균일한 안개색 — 즉 가리개가 가리는 대신 **띠가 될 수 있다.**
//
// ── ⚠ 이것은 가설이다. 단정하지 마라 (검수관 지적) ─────────────────────────
// 첫 판본은 여기에 *"낮의 띠(하늘 217 / 띠 229.3)가 색공간과 무관하게 남아 있던
// 것이 이것으로 설명된다"* 고 **단정**해서 적었다. 앞 절반은 참이다(`nightFloor`
// 는 낮에 전 채널 0 이라 밤 하한 버그가 낮에 작동하지 않는다 — 검수관 확인). 그러나
// **그렇다고 원인이 안개 거리라는 결론이 따라 나오지 않는다.** 유력한 반례를 하나도
// 배제하지 않은 채 쓴 문장이었다. 이 저장소가 반복해 데인 *"참인 문장에서 성립하지
// 않는 결론을 뽑는"* 형태 그대로다.
//
// ── ★ 배제되지 않은 반례: `HorizonBand` 는 이 노브로 안 사라진다 ────────────
// `systems/horizon.ts` 의 수평선 밴드(#202, **이 저장소에서 문자 그대로 "밴드"**)는
// `?hz=0` 이 아닌 한 항상 생성되고, 색을 **`scene.fog` 에서 가져오지 않는다** —
// `lightOf()` 팔레트를 직접 읽는다(`tests/world2-horizon.test.ts` 가 못박고 있다:
// *"밴드 색은 팔레트에서 온다 — scene.fog 를 경유하지 않는다"*).
//
// 그래서 **`?fogd=0` 을 켜도 밴드는 사라지지도 색이 바뀌지도 않는다.** 오히려
// 세기가 지금 0(감독 확정)이라 `dim=1` 이고, 밴드는 안개색을 **감쇠 없이 그대로**
// 칠한다 — 주변이 안개 없이 원 재질색으로 그려지는 화면에서 밴드만 안개색이면
// **전에 없던 불일치가 새로 생길 수 있다.** `decide/horizon.ts` 가 실측으로 확인한
// *"띠가 원근이 아니라 독립된 물체로 보인다"* 가 정확히 그 형태다.
//
// 감독이 *"띠 가리개"* 라고 부른 것이 **안개인지 이 밴드인지 아직 안 갈렸다.**
// 두 축을 따로 꺼봐야 갈린다:
//
//   ?fogd=0          안개만 끔 — 밴드와 아래 돔 채움은 남는다
//   ?hz=0            밴드만 끔 — 안개와 돔 채움은 남는다
//   ?fogd=0&hz=0     둘 다 끔 — **그래도 돔 채움은 남는다**(아래)
//   (노브 없음)       대조군
//
// ── ★ 세 번째 후보: 두 노브 어느 쪽에도 안 걸린다 (검수관 2차 지적) ────────
// `sky.js:286-296`(⑨ 규칙). **하늘 돔은 완전구**이고 지평선 아래 절반이 팔레트
// `L.fog` 색으로 **단색 채워져** 있다 — 이음새 방지용이다. `scene.fog` 도 아니고
// `HorizonBand` 도 아닌 **제3의 안개색 표면**이고, `?fogd=0&hz=0` 으로도 안 꺼진다.
// 끄는 노브가 없다.
//
// 그 채움이 평소 안 보이는 이유를 주석이 스스로 적고 있다 — **"지면에 가려"**.
// **그런데 이번 조사는 전부 `?at=sea` 다. 바다에는 지면이 없다.** 물 평면
// (`ocean.ts` 의 `PLANE` ≈1920m)이 우연히 근사 차폐를 하고 있을 수는 있으나 그것은
// 검증된 적이 없고 설계 전제도 아니다.
//
// **그러니 `?fogd=0&hz=0` 에서도 띠가 남으면 "미지의 것" 이 아니라 여기부터 본다.**
// 이름 있는 후보를 미지로 남겨 두는 것이 위에서 한 번 데인 그 형태다.
//
// ── ★ 네 번째 후보: 글린트 — 그리고 이것은 실측표를 흔든다 ─────────────────
// `sky.js:867-923`. 수면 빛반사 띠. 주석이 스스로 **"수평선(fog 원단)까지 닿는
// 빛기둥"** 이라고 적는다 — 정확히 지금 조사 중인 그 자리다. `waterY !== null` 이고
// `weather === 'clear'` 이면 **항상** 생성·표시되는 가산합성 평면이고, `?fogd` 도
// `?hz` 도 참조하지 않는다. 끄는 노브가 없다.
//
// **그리고 이것이 근거를 흔든다.** `decide/horizon.ts` 의 실측 조건은 **"밤 맑음"**
// 이었다 — 글린트가 켜지는 조건과 정확히 겹친다. 시선(`yaw π`)이 광원 방위와 겹치면
// 그 띠가 표본 열(화면 가운데 10%)에 들어와 밝기를 보탰을 것이고, 안 겹치면 안
// 보탰을 것이다. **어느 쪽인지 검증된 적이 없다.** 즉 우리가 근거로 삼아온 표
// (하늘 98 / 띠 143 / 물 85)에 글린트가 섞여 있었을 가능성을 배제할 수 없다.
//
// ── ⚠ 이 목록이 완결됐다고 믿지 마라 ───────────────────────────────────────
// 검수관 재확인마다 후보가 **하나씩** 나왔다 — 1차 `HorizonBand`, 2차 ⑨ 돔 하반부
// 채움, 3차 글린트. 매번 "이제 다 봤다" 고 생각한 다음에 나왔다. 부수 후보로
// `ocean.ts` 윤슬(광원 방위 반짝임, 수평선 한 줄이 아니라 표면 전반이라 형태가
// 다르다)과 블룸(WebGPU 전용 — 헤드리스 실측엔 무관하고 실기기에서는 배제 못 함)도
// 훑었으나 확실성이 낮아 이름만 남긴다.
//
// **끄는 노브가 없는 후보가 셋이다**(⑨ 채움 · 글린트 · 윤슬). 화면에서 가르려면
// 그것들도 열어야 한다 — 이 커밋은 거기까지 안 갔다.
//
//   ?fogd=1    지금(기본). near 51.2m · far 76.8m
//   ?fogd=3~10 멀리 밀기. 띠는 물러나고 세계 끝이 드러나기 시작한다
//
// **`scene.fog` 만 끈다.** `fogBand` 의 다른 소비자(NPC 은닉 임계·바다 타일 범위·
// 수평선 밴드 반경)는 **거리 판정**이라 그대로 둔다 — 시각 효과를 끄자고 판정
// 논리까지 0 으로 만들면 NPC 가 사라지거나 바다가 없어져 **무엇을 보는지 자체가
// 달라진다.** 실험은 한 축만 움직여야 한다.
//
// 안개 거리는 여전히 `decide/fog.ts` 가 SSOT 다. 여기서 곱하는 것은 **비교용
// 배수**이고 기본값 1 이 그 SSOT 를 그대로 쓴다 — 배수를 여기 상수로 박지 않는다.
