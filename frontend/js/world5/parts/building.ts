...
export const building: PartSpec = {
   ...
   *
   * ── 왜 값이 이것인가 ──────────────────────────────────────────────────────
   * 세 개의 부등식이 이 두 숫자를 가둔다. 하나라도 어기면 화면이 무너지거나 게이트가
   * 빨간불이 된다 — 셋 다 `tests/world5-parts-assets.test.ts` 와
   * `tests/world5-neon-glow.test.ts` 가 지킨다.
   *
   * ① **`night` > `BLOOM_THRESHOLD × 1.2` = 0.9.** 밑으로 내려가면 기존 블룸 여유
   *    검사가 떨어진다. world3 에서 문턱 0.85 에 휘도 0.805 라 **블룸이 정상 동작하면서
   *    걸리는 픽셀이 0** 인 사고가 있었다 — 코드도 설정도 멀쩡한데 결과만 없는 형태다.
   * ② **`night` < `tower.night`(1).** 마천루보다 밝으면 스카이라인의 주인공이 뒤집힌다.
   *    0.95 는 ①의 하한(0.9)과 ②의 상한(1) 사이에서 위쪽에 붙인 값이다 — 여유 5.6%.
   *    **아래로 더 내릴 자리가 없다**는 것을 알고 쓴다. 더 어둡게 하고 싶으면 세기가
   *    아니라 **마스크 알파**를 내려야 한다(그쪽이 진짜 룩 축이다. 아래 `A_*` 상수).
   * ③ **`day` < `tower.day`(0.25).** 일반 건물 창은 광고가 아니다.
   *
   * ── 왜 `day` 가 0 이 아닌가 ────────────────────────────────────────────────
   ...
   * 세 부등식과 마스크 텍셀 사이의 **상대비**뿐이다.
   */
  neon: { day: 0.10, night: 0.95 },
 ...
 */
const SIDE_ACCENT: readonly { cornice: number; sign: number }[] = [
  { cornice: V.neonCyan, sign: V.neonMagenta },
  { cornice: V.neonMagenta, sign: V.neonLime },
  { cornice: V.neonAmber, sign: V.neonCyan },
  { cornice: V.neonLime, sign: V.neonAmber },
];
