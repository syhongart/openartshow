...
export function findKnobBar(doc: Document): KnobBarParts | null {
  const panel = doc.getElementById('w5-sliders');
  const toggle = doc.getElementById('w5-sliders-toggle');
  const body = doc.getElementById('w5-sliders-body');
...
export function attachKnobBar(parts: KnobBarParts, specs: readonly KnobSpec[]): KnobBar {
  ...
  for (const spec of specs) {
    ...
    val.className = 'w5-slide-val';

    const reset = doc.createElement('button');
    reset.type = 'button';
    reset.className = 'w5-slide-reset';
