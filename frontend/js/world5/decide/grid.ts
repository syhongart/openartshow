 ...
 * far 반경(76.8m) 안의 **정면 시야에 도시 파셀이 0개**다(광장 4칸 + 공원 1칸이 전부).
 * 첫 화면에 건물이 한 채도 안 나오는 것은 우연이 아니라 이 크기의 결과다.
 *
 * ⚠️ 그런데도 줄이지 않는다 — 1칸(32m)이면 분수대와 시계탑이 서로를 가리고, 중앙
 * 광장은 유일한 스폰 지점이며 시계탑(14m)이 서는 곳이다. **처방은 광장을 줄이는 것이
 * 아니라 채우는 것**이다(감독 지시 *"타임스퀘어도 하고"* — 태스크 #14). 광장이 광고
 * 스크린으로 둘러싸이면 같은 크기가 빈 곳이 아니라 목적지가 된다.
 */
