...
export async function startWorld5(canvas: HTMLCanvasElement): Promise<WorldHandle | null> {
  ...
  // 물속 틴트 판. 마크업은 `world5.html` 에 있고 색도 거기 있다 — 여기서는 세기만
  // 실어 나른다. 요소가 없어도(구버전 HTML 캐시) 그냥 안 보일 뿐 터지지 않는다.
  const underwaterEl = document.getElementById('w5-underwater');
  /**
  ...
  const hudRoot = document.getElementById('w5-hud');
  const hud: PerfHud | null = (hudRoot
    && document.getElementById('w5-hud-body')
    && document.getElementById('w5-hud-copy')
    && document.getElementById('w5-hud-toggle'))
    ? attachHud({
      root: hudRoot,
      body: document.getElementById('w5-hud-body')!,
      copy: document.getElementById('w5-hud-copy')!,
      toggle: document.getElementById('w5-hud-toggle')!,
    }, {
  ...
  const ok = await runBoot({
    ...
    onError: (stage, err) => {
      ...
      console.error('[world5] 부팅 실패', stage, err);
    },
    steps: {
      renderer: async () => {
      ...
      pools: () => {
        ...
        features = mountFeatures(
          ...
          (name, err) => console.error(`[world5] 기능 조립 실패: ${name}`, err),
        );
      },
  ...
  const stickBase = document.getElementById('w5-stick');
  const stickKnob = document.getElementById('w5-stick-knob');
  ...
  (window as unknown as Record<string, unknown>).__world5 = {
    /** 부팅 단계별 경과(ms) */
    timeline,
  ...
  return {
    ...
    dispose() {
      ...
      // 조명 타깃도 씬에서 뺀다. 지금은 `startWorld5` 가 페이지당 한 번뿐이라 **도달하지
      ...
      for (const m of features) {
        try { m.instance.dispose?.(); } catch (err) { console.error(`[world5] ${m.name} 정리 실패`, err); }
      }
