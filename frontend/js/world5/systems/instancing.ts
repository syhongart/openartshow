...
export class InstancePools {
  ...
  constructor(parent: THREE.Object3D) {
    this.group.name = 'world5:instances';
  ...
  create(spec: PoolSpec): void {
    if (this.sealedAt !== null) {
      throw new Error(`[world5] 풀 생성은 부팅 중에만 허용된다(개수 불변식). key=${spec.key}`);
    }
    if (this.pools.has(spec.key)) throw new Error(`[world5] 중복 풀 키: ${spec.key}`);
