...
// 아니다. 규율이 *"못 잰 것은 통과가 아니다"* 라고 적을 때 실제로 금지하는 것은
// **재지 않은 것에 판정을 붙이는 일**이고, 「통과」와 「미달」은 **둘 다 판정이다.**
// 그래서 안 잰 구간에 fail 을 붙이는 것도 같은 결함이다.
// ⚠ 첫 판본은 이것을 규율의 **「대우」**라고 불렀고 그것은 틀렸다(검수관 권고, 형식논리).
//   *"¬측정 → ¬통과"* 의 대우는 *"통과 → 측정"* 이지 *"¬측정 → ¬미달"* 이 아니다.
//   결론은 위처럼 독립적으로 서지만, **논리 용어를 틀리게 쓰면 다음 사람이 그것을 형식
//   추론으로 믿고 다른 자리에 옮긴다** — 이 저장소가 거짓 진술 하나로 7일을 잃은 형태다.
