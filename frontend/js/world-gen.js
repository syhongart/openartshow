...
export function genStreet(px, pz, seed, grid, cell = { x: 24, z: 24 }, parcel) {
  ...
  // [동선 확보] 감독 실기기 판정 — 도로에서 0.95m는 발치 tree pit(반경 1.3, world.js PIT_RADIUS)이
  //  도로 안쪽 경계를 0.35m 침범해 걷는 선과 겹쳤다. 2.3m로 물리면 pit 바깥 가장자리가 8.5로 도로
  //  경계(9.5)까지 1.0m 남아 통행 폭 2.5m가 온전히 빈다. 건물과 가까워지는 만큼 safe()가 자동
  //  배제하므로 겹침은 생기지 않고, 배제된 그루만큼 그루 수가 줄어 부하도 함께 준다.
  ...
  //    남동 코너에서 건물과 겹치지 않는다. ──
  //  [동선 확보] 감독 실기기 판정으로 좌표를 cx/2-1.25(=10.75)에서 roadE-0.3(=9.2)로 물렸다. 도로는
  //  각 파셀이 자기 남/동 가장자리에만 그리는 폭 2.5m 스트립이라(world.js beginParcel의 rgS/rgE)
