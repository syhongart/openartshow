...
if (canvas instanceof HTMLCanvasElement) {
  startGlbWorld(canvas, {
    ...
    // ── 기본 백엔드 = **WebGL** (감독 판정 2026-09-17) ───────────────────────
    // 감독 원문: ***"마지막 링크가 제일 좋아"*** — 그 링크가 `?webgl=1` 이었다.
    //
    // **무엇을 고치는가**: 월드11 에서 **WebGPU 일 때만** `asphalt` 도로면과 `concrete`
    // 건물 외벽이 안 그려지고 그 자리에 배경(바다 평면)이 비쳤다(감독 신고 *"바닥에
    // 물가로 보였어"*). WebGL 경로에서는 같은 자산이 정상으로 그려진다.
    //
    // **성능은 문제가 아니었다** — 감독 실기기 리포트(`?webgl=1`, iPhone iOS 27):
    // 평균 **60.0fps · 히칭 0회** · 삼각형 평균 904,468 · pipeline/geometry/texture
    // **전부 상수** · 거리 컬링 셀 2409/2423 · 반경 146m. 즉 WebGL 로 내려도 프레임을
    // 잃지 않는다. 이 실측이 없었으면 이 판정은 «보이지만 느린» 교환이었을 것이다.
    //
    // ── 🔴 **이것은 임시 우회이지 원인 해결이 아니다** ──────────────────────
    // 닫힌 축(다시 조사하지 않는다): 거리 컬링 · 적응형 강등 · 우리 강 위치·높이 ·
    // 게임풍 수면(`?wstyle=0` 으로도 여전히 물이었다 — 감독 실기기 스크린샷).
    // 레퍼런스 조사: 탄젠트 부재는 원인 아님(r171 `NormalMapNode.js` 가
    // `perturbNormal2Arb` 로 자동 폴백) · clearcoat 무관(해당 재질에 확장이 없다) ·
    // 가장 유력한 것은 **「WebGPU + glTF + InstancedMesh」 결함 클래스**(three.js
    // #29683 · #30599 · #32099 — 전부 「WebGPU 에서만」). 다만 **재질별로 선택적으로
    // 사라지는** 우리 증상과 1:1 일치하는 보고는 **못 찾았다(미확인)**.
    //
    // **되돌리는 조건**: 원인이 잡히면 이 줄을 지운다 — 지우면 자동 판정으로 돌아간다
    // (`decide/backend-knob.ts` 의 우선순위 ③). 되돌림이 한 줄인 것이 이 형태를 고른 이유다.
    //
    // ── ⚠ **WebGL 로 내려서 잃는 것** (백엔드 분기 전수 grep, 2026-09-17) ────
    // 감독이 «우리 강이 더 멋있지나» 라고 했으므로 이 목록은 감독이 알아야 한다.
    //
    //   **노브 없이 기본으로 잃는 것 셋**
    //   ① **블룸**(`features/postfx.ts:214`) — `?bloom=0` 이 아니면 기본 켜짐인데
    //      WebGL 백엔드에서는 켜지 않는다(켜면 warmup 에서 부팅이 깨진다 — 그 파일의
    //      실측 스택). 가로등·간판의 번짐이 사라진다.
    //   ② **게임풍 수면**(`decide/water-style.ts:82` → `features/water-style.ts:205`) —
    //      `?styl` 기본값이 **1**(`decide/stylized.ts`)이라 기본 켜짐이다. 프레넬·화면
    //      깊이가 TSL 노드라 WebGL 에는 렌더 경로가 없다. 기존 수면이 그대로 보인다.
    //   ③ **잔디 TSL 바람 + 잎의 정적 굽힘**(`decide/grass.ts:66` →
    //      `features/grass.ts:268`) — 잔디 자체는 서지만 바람이 멈추고, 굽힘도
    //      `positionNode` 항이라 **잎이 완전히 곧게 선다**(그 파일의 `STATIC_LEAN` 절).
    //
    //   **노브를 켜야 드러나는 손실 둘**
    //   ④ **TSL 수면 `?water=tsl`**(`decide/water.ts:502` → `features/ocean.ts:1032`) —
    //      요청해도 기존 물로 폴백한다(콘솔 경고 1줄).
    //   ⑤ **지면 안개 `?fogh`·`?fogk`·`?fogs`**(`features/ground-fog.ts:41`) — 기본
    //      세기가 0 이라 평소엔 꺼져 있지만, 켜도 WebGL 에서는 안 붙는다.
    //
    //   **그 밖 — 화면이 아니라 계측이 갈리는 것**(`adapters/renderer.ts`):
    //   파이프라인 수 출처가 `renderer._pipelines.caches` → `info.programs` 로,
    //   드로우콜 필드가 `render.drawCalls` → `render.calls` 로 바뀐다. 두 백엔드의
    //   수치를 **가로로 비교하지 마라** — 어댑터가 흡수하는 것은 이름뿐이고 의미가 같지 않다.
    //
    // ⚠⚠ **world7·world8 은 이 값을 안 넘긴다** — 저쪽은 자동 판정 그대로다.
    // `?webgl=0` 이면 감독이 이 페이지에서도 WebGPU 판을 되볼 수 있다(URL 이 이긴다).
    defaultWebGL: true,
    // ── `?far=` 의 기준 — **이 자산의 크기**다 ───────────────────────────────
