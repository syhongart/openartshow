...
if (canvas instanceof HTMLCanvasElement) {
  startGlbWorld(canvas, {
    ...
    viewSpan: spanOf,
    // ── 치비가 다닐 격자를 **세계에서 굽는다** (감독 요구 2026-09-19) ───────
    // 감독 원문: ***"우리 치비들이 건물. 길. 벽.을 인식해서 다닐수 있개 하는 것은 무겁나"*** ·
    // ***"블렌더 파일이 계속 바뀔수있자나. 맨하탄이 홍콩이 될수도 있고. 매번 치비를
    // 수정하지 않아도 자동으로 다니게."***
    //
    // 치비 걷기는 **파셀 도로 격자**를 따라간다(`world-glb/decide/npc-walk.ts`). 맨해튼에는
    // 그 격자가 없어 사람들이 건물을 가로질러 걸었다. 켜면 세계의 기하에서 격자를 굽는다 —
    // **재질 이름으로 도로를 고르지 않는다**(그러면 홍콩 자산에서 틀린다).
    //
    // 비용: 이 자산에서 **136ms**(칸 0.34m, node 실측 — 같은 자산의 GLB 파싱이 1,544ms 다).
    // 해상도별 표는 `world-glb/systems/glb-walkmap.ts` 헤더 **한 곳**이다.
    //
    // ⚠ **world7·world8 은 이 값을 안 넘긴다** — 저쪽은 격자를 안 굽고 걷기가 종전 파셀
    // 경로 그대로다. world7 은 사람이 고른 임의 파일이라 「지상 1층」이 성립하지 않는
    // 자산이 올 수 있고, 그 판정은 이 회차의 경계 밖이다.
    //
    // ⚠⚠ **칸 크기는 여기서 안 고른다.** `?walkcell=` 로 감독이 후보를 비교한다 —
    // 유도식과 후보표는 `world-glb/decide/walkable.ts` 한 곳이다.
    walkmap: true,
