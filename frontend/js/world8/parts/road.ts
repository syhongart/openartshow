 ...
 * 도로판 **윗면**의 월드 높이(m).
 *
 * ── 왜 상수로 꺼냈나 (2026-08-11) ──────────────────────────────────────────
 * 이 값은 원래 `place()` 안 지역 변수(`const y = 0.14`)였고, **광장 랜드마크가 이 위에
 * 선다**는 계약이 아래 중심 조각 주석에만 산문으로 있었다. 그래서 `fountain`·`clocktower`
 * 가 `y: 0` 을 신고한 채 12cm 파묻혀 있었고 아무 검사도 안 걸렸다 — 계약이 코드가 아니라
 * 문장이었기 때문이다. 이제 랜드마크가 이 상수를 **직접 읽는다.**
 *
 * ⚠ 이 값을 바꾸면 광장 랜드마크의 높이와 그 그림자가 **함께** 따라온다. 그것이 상수로
 * 꺼낸 이유이고, 두 곳에 0.14 를 적으면 그 연동이 끊긴다.
 */
export const ROAD_SURFACE_Y = 0.14;
...
export const road: PartSpec = {
  ...
  place: ({ px, pz, o }) => {
    ...
    // 중심 — 막다른 길이어도 길머리는 있어야 한다. **광장에서도 깐다.**
    //
    // ══════════════════════════════════════════════════════════════════════
    // ⚠️ 여기 `if (!isPlaza(px, pz))` 가 있었고, 그것이 감독이 본 회색의 원인이다
    // ══════════════════════════════════════════════════════════════════════
    // 감독 2026-08-09: *"월드2에서 길 겹쳐져있는 곳 저렇게 나오는데."* →
    // ***"길이 겹쳐진 곳이 회색으로 되어있잖아"***
    //
    // 옛 주석은 비우는 이유를 *"그 자리에 분수대나 시계탑이 선다"* 로 적었다.
    // **광장 9칸 중 2칸에서만 참이었다** — `plazaLandmark` 가 나머지 6칸에서 `null`
    // 이다(`plaza.ts`). 그래서 팔 넷이 만나는 자리에 `ROAD_SEG`×`ROAD_SEG`(5×5m)
    // 빈칸이 남고, 광장에는 잔디도 안 깔리므로(`garden.ts` 의 `isPlaza` 가드)
    // 그 구멍으로 **지면 판이 그대로 드러났다.**
    //
    // ⚠️ **그리고 9칸이 전부가 아니었다.** 이 가드가 부른 것은 `isCentralPlaza` 가
    // 아니라 `isPlaza` 이고, 그것은 중앙 9칸 + **위성 광장**까지 참이다. 격자 전체
    // (−15..14) 실측: **38칸**(중앙 9 · 위성 29), 전부 길이 있는 칸.
    // 랜드마크는 중앙 2칸에만 서므로 **위성 29칸에는 덮을 것이 애초에 없었다** —
    // 옛 주석은 거기서 참이었던 적이 한 번도 없다.
    //
    // 그래서 이 결함은 감독이 본 한 자리가 아니라 **맵 전역에 38개** 있었다.
    // (골든은 −12..12 만 도므로 그중 30칸 × 3 tier = 90 만 셌다. 골든의 창이 세상보다
    // 좁다는 것을 여기 적어 둔다 — 골든 증분을 결함 전량으로 읽으면 8칸을 놓친다.)
    //
    // 밤에 특히 도드라진 것도 계산과 맞는다 — `NIGHT_GROUND_LIFT`(2.4)가 지면
    // 알베도를 들어 올려 선형휘도가 지면 0.324 대 아스팔트 0.063, **5.2배**다.
    //
    // ── 랜드마크 2칸도 못 덮는다 (팀장 조건 2, 2026-08-09) ──────────────────
    // 처음에는 가드를 `plazaLandmark(px,pz) !== null` 로 **좁히려** 했다. 팀장이
    // 그것을 뒤집었다 — 분수대 최대 반경이 2.4m 라 5×5 를 못 덮어 **네 귀퉁이가
    // 남고**, 감독 스크린샷이 하필 분수대 정면이다. 6칸을 메워도 **감독이 본 그
    // 화면은 그대로**였을 것이다.
    //
    // *"수치(6칸)가 맞아도 화면(분수 칸)이 안 고쳐지면 미완이다."*
    //
    // ── 그래서 가드를 없앤다. 겹침은 안 생긴다 ─────────────────────────────
    // 중심 조각은 [−2.5, 2.5]² 이고 랜드마크가 그 자리에 선다. 아래 팔 주석이 기록한
    // z-fighting 은 **같은 높이의 두 면**이 다투는 것이었고, 여기는 높이가 다르므로 그
    // 형태가 아니다.
    //
    // ⚠ **정정(2026-08-11, 검수관 P3)**: 직전 문장은 *"랜드마크는 그 **위에** 선다
    // (분수·시계탑은 지면 피벗에서 올라간다)"* 였고 **메시에 대해서는 거짓이다.**
    // 분수·시계탑의 `place()` 는 `y: 0` 이고(밑동 불변식) 그대로다 — 즉 **메시는 여전히
    // 이 판(0.14) 속에 밑동이 파묻혀 있다.**
    //
    // 이번에 도로판 위로 올라간 것은 **그림자뿐**이다(`standsOn` 신고 → 데칼 높이 보정).
    // 그 둘을 뭉뚱그리면 다음 사람이 "랜드마크가 도로 위로 올라갔다" 로 읽고 메시 클리핑을
    // 안 본다. **메시 파묻힘은 아직 미해결이다** — 고치려면 밑동 불변식과 배치 골든을
    // 함께 다뤄야 하고, 그것은 별건이다.
    //
    // 잃는 것: 광장 한복판이 아스팔트가 된다. 옛 주석이 노린 *"길이 광장 둘레를
    // 도는 로터리처럼 읽힌다"* 는 인상은 사라진다. **그 인상의 대가가 회색
    // 구멍이었고, 감독이 구멍 쪽을 지적했다.**
    //
    // ── ⚠️ 이것으로 "회색을 다 잡았다" 가 아니다 (검수관 실측 2026-08-09) ────
    // 메운 것은 파셀당 **25㎡**, 광장 칸 면적(1024㎡)의 **2.4%p** 다. 4방향(사거리)
    // 칸의 **71.2%**, 3방향 칸의 **77.8%** 는 **여전히 `ground` 노출**이며 그것은
    // 이번 수정의 범위가 아니다 — `garden.ts:78` 이 *"광장은 포장"* 이라며 잔디를
    // 걷는데 `ground.ts` 에 광장 예외가 없고 포장 파츠가 아예 없다(태스크 #17,
    // 이번 결함과 **같은 형태의 거짓 주석**이다).
    //
    // 이 수치를 여기 적는 이유: **뮤테이션 검출력도 골든 증분도 "메운 자리가
    // 정확한가" 만 보증하고 "그것이 전체 문제의 몇 %인가" 는 어떤 게이트도 안 잰다.**
    // 90개가 늘고 뮤테이션 2종이 깨졌다는 사실만 읽으면 다 잡은 것으로 오해한다 —
    // 실제로 내가 그렇게 적었고, 검수관이 면적을 따로 계산해서야 드러났다.
    seg(0, 0, 0, ROAD_SEG);
