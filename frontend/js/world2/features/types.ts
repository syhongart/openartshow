...
export interface FeatureEnv {
   ...
   *
   * **언제 등록해도 된다.** 집행(`surface-paint.ts`)이 부팅 스냅샷을 **지연으로도** 뜨므로,
   * `FEATURES` 배열 순서가 바뀌거나 재질이 비동기로 늦게 생겨도 그 표면이 칠할 수 없게
   * 되지 않는다. 다음 폴링(`update`)에서 반영된다.
   *
   * ⚠⚠ **이 문단은 한때 거짓이었다**(검수관 블로커 B1, 2026-08-15). 그때는 스냅샷을 뜨는
   * 곳이 `create()` 부팅 루프 하나뿐이라, 그 시점에 재질이 없던 표면은 «그 프레임» 이 아니라
   * **세션 전체에서 영구히** 칠할 수도 되돌릴 수도 없었다 — 동작하던 이유는 배열 순서가
   * 우연히 맞아서였다. 문장을 약하게 고치는 대신 **집행을 고쳐 문장을 참으로 만들었고**,
   * 그것을 검사로 못 박았다(GS-3 전례와 같은 처방).
   *
   * 지키는 축 둘:
   *   · `tests/world2-ocean.test.ts` 의 「표면 재질 레지스트리」 절 — 부팅 등록·재질 동일성.
   *     실제 `oceanFeature.create` 를 태우고, 등록 호출을 지우면 4축이 깨진다(실측).
   *   · `tests/world2-surface-paint.test.ts` 의 「늦은 등록」 절 — **부팅에 없던 표면도
   *     나중에 등록되면 칠해지고 되돌려진다.** 위 문단이 참인지를 재는 자리다.
   */
