  ...
  create(env: FeatureEnv): FeatureInstance {
    ...
    const sky = new SkySystem(
      ...
      () => ({ x: env.player.position.x, z: env.player.position.z }),
      {
        ...
        // ── ⑨ 규칙 집행: `scene.fog` 를 하늘 지평색에 맞춘다 (`?skyfog=`) ──
        //
        // 감독 실기기 2026-08-07: *"멀리서 건물 윤곽"* 이 팍 하고 나타난다.
        // LOD 페이드(#213)를 넣었으나 *"별 차이가 없네"* — 시작색이 배경과 딴판이라
        // 페이드가 검정에서 출발했기 때문이다. 왜 그런지는 `systems/sky.ts` 의
        // `updateSkyTint` 머리말 **한 곳**이 소유한다(여기에 팔레트 값을 적지 않는다).
        //
        //   ?skyfog=1  안개 = 하늘 지평색 (기본)
        //   ?skyfog=0  안개 고정 — 이 diff 이전 동작. **대조군이다**
        //
        // `fogTint`(위 `?fogsky=`)와 헷갈리지 말 것: 저것은 팔레트 안개색을 하늘 쪽으로
        // 얼마나 당길지의 **세기**이고, 이것은 그렇게 나온 색을 `scene.fog` 에 **실제로
        // 쓰는가**이다. 지금까지 세기만 있고 쓰는 쪽이 없었다.
        matchSceneFog: readNum('skyfog', 1, 0, 1) >= 1,
