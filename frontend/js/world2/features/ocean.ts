 ...
 * 그때까지 이 씬에는 `envMap` 도 `scene.environment` 도 없었고, 그래서 물의 «반짝임» 이
 * 광원과 무관한 `emissiveMap` 스티커였다(감독: *"왜 밑 반사는 움직이지 않을까"*).
 * 무엇을 굽는지·왜 PMREM 이 아닌지는 `decide/water-env.ts` 한 곳이다.
 *
 * ⚠ 기본 `1`(그대로 반사)에서 시작한다. 이 값은 **거울면 쪽 밝기를 통째로 쥔다** —
 * 낮 `roughness` 가 0.18 이라 수면이 거울에 가깝고, 세기를 올리면 점이 아니라 **면**이
 * 밝아져 감독이 이미 반려한 *"흰 덩어리"* 로 되돌아간다. 되돌리기가 비싼 쪽을 기본값으로
 * 삼지 않는다.
 */
 ...
 *
 * ⚠ 가장 빠른 층을 기준으로 잡는 것이 **보수적인 쪽**이다. 층1 을 강에 맞추면 배율이 24.5
 * 가 되고 층2 가 2.9 m/s(걷는 속도의 절반)로 흘러 «급류» 가 된다. 더 원하면 `?wripv=24`
 * 가 그 자리다 — 되돌리기가 비싼 쪽을 기본값으로 삼지 않는다(`WAVE_AMP_DEFAULT` 와 같다).
 */
