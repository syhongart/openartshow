...
export const oceanFeature: Feature = {
  ...
  create(env: FeatureEnv): FeatureInstance | null {
    ...
    // ⚠ **층2를 숨기는 소비자를 위해 원래 높이를 남긴다** (2026-08-20).
    // `features/water-style.ts` 는 `ocean-wave2`(층2)를 숨기고 `ocean`(층1)만 대역한다.
    // 그 경로에서는 교차할 층2가 없으므로 내릴 이유도 없는데, 내린 채로 두면 **스타일
    // 물에서만 바다 수면이 `?wamp` 만큼 낮아진다.** 값을 저쪽에 다시 적으면 미러링이라
    // (`SEA_Y` 가 두 곳이 된다) 메시가 자기 기준 높이를 들고 다닌다.
    sea.userData.baseY = SEA_Y;
