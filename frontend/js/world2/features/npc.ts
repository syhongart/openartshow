...
//
// **밴드 형태(최소:최대 = 1:2)는 보존한다.** 형태까지 바꾸면 그것 자체가 임의
// 판단이 된다 — 지금 고치는 것은 "얼마나 먼가" 이지 "어떤 모양인가" 가 아니다.
// 상한만 당기고 비율로 하한을 유도하면, 안개가 바뀔 때 밴드가 통째로 따라온다.
//
// **여기에 셀 배수를 숫자로 적지 않는다**(팀장 조건 1). 적었다가 값이 갈라진 것이
// 바로 위 `RECYCLE_CELLS` 주석이 기록한 사고다.

/** 스폰 최대(셀). **안개 시작과 같다** — 그 밖에서 태어나면 대비가 깎인 채 시작한다 */
const SPAWN_REACH = FOG_NEAR_CELLS;

/**
 * 스폰 최소(셀). 눈앞에 튀어나오지 않게 띄우되, 기존 밴드 비율(1:2)을 보존한다.
 * 상한이 움직이면 하한도 같은 비율로 따라온다.
 */
const SPAWN_RING = SPAWN_REACH / 2;
