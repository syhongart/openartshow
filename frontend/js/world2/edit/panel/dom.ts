...
export interface PanelHandlers {
   ...
   * (편집이 그것을 떼지 않는다 — 백로그 `G-EDIT14`). 패널 버튼은 그 충돌 밖에 있다.
   * ⚠ 이 줄은 오래 **번호 없이** *"백로그"* 라고만 적고 있었고 **그 항목은 실재하지
   * 않았다**(2026-08-21 실측: `docs/BACKLOG.md` 에 터치 관련 항목 0건). 가리키는 대상이
   * 없는 참조는 다음 사람에게 «누가 보고 있겠지» 로 읽힌다 — 문장을 고치는 대신 항목을
   * 만들어 참으로 되돌렸다.
   */
