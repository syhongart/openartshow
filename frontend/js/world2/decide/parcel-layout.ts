...
//    바뀐다.
//    ⚠ **이 문장에 예외가 예약됐다 — 아직 열리지 않았다**(팀장 판정 2026-08-12).
//    감독이 마을 건물을 손으로 옮기려면 그 파셀은 «저장된 배치» 를 써야 하고, 그것을 담을
//    자리가 `decide/overlay.ts` 계약 v2 의 `parcels[]`(파셀 동결)로 만들어졌다. **다만 그
//    데이터를 실제로 소비하는 코드는 아직 없으므로 위 문장은 지금 그대로 참이다.**
//    소비자가 붙는 커밋에서 이 문장을 *"단, 동결된 파셀은 예외"* 로 고친다 — 팀장이
//    그것을 **같은 커밋**으로 못 박았다(거짓 진술을 규율 문서에 남긴 것이 이 저장소에서
//    7일을 잃은 형태다). 그때 근거의 SSOT 는 동결 모듈 헤더 한 곳이다.
//    ⚠ **예외는 파셀 단위이고 전역이 아니다.** 동결하지 않은 파셀은 그대로 계산된다.
//    그리고 동결은 **이 함수 «바깥»에서** 적용된다 — 여기에 분기를 넣으면 골든 해시가
//    깨지고, 그때 고칠 것은 해시가 아니라 설계다.
