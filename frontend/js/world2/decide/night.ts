 ...
 * 항등원은 1 이 아니라 **0.196** 이다(유도·실측·왜 근본 수정을 안 하는지는
 * `NIGHT_FOG_SCALE` 주석 한 곳이 소유한다 — 여기에 표를 다시 적지 않는다).
 *
 ...
 *   ────┼──────────────────────┼──────────────────┼────────────────
 *   r     0.2392                 0.0466             5.13×
 *   g     0.2784                 0.0724             3.85×
 ...
 * `linear(x)/x` 이고 가장 빡빡한 것이 r 의 0.0466/0.2392 = **0.196** 이다.
 * 0.2 는 그 바로 위 — r 만 2.7% 남고 g·b 는 여유롭게 팔레트가 이긴다. 즉 0.2 는
 * **원래 1.0 이 하겠다고 적어 둔 일(= 아무 일도 안 하기)을 실제로 하는 값**이다.
 * 감독이 고른 값과 유도값이 같은 자리에 떨어졌다.
 *
