...
/**
 * Rec.709 상대휘도. 채널은 0..1.
 *
 * **선형 변환을 하지 않는다.** 엄밀히는 sRGB 채널에 이 가중치를 곱한 값은 물리 휘도가
 * 아니다. 그런데 이 저장소에서 이 값이 쓰이는 곳은 단 하나 — **블룸 문턱과의 비교**이고,
 * 문턱(`BLOOM_THRESHOLD`)도 같은 축에서 정해졌다. 여기서만 선형으로 바꾸면 두 기준이
 * 섞여 "가로등은 넘는데 달은 못 넘는" 식의 설명 불가능한 상태가 된다.
 *
 * 축을 바꿔야 한다면 문턱과 함께 바꾼다. 한쪽만 옮기는 것이 이 저장소가 반복해 온 사고다.
 */
 ...
 */
export const LAMP_LUMINANCE = relativeLuminance(0xff / 255, 0xc8 / 255, 0x6e / 255);
...
/**
 * 달 발광체의 발광 배수 — **가로등 피크와 같은 휘도가 되도록 유도한다.**
 *
 * ── 왜 상수를 안 적고 유도하는가 (감독 지시 2026-07-30 *"달에 블룸효과 넣어줘"*) ──
 * 필요한 것은 "달이 블룸 문턱을 넘는 것"이고, 넘기만 하면 되는 값은 무한히 많다. 그중
 * 하나를 골라 적으면 그것은 근거 없는 숫자이고, 나중에 문턱이나 가로등을 바꿀 때 **아무도
 * 이 값을 같이 보지 않는다.** 이 저장소가 반복해 온 사고가 정확히 그것이다.
 *
 * 그래서 기준을 하나 정한다: **달은 가로등과 같은 세기로 번진다.** 감독이 *"불름 잘나와"*
 * 라고 확인한 유일한 실측 기준점이 가로등이고, 그것과 같게 두면 문턱을 넘는 것이 저절로
 * 따라온다(가로등이 넘으니까). 가로등을 바꾸면 달도 함께 따라온다.
 *
 * 실제 화면에서 달은 이보다 밝다 — 발광체를 하늘 텍스처의 달 **위에 더하기** 때문이다
 * (가산합성). 그 텍스처 달의 휘도(≈0.96)가 그대로 얹힌다.
 *
 * @param coreLuminance 하늘 텍스처가 그리는 달 원반 코어색의 휘도(`relativeLuminance`)
 */
export function moonGlow(coreLuminance: number): number {
  // 0 으로 나누는 것을 막는다 — 코어색이 검정이면 어떤 배수로도 문턱을 못 넘으므로
  // 발광 자체가 성립하지 않는다. 그때는 0 을 돌려주고, 진단이 그 사실을 드러낸다.
  if (!(coreLuminance > 0)) return 0;
  return (LAMP_LUMINANCE * LAMP_MAX_GLOW) / coreLuminance;
}
