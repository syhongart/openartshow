 ...
 *
 * ── 이 값의 상한은 **3.2초**다 (검수관 권고 P1 에 대한 정정, 2026-08-09) ────
 * 권고는 *"위반이 무한정 커지는 것을 안 막는다"* 였는데 **그렇지 않다** — 같은 리포트의
 * 게이트 명세가 이미 적고 있듯 걷기 검사(`걷기 통과 3.2초 > FADE_SECONDS`)가 상한
 * 노릇을 한다. 3.2 를 넘기는 순간 그 검사가 깨진다. 즉 없는 것은 상한이 아니라 **상한이
 * 있다는 사실의 기록**이었고, 그래서 검사를 하나 더 만드는 대신 이 문단을 넣는다.
 * (2.5 와 3.2 사이로 더 올리는 것은 여전히 안 막힌다 — 거기부터는 감독 육안이 축이다.)
 */
