...
import { WIND_DIR_X, WIND_DIR_Z } from './wind.js';
 ...
 */
// ── ⚠ 밀도 상향 판정 기록 (팀장 판정 2026-08-18, 감독 부재 중) ──────────────
//
// 감독이 **두 번** 같은 것을 지적했다: *"그리고 너무 듬성듬성"* → (링 도입 후에도)
// *"지금 듬성듬성해보여"*. 그래서 28/5/1 → **55/8/1.4** 로 올렸다.
//
// 산술: `(2R)² × d` 로 43,120 + 36,992 + 27,440 = **107,552**. 상한을 110,000 으로 함께
// 올려야 클램프를 안 먹는다(`ringCounts` 가 비율 유지로 눌러 담으므로, 상한이 낮으면
// 밀도를 올려도 실제로는 안 빽빽해진다 — 둘은 언제나 짝이다).
//
// **대가는 정점 셰이더 호출이다**: 64.7만 → 107.6만(잎당 정점 10). 0 스케일 슬롯도 정점
// 셰이더는 돈다. 드로우콜·지오·텍스처는 불변이라 개수 불변식에는 영향이 없다.
//
// ⚠⚠ **프레임타임은 실측하지 못했다.** 이 환경은 WebGPU 를 못 돌리고(헤드리스는 WebGL,
// 바람 셰이더 경로를 아예 안 탄다) 위 수치는 전부 **산술 추정**이다. 감독 실기기가 유일한
// 판정 축이고, 그래서 팀장이 `?gden=` 비교 링크를 함께 드리는 것을 조건으로 걸었다.
// 부팅 동기 배치도 약 12ms → 약 20ms 로 추정된다(프레임 하나를 넘긴다).
//
// **경계 — 다음 밀도 재론은 감독이 실기기에서 «느리다» 또는 «또 듬성듬성하다» 라고 할
// 때만이다.** 그 말이 없으면 이 값을 다시 만지지 않는다. 안 그러면 같은 고리를 또 돈다
// (밤 밝기가 네 바퀴를 돈 전례가 이 저장소에 있다).
//
// ── ⚠ 2차 상향 (감독 지시 *"1.4로 하고"*, 2026-08-18) + **성능 실측이 들어왔다** ────
//
// 55/8/1.4 → **77/11.2/1.96**(= 정확히 1.4배), 상한 110,000 → **155,000**.
//
// **왜 상한을 함께 올려야 했나 — 감독이 본 `?gden=1.4` 는 사실 기본값이었다.**
// `ringCounts` 가 상한 초과 시 비율을 유지한 채 눌러 담는데, 1.4배는 150,573 이 필요하고
// 상한이 110,000 이라 **109,999 로 클램프**됐다. 기본(107,552) 대비 **2.3%** 차이다.
// 즉 감독은 «같은 화면 둘» 을 비교하고 «1.4» 를 고른 셈이고, 그 사실을 내가 링크를 드리며
// 확인하지 않았다. **노브를 열 때는 그 노브가 실제로 무엇을 바꾸는지 먼저 재야 한다** —
// 상한이 있는 노브는 상한 근처에서 조용히 무력해진다.
//
// **성능은 이제 추정이 아니라 실측이다** (감독 실기기 리포트, iPhone / WebGPU / DPR 3.0):
//
//     평균 59.8fps · 히칭(>100ms) **0회** · 20.2초 1,172프레임
//     frame avg 16.7ms │ render avg **2.8ms** (p95 4.0) │ out avg 13.1ms
//     삼각형 평균 110만
//
// **render 가 2.8ms 뿐이고 out(브라우저 합성·표시)이 13.1ms 다.** 즉 60fps 를 잡고 있는
// 것은 GPU 렌더가 아니라 표시 파이프라인이고, 잔디를 1.4배(정점 107.6만 → 150.6만) 늘려도
// render 가 4ms 안쪽일 것으로 본다. **이것은 여전히 외삽이다** — 1.4배 실측은 이 배포
// 이후에 감독이 다시 재야 확정된다.
export const GRASS_RINGS: readonly GrassRing[] = [
  { radius: 14, density: 115.5, scale: 1.00 },
  { radius: 34, density: 16.8, scale: 1.35 },
  { radius: 70, density: 2.94, scale: 1.90 },
];
 ...
 *
 * 155,000 의 근거: 3링 합이 **150,573** 이다(위 `GRASS_RINGS` 의 산술). 여유가 2,448 뿐인
 * 것은 의도다 — 밀도 노브를 올리면 `ringCounts` 가 비율을 유지한 채 눌러 담는다. 그 위(또는 반경 최대 52)에서는 상한에 눌려
 ...
 */
export const MAX_BLADES = 240000;

// ⚠ **기본값은 안 눌리지만 `?gden` 은 여전히 눌린다**(2026-08-19 실측).
//    `gden=1` → 225,859 (여유 14,141) · **`gden=1.4` → 239,999 로 상한에 눌린다** ·
//    `gden=2` → 239,998. 즉 1.06배쯤부터 노브가 무력해진다.
//    감독이 예전에 `?gden=1.4` 를 보고 «1.4 로 하자» 고 판정했는데 그것이 상한에 눌려
//    기본값과 2.3% 차이였던 사고가 바로 이 자리다. **밀도 노브로 비교를 청할 때는 먼저
//    `ringCounts` 합을 재라.** 상한을 더 올리지 않는 이유는 성능이다 — 240,000 포기가
//    이미 삼각형 192만이고, 감독 실기기 실측(render 2.9ms · out 13.0ms)에서 확인된 것은
//    150,573 포기까지다.
 ...
 * 급하게 좁아진다.
 *
 * ⚠⚠⚠ **0.26 → 0.6 (감독 판정 2026-08-19 8회차 *"잎 뾰족하기 전 이 나아"*).**
 * **2차 값으로 돌아간다.** 3차에 감독이 *"끝이 뾰족하지 않고"* 라고 해서 0.6 → 0.14 로
 * 뾰족하게 만든 것이 이 축의 출발이었는데, 여덟 회차를 돌아 감독이 **뭉툭한 쪽**을
 * 골랐다. 2차에 이 값이 받은 판정은 *"잎모양은 이상한데"* 였지만, 그때는 폭이 13cm 라
 * 끝 굵기가 3.90cm 였다 — **판자**였다. 지금 폭이 5.2cm 이므로 같은 비율에서 끝이
 * 1.56cm 로 그때의 40% 다. 「뭉툭한 비율 + 가는 잎」은 아직 아무도 안 본 조합이다.
 * ⚠ **같은 값이 다른 화면을 만든다** — 비율로 적힌 값은 기준(폭)이 바뀌면 다른 것이 된다.
 * 5회차에 0.14 를 그대로 뒀다가 끝이 바늘이 된 것도 같은 형태였다.
 *
 * ⚠⚠ **0.14 → 0.26 (감독 판정 2026-08-19 5회차 *"뭐 이리 가시처럼 서있나"*).**
 * 0.14 는 폭이 13cm 이던 때의 값이다 — 그때 끝 반폭은 0.91cm 였다. 같은 회차에 폭을
 * 5.2cm 로 줄이면서 이 비율을 그대로 뒀더니 끝 반폭이 **0.36cm** 가 됐다. 거의 바늘이다.
 * 0.26 이면 0.68cm 로, 13cm 시절의 절대 굵기(0.91cm)에 가까워진다.
 * **비율로 적힌 값은 기준이 바뀌면 함께 봐야 한다** — 이 회차의 두 실수가 같은 형태다
 * (여기와 `STATIC_LEAN`).
 */
 ...
 *
 * ⚠ **위 두 문단은 2026-08-19 감독 판정으로 이미 뒤집혔다** — 이 배열은 더 이상 잎 색을
 * 직접 정하지 않는다. `decide/blade-shape.ts` 의 `bladeToneHex` 가 이 값과 **감독 원안
 * 팔레트** 사이를 `?gpal` 로 섞고, 기본값은 **감독 원안**이다. 여기 적힌 *"감독 팔레트를
 * 그대로 쓰지 않는다"* 는 판단이 바로 그 커밋에서 반려됐다. 배열을 지우지 않은 것은
 * `?gpal=0` 이 되돌아갈 끝점이기 때문이다.
 *
 * ⚠⚠ **아래 「실제 위험」도 실기기에서 안 났다**(감독 2026-08-19 밤 화면 *"생각보다
 * 좋다"* — 스크린샷에서 잔디가 도로보다 밝게 읽힌다). 예측 자체는 코드상 맞았지만
 * (`GROUND_KEYS` 가 블레이드를 안 밝히는 것은 지금도 사실이다) **화면에서 문제가 되지
 * 않았다.** 채도 상향이 그 몫을 대신했다. 아래 문단은 기록으로 남긴다 — 밤 밝기를
 * 다시 만질 사람이 이 경로를 알아야 한다.
 *
 * ⚠⚠⚠ **원래 이 자리에 있던 진술:** `systems/ground-lift.ts` 는
...
/**
 * 잎 평면을 바람 방향으로 얼마나 모으는가. `0` = 완전 랜덤(4회차 이전) · `1` = 전부 나란.
 *
 * ── 감독 판정 2026-08-19 (4회차): *"어떤것은 0도 어떤것은 180도로 있으니 가위처럼
 * 움직이는것 같아"* ───────────────────────────────────────────────────────────
 *
 * **감독이 원인을 직접 지목했고, 그 자리는 바람이 아니라 여기였다.** 나는 세 회차를
 * 바람만 보고 돌았다 — 진동/편향 비율, 정적 굽힘, 셰이더 이동. 직전 회차에 «방향은 이제
 * 전부 같다» 고 적은 것은 실제로 참이다(three r171 `NodeMaterial.setupPosition` 실측:
 * `instancedMesh().append()` 가 `positionLocal.assign(positionNode)` **앞**이라 바람
 * 오프셋은 인스턴스 회전을 타지 않는다). **그런데 잎이 어느 쪽을 보고 서 있는지는 한
 * 번도 안 봤다.** `bladeBase.rot` 이 `hash × 2π` 로 0~360° 완전 균등이었다.
 *
 * 잎은 평면 한 장이라 **180° 주기**다 — 그래서 `align=0` 의 실질 범위가 ±90° 이고, 그것이
 * 곧 «전부 제각각» 이다. 감독이 «0도와 180도» 라고 말한 그 상태다.
 *
 * **기준각을 상수로 박지 않는다**(`alignedRot` 이 `WIND_DIR_*` 에서 유도한다). 바람
 * 방향을 바꾸면 잎도 따라 돌아야 하는데, 두 곳에 적으면 한쪽만 고쳐도 아무도 모른다.
 *
 * ⚠ **1.0 은 위험하다** — 모든 잎 평면이 나란해져 특정 방향에서 보면 잔디밭이 얇은 선들로
 * 사라진다. 0.55 는 ±40° 이고, 여기서 멈춘 이유는 «방향이 읽히는 최소» 와 «평면이 사라지지
 * 않는 최대» 사이라고 **판단**했기 때문이다 — 실측이 아니라 판단이므로 노브로 연다.
 */
export const GRASS_ALIGN = 0.55;

/**
 * 정렬된 잎 Y 회전. `h` 는 `[0,1)` 해시.
 *
 * 기준각은 잎 평면(로컬 XY, 법선 +Z)이 바람과 **나란**해지는 각이다 — 법선을 바람에
 * 수직으로 두면 잎이 자기 평면 안에서 눕고, 그때 굽은 곡선이 실루엣에 그대로 보인다.
 * 법선을 바람 쪽으로 두면(=평면이 바람에 수직) 잎이 통째로 옆으로 미끄러지는 것처럼 보인다.
 *
 * **실측**: 바람 (0.86, 0.51) → 방향 59.3°(Z 기준) → 기준각 **149.3°**.
 * 범위는 `±90°×(1−align)` — `align=0` 이 ±90°(= 전체), `0.55` 가 ±40°, `1` 이 0°.
 */
export function alignedRot(h: number, align: number): number {
  const a = Math.max(0, Math.min(1, align));
  const base = Math.atan2(WIND_DIR_X, WIND_DIR_Z) + Math.PI / 2;
  return base + (h - 0.5) * Math.PI * (1 - a);
}
 ...
 */
export function bladeBase(i: number, radius: number, align: number = GRASS_ALIGN): {
  /** 타일 로컬 좌표 [0, 2R) */
  bx: number;
  bz: number;
  /** Y 회전(rad) */
  ...
  return {
    ...
    // ⚠ `× 2π` 였다 — 0~360° 완전 균등. 그것이 감독이 본 «0도와 180도» 다(4회차).
    //    정렬 근거는 `GRASS_ALIGN` 한 곳이다.
    rot: alignedRot(bladeHash(i, 0x9b05688c), align),
