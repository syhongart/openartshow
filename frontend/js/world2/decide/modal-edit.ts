...
// 회전은 `ry` 하나뿐이고 크기는 균등(비율 유지)이라 **고정할 축이 애초에 없다.** 그래서
// `ModalAxis`·`ModalDelta.dx/dy/dz`·`MOVE_PER_PX` 가 통째로 죽었고, 죽은 코드를 남기지
// 않았다(이 저장소는 「쓸 소비자가 없는 문을 미리 내지 않는다」를 여러 번 적었다).
//
