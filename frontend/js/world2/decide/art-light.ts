 ...
 *
 * ⚠ **풀 크기는 이 값으로 줄이지 않는다.** 줄이면 세션 중 개수가 변해 조건 1(개수 불변
 * 절대)이 깨진다 — 풀은 언제나 최대치로 잡고, soft 는 **몇 개를 켜는가**만 바꾼다.
 *
 * ⚠⚠ **이 문장은 한동안 거짓이었다**(검수관 블로커 B1, 2026-08-17). 집행부가
 * `artLightPoolSize(perParcel)` 을 부르고 있어서 soft 를 넘기면 풀이 28 → **7** 로 줄었다.
 * 같은 회차의 `tests/world2-artwork-scene.test.ts` 는 *"풀은 `perParcel` 을 따라 잡히므로
 * soft 세션의 풀은 작다"* 라고 **반대**를 적고 있었다 — 두 파일이 서로 모순인 채 게이트가
 * 전부 초록이었다.
 *
 * 왜 안 잡혔나: 그것을 «잰다» 던 단언이 `artLightPoolSize(4) === artLightPoolSize(4)` 라
 * **동어반복**이었다. 검수관이 코드를 이 문장대로 고치는 뮤테이션을 넣자 **0 failed** 였고
 * (대조로 조건 1·2·4·5 는 각각 4·1·6·1 failed), 다섯 중 하나만 장식이었는데 나는 다섯 다
 * 있다고 보고했다.
 *
 * **이 문장을 지키는 자리는 여기가 아니라 집행부다** — 순수 함수는 인자로 받은 값을 그대로
 * 쓸 뿐 «누가 무엇을 넘기는가» 를 강제할 수 없다. 축은
 * `tests/world2-artwork-scene.test.ts` 의 **「★ GS-C」**이고, 그것은 `perParcel` 1 과 4 로
 * 각각 씬을 만들어 **라이트 수가 같고 켜진 수만 다른가**를 한 테스트에서 비교한다.
 * 두 값을 한자리에서 비교하지 않으면 어떤 단언도 동어반복으로 무너진다.
 */
