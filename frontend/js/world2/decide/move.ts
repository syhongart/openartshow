// world2/decide/move.ts — **걷기·보기의 순수 판정.** three 도 DOM 도 안 만진다.
//
// ── 왜 떼어냈나 (팀장 조건 ②, 2026-08-19) ──────────────────────────────────
// 비행 편집(`decide/fly.ts`)을 `PlayerSystem` 에 얹으려는데 **게이트가 막았다.**
// `scripts/smoke/filesize-baseline.json` 이 `systems/player.ts` 를 **633줄로 동결**하고
// 있어서(`over` 항목), 배선을 한 줄만 더해도 `check:filesize` 가 FAIL 한다. 팀장이
// *"분해를 선행 PR 로 (행위 불변 · 기존 테스트 단언 무수정)"* 을 조건으로 단 실질적
// 근거가 이것이다 — **얹으려면 먼저 덜어야 한다.**
//
// 그 게이트는 「기존 부채는 동결하고 새 부채는 막는다」가 목적이고(감독 지시 2026-08-16
// *"파일사이즈 폭주 안되고"*), 여기서 정확히 설계대로 동작했다. 우회하지 않고 따른다.
//
