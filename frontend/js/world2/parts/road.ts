...
/**
 * 노면 높이(미터). **다리가 이 값을 가져다 쓴다** — 어긋나면 진입부에 턱이 생긴다.
 *
 * 지면(피벗 위쪽)보다 아주 살짝 위다. 같은 높이면 z-fighting 으로 지글거린다.
 * 정원(0.07) 위이기도 하다 — 예전 0.06 은 정원(당시 0.03)과 3cm 차이라 먼 거리에서
 * 깊이가 뭉갤 수 있었다. 14cm 로 벌린다(계단으로 보이지 않으면서 확실히 앞이다).
 *
 * ── 왜 export 하는가 (검수관 권고 2026-07-30) ──────────────────────────────
 * `parts/bridge.ts` 가 노면을 여기에 맞춘다. 처음엔 다리 쪽에 `0.14` 를 따로 적고
 * 주석에 *"도로와 정확히 같다"* 고 썼는데, **그것이 곧 값 미러링이다** — 이 값을 바꾸면
 * 다리만 옛 높이에 남아 진입부에 턱이 생기고, 그것을 잡을 테스트가 없었다.
 * 이 저장소가 색·고도·임계값에서 이미 세 번 겪은 형태다.
 */
