 ...
 * 12cm 떠 버린다.
 *
 * 바닥 높이는 종류가 아니라 **자리**의 속성이다(`parts/surface.ts`, 팀장 판정
 * 2026-08-12). 그래서 캐스터인 나무의 `y` 에 얹는다 — 나무는 이미 그 자리의 바닥을
 * 받은 뒤이므로, 여기서는 판끼리 깊이 다툼을 피할 만큼만 띄우면 된다.
 *
 * 5cm 인 것은 `road.ts` 가 남긴 실측 때문이다 — *"3cm 차이라 먼 거리에서 깊이가 뭉갤
 * 수 있었다."* 잔디 위에서는 결과가 0.12 로 예전과 같다.
 */
const PIT_LIFT = 0.05;
