 ...
 *
 * 아래 프로파일의 최대 반지름과 같은 값이다. 스폰이 이 안에 들어오면 카메라가 분수대
 * 속에 갇히는데, 이 프로젝트에는 플레이어-메시 충돌이 없어 아무것도 밀어내지 않는다.
 * `tests/world2-grid.test.ts` 가 이 값과 `SPAWN` 의 거리를 비교해 지킨다.
 */
...
export const fountain: PartSpec = {
  ...
  // ⚠ 그 사실을 여기서 `standsOn: ROAD_SURFACE_Y` 로 신고했었고 **2026-08-12 에 걷었다**
  // (팀장 판정). 높이는 파츠가 아니라 **자리**의 속성이라 `parts/surface.ts` 가 답한다 —
  // 이 파츠는 도로 위에 서므로 그쪽 `onRoad` 분기가 같은 0.14 를 준다.
  shadowProfile: 'round',
  footprint: () => 2.7,
