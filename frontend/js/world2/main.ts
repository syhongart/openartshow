...
export async function startWorld2(canvas: HTMLCanvasElement): Promise<WorldHandle | null> {
  ...
  // 그 밖이 전부 균일한 안개색 — 즉 가리개가 가리는 대신 **띠가 된다.** 낮의 띠
  // (하늘 217 / 띠 229.3)가 색공간과 무관하게 남아 있던 것이 이것으로 설명된다.
  // 색공간·수평선 밴드로 건드린 것은 그 위에 얹힌 증상이었다.
  //
  //   ?fogd=0    안개 없음 — **가리개를 없앤 화면.** LOD 경계가 드러난다(그것을 본다)
