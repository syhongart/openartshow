...
/**
 * 돔 반경. 카메라 far(1200)보다 작아야 잘리지 않는다(`sky.js`가 지오메트리를 교체하며 유지).
 *
 * ── 노브로 연 이유 (감독 지시 2026-08-05) ────────────────────────────────────
 * *"하늘도 천장이 좀 가까워. 하늘도 더 더 높아야 돼."*
 *
 * **돔이 플레이어를 따라오면(아래 `update`) 반경은 시차에 영향을 주지 않는다** — 어디에
 * 서든 같은 거리이므로 무한대로 읽힌다. 그래서 반경이 정하는 것은 "얼마나 먼가" 가
 * 아니라 **천장의 높이감**(같은 화각에서 하늘이 덮는 곡률)과, 돔 안쪽에 붙는 구름·별
 * 레이어의 스케일이다. 폴리곤 수는 반경과 무관하므로 **키워도 비용이 같다.**
 *
 * 값은 감독 실기기 판정 대기다(`?dome=` 로 비교). 판정이 나오면 근거를 여기 적는다.
 */
const DOME_MIN = 300;
 ...
 * 비용이 없다**는 것을 실측하고 열었다 — 24비트 깊이 버퍼의 정밀도는 `near` 가
 * 지배하므로 far 를 1200 → 5000 으로 바꿔도 z=76.8m 에서 3.515mm → 3.516mm 다.
 * 즉 z-fighting 위험이 사실상 없다.
 *
