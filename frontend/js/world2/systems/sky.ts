 ...
 * 그중 하나만 바뀌어도 근거가 썩는다. 위 비율은 곱해지는 인자가 양쪽에서 약분되므로
 * **z 에도, 비트수에도, 버퍼 포맷에도 의존하지 않는다** — 16비트든 WebGPU 의
 * `depth24plus` 든 reversed-Z float 든 0.007% 다. 그래서 이 항목에 한해 **WebGPU
 * 미검증이 사각이 아니다**(산술이 백엔드 독립이다).
 *
