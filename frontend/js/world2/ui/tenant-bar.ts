// world2/ui/tenant-bar.ts — 「누구의 땅을 보고 있는가」 + 다른 작가로 가는 문.
//
...
//
// ── 룩은 새로 정하지 않는다 ───────────────────────────────────────────────
// 색·간격은 `#w2-god` 패널의 것을 그대로 따른다(`world2.html` 의 CSS 변수). 감독이
...
export function findTenantBar(doc: Document): TenantBarParts | null {
  const wrap = doc.getElementById('w2-tenant');
  const title = doc.getElementById('w2-tenant-title');
  const notice = doc.getElementById('w2-tenant-notice');
  const list = doc.getElementById('w2-tenant-list') as HTMLSelectElement | null;
  const input = doc.getElementById('w2-tenant-input') as HTMLInputElement | null;
  const go = doc.getElementById('w2-tenant-go');
  const reset = doc.getElementById('w2-tenant-reset');
