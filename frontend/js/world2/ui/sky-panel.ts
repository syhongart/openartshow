...
//
// ⚠ **`time` 축만 예외다** (2026-08-21). 복합씬(`daylit`)은 팔레트로 갈 때 낮으로 접히
// 므로 「반영된 상태」를 그대로 표시하면 **「복합」 버튼이 눌린 것으로 안 보인다.** 그래서
// `SkySystem.get().time` 은 요청 원값을 내고, 엔진 반영값은 같은 객체의 `engineTime` 으로
// 따로 실린다(그 필드 주석이 이유를 소유한다). 아래 계약은 나머지 축에 대해 그대로다.
//
...
export function findSkyPanel(doc: Document): SkyPanelParts | null {
  const panel = doc.getElementById('w2-god');
  const toggle = doc.getElementById('w2-god-toggle');
  const body = doc.getElementById('w2-god-body');
