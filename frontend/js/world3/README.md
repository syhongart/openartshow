...
검수관 권고 P5(2026-08-08): *"world2 트리에는 night·daylight·sky-fog-tint 등이
있고 world3 에는 둘뿐인데, **어떤 판정 게이트가 포크와 함께 오지 않았는가**가
어디에도 없다. 승격 때 무엇을 채워야 하는지 아무도 모르게 된다."*

테스트는 `tests/` 에 있고 `world2/**` 를 import 하므로 **포크를 따라오지 않는다.**
주장(주석)만 따라온다 — 그래서 포크 직후 world3 에는 *"그 정합은 테스트가 지킨다"*
라고 적힌 자리가 **검사 없이** 있었다(검수관 반려 B1).

| 축 | world2 | world3 | 상태 |
|---|---|---|---|
| 격리(트리·진입점·HTML) | — | `world3-independence.test.ts` | ✅ |
| 파츠 자산·팔레트·색 리터럴 | `world2-parts-assets` | `world3-parts-assets.test.ts` | ✅ |
| 밤 안개 ↔ 하늘 팔레트 정합 | `world2-night` | `world3-night.test.ts` | ✅ (B1 처방) |
| 블룸 문턱 ↔ 등불 휘도 | `world2-night` | `world3-night.test.ts` | ✅ (P4 처방) |
| 개수 불변식 `[7]`·드로우콜 `[7.6]`·하늘 예열 `[8]` | 스모크 | **없음** | ⏸ 이연 — GS-4 가 승격을 막는다 |
| 그 밖의 world2 단위 테스트 40여 종(도로·파셀·스트리밍·LOD·NPC·물…) | 있음 | **없음** | ⏸ 이연 |

마지막 줄이 가장 크다. **`place` 계열을 한 줄도 안 건드린 것이 그 이연의 근거다** —
배치·슬롯 예산·겹침 로직이 world2 와 같은 코드이므로 world2 테스트가 그 로직의
정당성을 여전히 증명한다. 그러나 그것은 **지금 같기 때문**이고, world3 에서 배치를
손대는 순간 그 근거가 사라진다. 배치를 건드릴 사람은 이 줄을 먼저 읽어라.
