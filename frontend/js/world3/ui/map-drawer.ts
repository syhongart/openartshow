...
export function findMapDrawer(doc: Document): MapDrawerParts | null {
  const wrap = doc.getElementById('w3-mapwrap');
  const tab = doc.getElementById('w3-map-tab');
...
// 펼쳐져 있으면 그 캡처마다 왼쪽이 UI 로 먹힌다. 성능 패널(`#w3-hud`)은 이미
// `data-open="0"` 으로 접혀 있었고, 지도만 열려 있었다.
//
// 기본값을 바꾸는 것으로 끝내고 새 코드를 만들지 않는다 — 책갈피·토글·transform 접기가
// 이미 여기 다 있다. **`world3.html` 의 `data-open` 속성도 함께 `0` 으로 맞춘다**:
