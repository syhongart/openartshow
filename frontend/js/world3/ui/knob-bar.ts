...
export function findKnobBar(doc: Document): KnobBarParts | null {
  const panel = doc.getElementById('w3-sliders');
  const toggle = doc.getElementById('w3-sliders-toggle');
  const body = doc.getElementById('w3-sliders-body');
...
export function attachKnobBar(parts: KnobBarParts, specs: readonly KnobSpec[]): KnobBar {
  ...
  for (const spec of specs) {
    ...
    val.className = 'w3-slide-val';

    const reset = doc.createElement('button');
    reset.type = 'button';
    reset.className = 'w3-slide-reset';
