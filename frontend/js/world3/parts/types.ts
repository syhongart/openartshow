...
export interface PartSpec {
   ...
   *
   * 텍스처가 있으면 그 텍스처를 굽는 데 쓴 바탕색을 **같은 상수로** 적는다 —
   * 캔버스와 여기에 따로 적으면 미러링이다.
   *
   * ── ⚠️ "`map` 이 없으면 `0xffffff`" 는 **절반만 맞다** (2026-08-08 마을 개조) ──
   * 원래 이 자리에 *"`asset` 에 `map` 이 없으면 곱해질 것이 흰색이므로 `0xffffff` 다"*
   * 라고 적혀 있었다. world2 의 지면 파츠가 통짜 색이라 참이었을 뿐이고, **정점색을
   * 굽는 지면을 상정하지 않은 문장**이다.
   *
   * 곱해질 것이 있는지를 가르는 것은 `map` 의 유무가 아니라 **재질에 이미 색 정보가
   * 있는가** 이고, 그 색이 텍스처에서 왔는지 정점에서 왔는지는 상관없다
   * (`tests/world3-parts-assets.test.ts` 의 `isMultiplier` 가 같은 기준을 쓴다).
   * world3 의 `ground` 는 `map` 없이 정점색으로 잔디 캡과 흙 단면을 나누므로,
   * 알베도 원천은 흰색이 아니라 **잔디색**이다.
   *
   * 다행히 판정식(`groundBase × tones 평균`)은 두 경우를 이미 같은 식으로 덮는다 —
   * 틀린 것은 계산이 아니라 **산문**이었다. 초목 담당이 실측으로 잡았고, 여기 고친다.
   * 규칙을 다시 적으면: **정점색이든 텍스처든, 그 재질이 실제로 내는 바탕색을 적어라.
   * 아무 색도 없을 때만 `0xffffff` 다.**
   *
