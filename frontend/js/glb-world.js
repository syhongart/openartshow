...
//
// ── 감독 지시 2026-08-25·26 ─────────────────────────────────────────────────
// *"아까 그 파일을 올려서 월드7로 해봐"* · *"월드8에 그 glb를 올려보자"*.
// **파일이 곧 세계다** — world2 의 「편집본 불러오기」가 재질 이름으로 우리 파츠를
// 알아보고 우리 모양으로 다시 세우는 것과 반대다(그래서 블렌더의 모양 변경이 저기서는
// 안 보인다). world8 의 고정 자산은 감독이 파일을 못 올려서 **같은 왕복을 저장소에서
// 재현해** 구운 것이다(`scripts/asset/` 두 스크립트).
// `lab-glb.js` 를 안 쓴 이유: 저쪽은 고정 파일·고정 층·고정 바운딩이 전제이고 이미
// 509줄이라 상한에 닿아 있다.
//
...
//
// ── 지금 **안** 하는 것 ─────────────────────────────────────────────────────
// · **후처리·잔디·TSL 물 — 아직**(순서는 `docs/BOARD.md` 팀장 판정 한 곳).
// · **NPC·물 배치·편집·저장 — 원리상 안 된다**(절차적 판정 산출물에 의존).
//   ⚠ **미니맵은 이 목록에서 빠졌다**(2026-08-26): 코드 이식은 여전히 불가하지만
//   **위에서 찍어 굽는 길**이 따로 있었다 — `glb-hud.js`. 「코드를 못 옮긴다」와
//   「그 기능이 불가능하다」는 다른 일이고, 이 목록이 그것을 한 번 섞었다.
...
import { setupShadow, applyTimeOfDay } from './glb-shadow.js';
import { bakeMap, createHud } from './glb-hud.js';
...
let hud2 = null;         // 미니맵·계기·시간대 — 씬이 바뀌어도 한 번만 만든다
 ...
 *
 * ⚠ **첫 판본이 여기서 감독 신고를 냈다**(*"스폰 위치도 없고. 걸어다닐수가 없네"*).
 * *"가장 긴 축 바깥에서 안쪽을 보게 둔다"* 는 **작은 물건을 전제한 계산**이었고 1920m
 * 세계에서는 `pos {0, -2.5, 960}` — **남쪽 경계, 땅속 2.5m** 가 됐다(실측).
 * 「물건을 본다」는 밖에서, 「세계를 걷는다」는 안에 서야 한다 — `WALKABLE_SPAN` 로 가른다.
 * 지면은 바운딩으로 알 수 없어 레이캐스트로 찾는다(**스폰 1회**라 비용이 감당된다).
 */
function place(gltf) {
  disposeRoot();
  ...
  // ── 미니맵·계기·시간대 (감독 지시 2026-08-26) ───────────────────────────
  // 지도는 **위에서 한 번 찍어** 굽는다 — world2 의 절차적 미니맵은 임의 GLB 에 못
  // 옮기지만, 파일에 실제로 있는 것을 그대로 찍으면 더 정확하다. 사유는 `glb-hud.js`.
  if (!hud2) {
    hud2 = createHud(document, { onTime: (h) => applyTimeOfDay(scene, sun, h) });
    applyTimeOfDay(scene, sun, 12);
  }
  if (!box.isEmpty()) {
    const t1 = performance.now();
    hud2.setMap(bakeMap(renderer, scene, box));
    console.info(`[${PAGE}] 지도 굽기 ${Math.round(performance.now() - t1)}ms`);
  }
...
function frame(now) {
  ...
  if (ready) {
    ...
    hud2?.setPose(pos, yaw);
    camera.position.copy(pos);
    camera.rotation.set(pitch, yaw, 0, 'YXZ');
  }
  renderer.render(scene, camera);
  // 계기 — 1초 창으로 평균 낸다. 프레임마다 찍으면 숫자가 튀어 읽을 수가 없다.
  fpsN++;
  if (now - fpsAt >= 1000) {
    hud2?.setStat((fpsN * 1000) / (now - fpsAt), renderer.info.render);
    fpsAt = now; fpsN = 0;
  }
}
requestAnimationFrame(frame);
