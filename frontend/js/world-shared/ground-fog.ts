...
/**
 * 기본값. **세기 0 = 지면 안개를 켜지 않는다 — 감독 확정값이다.**
 *
 * ── 감독 판정 2026-09-05: *"기본이 낫다."* ─────────────────────────────────
 * 후보 셋을 라이브 링크로 드렸다 — `?fogs=0.5&fogh=1&fogk=0.45`(옅게) ·
 * `?fogs=0.8&fogh=2&fogk=0.3`(중간) · `?fogs=1&fogh=3&fogk=0.2`(짙게) — 그리고 대조군
 * `?fogs=0`(지금 라이브). 감독은 실기기(WebGPU)에서 넷을 보고 **대조군**을 골랐다.
 * 즉 「바닥에 깔리는 안개」는 이 세계에서 리얼로 읽히지 않았다. 처음 카드에서는
 * 「높이 안개까지」를 고르셨지만, 화면을 본 판정이 그것을 뒤집었다 — 화면이 최종이다.
 *
 * **여기서 멈춘다**(경계도 판정이다). 노브·코드는 남긴다 — 재론 조건은 감독이 안개를
 * 다시 발화하는 회차이고, 그때는 값이 아니라 **다른 축**(거리 밴드의 날씨별 분기,
 * 깊이 기반 후처리)부터 본다. 세 후보의 중간값을 다시 내미는 것은 같은 고리를 한 바퀴
 * 더 도는 것이다.
 *
 * h0·k 는 후보 링크의 출발점이었던 값 그대로 둔다(세기 0 이라 소비되지 않는다).
 * 근거는 눈높이 1.6m 에서 발치가 뿌옇고 3m 위 창문은 또렷하도록 층 천장 1.0m·두께
 * 3/k ≈ 6.7m — **실측 없음**.
 */
