...
import { createStorage, isExternalUrl, buildShareFragment } from './studio-storage.js';
...
(function () {
  ...
  // ---------- 공유 컨텍스트 + 영속(studio-storage.ts) ----------
  // ctx.state 는 단일 참조 — form(변이)·storage(직렬화)가 같은 객체를 공유(복사 금지).
  var ctx = { state: state, limits: _limits, nextUid: nextUid, scheduleSave: null, rerender: null };
  var _storage = createStorage(ctx, { saveIndicator: $saveIndicator });
  var scheduleSave = _storage.scheduleSave;
  var loadDraft = _storage.loadDraft;
  var buildGalleryJson = _storage.buildGalleryJson;
