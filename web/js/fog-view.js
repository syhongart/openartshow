 ...
 *
 * 채택 기준: 완전 소멸 단계(SHELL→UNLOAD)의 EXIT를 활성 fog.far까지 끌어올린다 — 단 상한(정상 EXIT)을
 * 넘지 않는다. 즉 실제 보장은 `shell >= Math.min(fog.far, shellExit)`이며 "프리셋 무관 100% 은폐"가
 * 아니다: fog.far가 정상 SHELL_EXIT(42m)보다 큰 프리셋(현재 `?lod=a`의 45.6m)에서는 shell=42로
 * 3.6m 미달한다. 그 경우 축소량이 0이 되어 **배후 로직 미적용(변경 전)과 동일 거리에서 언로드**되므로
 * 이 변경이 만드는 신규 노출은 없다(기존 baseline 그대로). 근본 해소는 STREAM_SHELL_EXIT 자체를
 * 올리는 설계 사안이라 이 함수 범위 밖. FULL→SHELL은 강등 후에도 shell이 남으므로 fog.far의 85%까지 완화.
 *
 * - **하한**(`Math.max`): 안개보다 안쪽에서 소멸/강등하지 않게 — 상한에 걸리지 않는 범위에서 충족.
