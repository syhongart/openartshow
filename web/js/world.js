 ...
 * PIT_RADIUS(1.3, 감독 확정값)는 불변이고, 건물에 가까운 나무만 반경이 줄어든다. 평면 데칼이 불투명
 * 건물 볼륨 아래로 파고들어 벽 하단에 흙·벽돌이 비치는 것을 막는 장치다. 현행 배치(genStreet가 가로수를
 * 도로에서 2.3m 안쪽에 세운다)에서 실측 클램프 대상은 331그루 중 2그루뿐이고 줄어드는 반경도 1.02~1.09
 * 수준이며, 나머지는 건물까지 거리가 1.3 이상이라 무변화다. 이 대상 수·파셀은 배치 좌표를 바꾸면 함께
 * 바뀌므로(가로수를 건물 쪽으로 물릴수록 늘어난다) 특정 파셀 번호를 여기 적어두지 않는다.
...
export async function createWorld({ canvas, parcels = [], opts = {} } = {}) {
  ...
  function processLoadQueue() {
    ...
    while (i < loadQueue.length) {
      ...
      // 완성 → 예열 후 부착. 큐에서만 빼고 queued·inflightFull·쿨다운 회수는 attachParcel이 "실제
      // 부착 시점"에 한다. 여기서 미리 회수하면 예열이 도는 동안 queued가 비어 updateStreaming이
      // 같은 키를 다시 큐잉하고(중복 조립), 쿨다운 기준시각도 컴파일 착수 시각으로 찍혀 컴파일이
      // 400ms를 넘는 기기에서 순차 등장이 깨진다(교차리뷰 블로커).
      // [히칭 프로파일] finalize(섀도 베이크 요청 등)도 별도 스테이지로 계측
      if (job.stage >= STAGES.length) { const _sf = perfNow(); finalizeParcel(job.ctx, true); profStage('final', perfNow() - _sf); loadQueue.splice(i, 1); }
      stagesRun++;
