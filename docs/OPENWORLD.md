...
신 모드에서 감독이 명시적으로 켜는 이벤트 연출(오로라 +4 → 야간맑음 256 등)의 일시 초과는 팀장 서명으로 수용한다.
근거 — 오로라 +4는 승인된 sky.js 2겹 커튼(transparent+DoubleSide 메시당 2패스)의 고유 비용이며 하늘 배선의 회귀가
아니다(배선 전후 기본 드로우콜 동일 252, 헤드리스 실측). 비/눈 253, 무지개 252 등 그 외 전 조합은 임계 이내.
