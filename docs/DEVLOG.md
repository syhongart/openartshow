...
| `gmode=card` · `gcard` | **다발 카드** — 넓은 사각 두 장 교차에 가는 잎 `gcard` 개를 절차적으로 그린 마스크. 인스턴스 배율은 **잎 면적 등가**로 유도(`cardDensityMul` — 첫 판본 1/N 은 맨땅이 드러났다). 감독 *"옆으로 넓은것 같아. 게임회사는 어떻게 해?"* | ✅ 월드8 기본 = card · 잎 10(감독 «4번으로 굽는다» 2026-09-06) · world2 는 노브 전용 | 3~12 |
| `gaa` | **잎 윤곽 AA** — 0 끔 / 1 alpha-to-coverage / 2 a2c+밉맵 / 3 a2c+해상도 4배 / 4 셋 다(요소별 — 첫 누적형은 «5링크 이상해»로 갈라 다시 열었다). ✅ 기본 4 = 셋 다(감독 «4번이 좋아» → «5번째가 좋아 보이네. 무겁지 않나?» — 비용 ≈0). 감독 *"나뭇잎에 알리아싱이 눈에 거슬리는데. 안티알리아싱하면 비용이 크지?"* → 거의 0(MSAA 는 이미 켜져 있고 알파 컷 윤곽만 못 잡는다 — `decide/grass-mode.ts` «잎 윤곽 안티알리아싱») | 0 | 0~3 |
...
### 카드 밀도 규정이 틀렸다 — 감독 *"마지막 링크 들어가면 이상해"*

배포된 다발 카드 링크(`?gmode=card&gcard=10`)에서 지면이 맨땅이었다. `cardDensityMul = 1/N`
— «카드 하나가 잎 N 개 몫» 을 **삼각형** 등가로 읽었고, 화면은 잎 **면적**으로 덮임을 본다.
카드 잎은 quad 잎 폭의 0.216 배(`CARD_LEAF_SCALE × CARD_WIDTH_MUL`)이고 교차 두 장의 투영
폭 평균은 4/π 장이라 N=6 카드는 quad 잎 1.65 개분인데 인스턴스를 6 분의 1 로 줄였다 →
덮임 27%. **수치(삼각형)가 맞아도 화면이 틀린다** — 헤드리스 대조 세 장(현행/card6/card10)
을 배포 **전에** 안 찍은 것이 사고의 형태다. 처방은 값이 아니라 **유도식**이다:
`1/(N × CARD_SHEETS × CARD_LEAF_SCALE × CARD_WIDTH_MUL)`, `CARD_SHEETS = 4/π`. 테스트도
같은 식에서 유도해 1/N 으로 되돌리면 깨진다. 재촬영에서 card6·card10 덮임이 현행과 같다.
삼각형: 카드 4tri × 배율 — N=6 은 quad 대비 **+22%**(경량이 아니다), N=10 은 −27%.

그런데 감독은 배포된 **옛 밀도의 잎 10 카드**를 *"방금 이걸로 보니 좋아 보이네"* 라고 했다.
내가 «맨땅» 으로 규정한 화면이다 — **수치가 이상한 것과 화면이 잘못된 것은 다른 일**이고 이번에도
그 형태다. 그래서 면적 등가 기본값을 굽되 옛 화면은 기존 노브로 재현해 나란히 드린다
(`?gmode=card&gcard=10&gden=0.27` — 0.1 / 0.364). 감독 판정이 최종이다.
같은 답에서 *"나뭇잎에 알리아싱이 눈에 거슬리는데. 안티알리아싱하면 비용이 크지?"* — 답은
**거의 0** 이다: MSAA 4x 는 이미 켜져 있고, 계단은 `alphaTest` 가 픽셀 단위로 버린 윤곽이라
MSAA 가 못 잡는 것뿐이다. alpha-to-coverage(GPU 가 이미 하는 리졸브를 쓴다) · 밉맵 · 마스크
해상도 셋을 `?gaa=1·2·3` 누적 후보로 열었다. 헤드리스(swiftshader MSAA 4x) 근접 대조에서 3 이
윤곽 조각이 덜하다 — 실기기 판정은 링크로.
