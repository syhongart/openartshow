...
| `gaa` | **잎 윤곽 AA** — 0 끔 / 1 alpha-to-coverage / 2 a2c+밉맵 / 3 a2c+해상도 4배 / 4 셋 다(요소별 — 첫 누적형은 «5링크 이상해»로 갈라 다시 열었다). ✅ 기본 2(감독 «4번이 좋아»). 감독 *"나뭇잎에 알리아싱이 눈에 거슬리는데. 안티알리아싱하면 비용이 크지?"* → 거의 0(MSAA 는 이미 켜져 있고 알파 컷 윤곽만 못 잡는다 — `decide/grass-mode.ts` «잎 윤곽 안티알리아싱») | 0 | 0~3 |
