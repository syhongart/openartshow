...
| `gmode=card` · `gcard` | **다발 카드** — 넓은 사각 두 장 교차에 가는 잎 `gcard` 개를 절차적으로 그린 마스크. 인스턴스 배율은 **잎 면적 등가**로 유도(`cardDensityMul` — 첫 판본 1/N 은 맨땅이 드러났다). 감독 *"옆으로 넓은것 같아. 게임회사는 어떻게 해?"* | card 는 노브 전용 · 잎 6 | 3~12 |
| `gaa` | **잎 윤곽 AA** — 0 현행 / 1 alpha-to-coverage / 2 +밉맵 / 3 +마스크 해상도 4배. 감독 *"나뭇잎에 알리아싱이 눈에 거슬리는데. 안티알리아싱하면 비용이 크지?"* → 거의 0(MSAA 는 이미 켜져 있고 알파 컷 윤곽만 못 잡는다 — `decide/grass-mode.ts` «잎 윤곽 안티알리아싱») | 0 | 0~3 |
