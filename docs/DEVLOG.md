...
> 감독: "우리 있는 걸 이런 식으로 다 검토해 보자. 이게 완성도고 이 차이가 크다.
> 제너레이티브 아트가 이렇게 9999개 캐릭터를 만들잖아. 수염·대머리·로봇·귀신·수영복도
> 만들어 보자." → 팀장에게 완성도 오버사이트를 위임.

### 원인
캐릭터 시스템(`web/js/chibi.js`)은 **파라미터가 곧 아바타**다 — 20축 JSON을 THREE
프리미티브로 동기 조립하고, 뼈 없이 피벗 그룹을 사인파로 움직인다. 종족·옷·색 조합은
많아졌지만, 개별 파츠의 **완성도(3초 안에 "저건 여우/펭귄이다"가 읽히는가)** 에 편차가
있었다. 감독이 짚은 방향은 "종 수를 늘리기 전에 있는 것부터 제대로".

### 분석 — 완성도의 정의
"완성도"를 **3초 식별 바(bar)** 로 정의했다. 정면·측면·후면·걷기 한 프레임에서 그 종족/
옵션이 즉시 읽혀야 통과. 이 기준으로 전 파츠를 감사한 결과:
- 꽃 장식·리본이 머리에 **박혀** 보임(표면 아래로 파묻힘).
- 안경 링이 과대, 사자 갈기가 한 줄이라 빈약, 펭귄이 배(흰색)가 없어 새로 안 읽힘.
- 호랑이 줄무늬 텍스처가 몸을 타고 어긋남 → **감독 판단으로 호랑이 종족 제외**.
- 동물 꼬리가 스윙 시 몸을 뚫음.

### 개선 — 4개 배치, 전부 §10 게이트(디자이너→구현→스모크→검수관→팀장 판정)
완성도 수리를 먼저(감독 우선순위 "완성도 수리부터"), 그다음 신규 확장.
...
### 그리고 — 꼬리 "공중 부양" 회귀 (감독 캐치)
배포 후 감독이 모바일에서 잡았다: "몇 개는 꼬리가 이렇게 떠 있어." 관통 방지 수정에서
꼬리 뿌리를 몸 뒤로 **과하게**(z=−0.28) 밀어낸 게 원인이었다. 뒤·측면에서 보면 엉덩이
