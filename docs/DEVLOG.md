...
**배포 요약 (run별)**
- **#137** — 정식 승격(베타 제거·sitemap·P2P 고지) + 그래픽 1단계(지면 텍스처·노멀·시임·타일링)
- **#140** — 모바일 A-1·A-2: 파셀 첫 로드 히칭 30.8→6.0ms (**80%↓**)
- **#141** — 모바일 B-1: FPS→해상도 실시간 적응(+고dpr no-op 결함 수정)
- **#146** — 팝인 완화: 가벼운 fog + 조명 0.5초 페이드업
- **#149** — 모바일 A-3(텍스처 힙 **-88%**) + B-2(하늘 오버드로우 lite 축소)
