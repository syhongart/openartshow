...
> 범위 밖 — 착수 시 **[`docs/SCALING.md`](./SCALING.md)** 에서 다룬다.
>
> ⚠ **이 줄은 오래 `MIGRATION.md` 도 함께 가리키고 있었고 그것은 빈 곳이었다**(2026-08-16
> 정정). 그 파일은 11줄짜리 **저장소 이사 완료 기록**(lifeunity → openartshow, 2026-07-13)
> 이고 스스로 *"절차는 종료됨"* 이라 닫아 놨다 — 스케일 전환과 아무 관계가 없다.
> **SSOT 가 「착수 시 여기서 다룬다」고 약속한 자리 절반이 종료된 남의 기록이었고**,
> 그 포인터를 따라간 사람은 빈손이 된다. 이 저장소가 `main` unprotected 오기로 7일을
> 잃은 것과 같은 형태다 — **틀린 사실이 SSOT 에 남아 있는 것의 대가.**
>
> **재론 조건이 이 절에 없다는 것도 함께 적어 둔다.** *"현행 유지"* 의 근거 원문은
> 저장소에서 못 찾았고(출처가 «감독 2026-07-25» 뿐), 언제 이 방침을 다시 보는지도
> 안 적혀 있다. 같은 문서 §7-0(Node 절)은 *"2026-10-20 에 이 절을 재검토한다"* 라고
> 날짜까지 박아 두었으므로 **관행이 없어서가 아니라 이 절에만 빠진 것**이다.
> 트리거는 `SCALING.md` 의 단계별 트리거가 대신한다 — 그쪽이 SSOT 다.

### 7-0. Node 버전 — **24 하나로 맞춘다** (감독 결정 2026-08-08 *"다 v24 로 맞추면"*)

SSOT 는 **`.nvmrc`(`24`) + `package.json` 의 `engines.node: "^24"`** 이고, 워크플로 6곳의
`node-version: '24'` 이 그것을 따른다. 두 파일은 주석을 달 수 없어(nvm 이 파싱 / JSON) 근거를
여기 적는다. **세 축의 정합은 `tests/node-version-consistency.test.ts`(G-NODE1)가 지킨다** —
그 검사가 없던 동안 검수관 뮤테이션(워크플로만 `'25'`)이 게이트 6종을 **전부 통과**했다.

`engines` 가 `>=24` 가 아니라 **`^24`(라인 고정)인 이유**: `>=24` 는 26·27 을 전부 허용한다.
그러면 이 절의 제목(*"24 **하나로** 맞춘다"*)과 값이 서로 다른 것을 말하게 되고, **2026-10-28
에 v26 이 LTS 로 승격되는 순간 갈라진다** — 개발자가 26 을 깔아도 경고가 안 나고, 개발 환경과
CI 가 다시 어긋난다(바로 그것을 없애려고 이 절을 만들었다).

**왜 바꿨나 — 감독 질문 *"우리 노드가 ci(node 20)이야?"* 에서 나왔다.** 실측(`schedule.json`):

| 라인 | 지원 종료 | 2026-08-08 기준 |
|---|---|---|
| **v20** (직전 판본) | **2026-04-30** | **EOL 3개월 경과** — 보안 패치 없음 |
| v25 | **2026-06-01** | EOL. **홀수 라인은 LTS 로 승격되지 않는다**(약 6개월 수명) |
| v22 | 2027-04-30 | maintenance |
| **v24 (채택)** | **2028-04-30** | Active LTS. **단 maintenance 진입 2026-10-20** |
| v26 | 2029-04-30 | LTS 승격 2026-10-28 예정 — 아직 Current |

⏳ **"현재 유일한 Active LTS" 라고 쓰지 않는다 — 그 문장은 2026-10-20 에 거짓이 된다.**
v24 는 그날 maintenance 로 내려가고 8일 뒤 v26 이 Active LTS 가 된다. **만료가 예정된 사실을
만료일 없이 문서에 박는 것**이 이 저장소가 반복한 형태다(`main` unprotected 오기가 7일을
잃은 그 형태). → **2026-10-20 에 이 절을 재검토한다.** 그때도 v24 는 2028-04-30 까지 보안
패치를 받으므로 **즉시 옮길 이유는 없다** — 재검토는 "옮겨라" 가 아니라 "이 표가 아직 참인가"다.
그리고 **문서에 적은 재검토 날짜는 그날 아무도 안 본다** — 그것을 기계가 보게 하는 것이
G-NODE2(태스크 #220)이고, 그것이 없는 동안 이 줄은 약속일 뿐이다.

**왜 20 이었나: 이유가 없었다.** 처음 적힌 값이 그대로 남았고 `engines` 도 `.nvmrc` 도 없어
**버전을 강제하는 장치 자체가 없었다.** 그래서 개발 환경(당시 22.22.2)과 CI(20)가 **서로 다른
버전으로 검사**하고 있었다 — 이 저장소가 가장 비싸게 배운 형태(재는 축이 실제와 다른 것)다.

**로컬 Node 24 실측 (2026-08-08).** 이 컨테이너의 기본 node 는 22 지만 24 로 돌렸다 — 방법은
`nodejs.org` 에서 tarball 을 받아 `PATH` 앞에 두는 것뿐이다(nvm 은 이 환경에서 안 돈다).

| 축 | Node 24.19.0 / npm 11.17.0 |
|---|---|
| `npm ci` | exit 0 |
| **`npm run gate` 6종** | **전부 PASS · 스탬프 `6b18980493b8`** (22 스탬프와 동일) |
| **`npm run smoke:vite`** | **PASS 17 · FAIL 0** |
| `vite build` · 생성기 6종 · `build-valuation`·`build-readme` | 전부 exit 0 |
| **`dist/` 74파일 + 생성기 산출물** | **Node 22 빌드와 바이트 동일**(`diff -r`) |

**누가 쟀는가** — 이 저장소에서 그것은 절차적 의미를 갖는다(§10-3 구현자 자기검증 금지).
`npm ci`·게이트 6종은 **executor**, 나머지 5행(`smoke:vite`·`vite build`·생성기·valuation·
`dist` 바이트 동일)은 **검수관**이 1차 리뷰에서 잰 것이다. **부팀장(구현자)이 직접 잰 행은
하나도 없다.** ⚠ 다만 `smoke:vite` 를 검수관이 돌린 것은 절차상 어긋난다 — §10-3 이 요구하는
것은 **독립 executor 스모크**이고 검수관 책임에는 "직접 스모크는 하지 않음" 이 있다. 결과는
유효한 실측이지만 **이 사이클의 executor 스모크 회차는 0회**다(검수관 자기 지적, 2026-08-08).

마지막 줄이 *"이 변경이 배포물을 안 건드린다"* 의 **진짜 근거**다. 바뀐 것은 파일이 아니라
**그 파일들을 만드는 런타임**이므로, "diff 가 `_site` 에 안 실린다" 로는 논증이 되지 않는다.

⚠ **이 자리에는 원래 *"로컬에서 검증할 수 없었다 … 24 로 게이트를 돌리지 못했다"* 라고
적혀 있었고 거짓이었다.** *"nvm 이 안 돈다"* 에서 *"24 를 못 얻는다"* 로 건너뛴 것이다.
검수관이 tarball 로 반증했고 재현도 됐다. **이 환경의 네트워크는 균일하지 않다** — 실측:
`api.github.com` 403 · 라이브 URL 403 인데 `nodejs.org`·`registry.npmjs.org`·
`raw.githubusercontent.com` 은 **200**이다. **하나가 막혔다고 전부 막힌 것이 아니다.**
*"수단이 없다"* 는 실측한 뒤에만 쓴다 — 검증에 관한 거짓 진술은 다음 사람이 확인을
생략하게 만든다.

**남은 미검증 축은 하나 — `verify-live` 의 `smoke:live`.** 프록시가 라이브 URL 을 403 으로
막아 로컬에서 실행할 수 없다(`node --check` 문법 통과만 확인). 그리고 `deploy.yml` 은
`push: main` 트리거뿐이라 **PR CI 에서 안 돈다** — *"CI 초록 = 24 실측"* 은 `node-version`
6곳 중 **3곳에서만**(`ci.yml` 2 + `review-record.yml` 1) 성립하고, `deploy`·`verify-live` 는
병합 후, `valuation`(cron)은 다음 04:07 KST 에 처음 24 로 돈다.

`engines` 가 `^24` 이므로 22 환경에서는 `npm` 이 `EBADENGINE` 경고를 낸다(실측: `engine-strict`
= false 라 `npm ci` 는 exit 0). **그 경고는 소음이 아니라 "이 환경이 뒤처져 있다" 는 신호다** —
끄지 마라.

**Node 24 는 npm 11 을 데려온다 — Node 만 바뀐 것이 아니다.** 실측한 행동 차이 2건:

1. **install script 기본 차단.** `npm ci` 가 `esbuild`·`puppeteer` 의 `postinstall` 을
   **실행하지 않고** 경고만 낸다. 지금은 무해하다 — esbuild 바이너리는 `@esbuild/linux-x64`
   optional dep 로 들어와 `vite build` 가 exit 0 이고, puppeteer 다운로드는 워크플로가
   이미 `PUPPETEER_SKIP_DOWNLOAD` 로 억제 중이다. **앞으로 postinstall 이 실제로 필요한
   의존성이 들어오면 조용히 미실행된다.**
2. **optional dep 해석 차이.** 같은 lockfile 로 npm 10 = 205 packages / npm 11 = **201**
   (`lightningcss` 계열 4개). 무해 — 저장소 참조 0건이고 `dist` 바이트 동일.

**아직 안 맞은 축 셋** (앞 둘은 감독 지시 2026-08-08 *"버전 매칭도 체크해"* 에서, 셋째는
검수관 자기 지적에서 나왔다):
...
>   **`gh-pages` head 가 정상인데도 라이브가 안 바뀌면 층이 하나 더 있다**(2026-08-06 실측).
>   우리 파이프라인 다음에 **GitHub 관리 워크플로 `pages-build-deployment`**(`dynamic/pages/…`)
>   가 돈다. 이것이 실패하면 gh-pages 는 최신인데 라이브만 옛 판본으로 남는다 —
>   `deploy` job 이 success 라서 우리 쪽 신호는 전부 초록이다.
>   · 판정: 워크플로 ID `311988143` 의 최근 run 을 본다. 실패 로그가
>     `Current status: deployment_in_progress` 반복 → `Timeout reached, aborting!` 이면
>     **GitHub Pages 서비스 층 문제이지 배포물 결함이 아니다.**
>   · 재시도 경로는 **main 에 push 를 넣는 것뿐이다.** `rerun_failed_jobs` 와
>     `workflow_dispatch` 는 통합 토큰 권한으로 403 이 난다(GitHub 관리 워크플로).
>   · 그래도 반복 실패하면 저장소 소유자만 할 수 있다: Settings→Pages 소스 확인 /
>     Actions 탭에서 `pages-build-deployment` 수동 Re-run / githubstatus.com 확인.
>   · **실측 사고(2026-08-06)**: 05:01 성공 이후 12:16 부터 5연속 실패로 라이브가 약
>     10시간 동안 옛 판본을 서빙했다. 그동안 main 커밋 7개·gh-pages 배포 5번이 전부
>     반영되지 않았고, 우리 CI 는 매번 초록이었다. **이 사고는 `verify-live` 없이는
>     아무도 몰랐을 것이다** — 배포 후 축이 왜 필요한지의 실례다.
>   · **재시도가 없으면 현재 상태를 모른다.** main push 가 없는 동안 Pages 는 아예 돌지
>     않으므로 **마지막 실패 기록은 "지금도 장애" 의 근거가 아니다.** 실측: 08-06 15:18
>     이후 9시간 동안 재시도가 0이었고, 그 사이 상태를 물으면 답할 수 있는 것은
>     "모른다 — 다시 걸어야 안다" 뿐이었다. 낡은 기록으로 현재를 단정하지 않는다.
