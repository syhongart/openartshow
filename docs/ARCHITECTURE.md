...
| **v24 (채택)** | **2028-04-30** | **현재 유일한 Active LTS** |
...
⚠ **이 변경은 로컬에서 검증할 수 없었다.** 이 개발 컨테이너에는 node 20/21/22 만 있고
(`/opt/node*`) nvm 이 동작하지 않아 **24 로 게이트를 돌리지 못했다.** 로컬 게이트는 22 에서
돈 것이고, **24 의 유일한 검증 축은 CI(`verify`·`smoke`)다.** 못 잰 것을 통과로 적지 않는다 —
다음에 개발 환경이 24 를 얻으면 로컬 게이트로 교차 확인할 것.
`engines` 가 `>=24` 이므로 22 환경에서는 `npm` 이 경고를 낸다(`engine-strict=false` 라 막지는
않는다). **그 경고는 소음이 아니라 "이 환경이 뒤처져 있다" 는 신호다** — 끄지 마라.
