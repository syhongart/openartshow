...
describe('낮 팔레트와의 관계', () => {
  ...
  it('⭐ 총량이 팔레트보다 **크다** — 그늘을 열었다(뒤집힌 계약, 팔레트에서 유도)', async () => {
    ...
    //
    // ⚠ **상한은 유도한다 — 첫 판본은 `1.5 배`라고 손으로 박았고 검수관이 잡았다**(권고
    // P1): 그것은 실측(1.28배)에 여유를 얹은 값이었고, 이 저장소는 *"실측에 여유를 얹은
    // 값은 근거가 아니다"* 를 규율로 적고 있다. 유도하면 전제가 바뀌어도 저절로 따라온다.
    //
    // 유도: 위 「태양이 반구광보다 세다」가 `hemi < sun` 을 못 박으므로 총량은
    // `sun + hemi < 2 * sun` 이다. 즉 **태양 우위가 유지되는 한 총량의 상한은
    // `DAY_SUN_I` 에서 나온다** — 별도 배수가 필요 없다.
    expect(after, `총량이 태양 우위 한계(2×${DAY_SUN_I})를 넘었다 — 대비가 아니라 노출 변경이다`)
      .toBeLessThan(2 * DAY_SUN_I);
  });
...
//   (바) 1.2 → 1.29 (태양 우위 경계 직전)  → 2 failed  ← 유도 상한으로 바꾼 뒤 추가
//
// 6/6. (바)는 검수관 권고 P1 로 상한을 `1.5 배`(실측+여유)에서 `2 × DAY_SUN_I`(유도)로
// 바꾼 뒤 경계 근처를 확인한 것이다 — 유도로 바꿔도 (마)의 검출력은 그대로 5 failed 다.
//
// **이 회차에 옛 계약 둘이 실제로 빨간불을 냈다** — 값을 바꾸자
