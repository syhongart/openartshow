...
// `frontend/js/world-glb/README.md` 한 곳이다.
//
...
const W8 = join(FRONTEND, 'js/world-glb');
 ...
 * 이라 `frontend/js/world-glb/` 하위를 훑는 `walk` 에 걸리지 않는다. 검수관이 `walk`
 * 로직을 재현해 실측했다:
 *
 *     walk(W8) 표본 크기: 81
  ...
  it('**역방향도 0 이다** — world2 가 world8 를 참조하지 않는다', () => {
    ...
    for (const file of W2_FILES) {
      for (const spec of specifiersOf(readFileSync(file, 'utf8'))) {
        ...
        if (out.startsWith('js/world-glb/')) offenders.push(`${relative(W2, file)} → ${spec}`);
      }
    }
    expect(offenders).toEqual([]);
  });
