 ...
 *
 * ── 🔴 **그래서 지금 남는 사각은 정확히 둘이다**(전부 실측) ─────────────────────
 *   ⓐ **은닉 + 표본 밖 키** — 은닉+`IntlYen` **0 failed** · 은닉+`ScrollLock` **0 failed**
 *   ⓑ **비동기 지연** — 표본·은닉과 무관하게 0 failed
 *
...
describe('★ 키 → 입력 — `createFlyInput`', () => {
  ...
  //
  // ⚠ 그리고 이 검사는 **「눌렀을 때 즉시」**만 본다(판정이 `keydown` 직후 동기다).
  // `setTimeout` 으로 `held` 를 채우는 형태는 **표본 안 키라도** 못 잡는다(검수관 실증).
  //
