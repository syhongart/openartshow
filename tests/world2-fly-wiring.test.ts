 ...
 *
 *   선언 밖 병합·대입(같은 파일)   → **GS-F5**(정적) — ⚠ **단서 있음, 아래 ①**
 *   `Readonly` 제거 · `export`     → **GS-F5 의 전제 검사**(정적)
 *   `onDown` 에 키 코드 하드코딩    → **GS-F6**(행위) — ⚠ **단서 둘, 아래 ②③**
 *   동적 이름 접근                 → 🔴 **아무도 안 본다**
 *
 * ── 🔴 그 「잡는다」에 붙는 단서 (검수관 실증 + 재실측, 2026-08-19) ────────────────
 * ① **GS-F5 는 문자열 리터럴 안의 `//` 에 눈이 먼다.** 주석을 걷는 단계가 문자열을
 *    구분하지 않으므로 `const _t = "note//"; Object.assign(FLY_KEYS as …, { … });`
 *    한 줄은 **통째로 지워진 채** GS-F5 에 도달하지 않는다.
 *    **다만 GS-F6 이 구제한다** — 은닉해 넣은 키가 **표본 안**이면 행위로 잡힌다.
 *    실측: 은닉+`F1` **1 failed** · 은닉+`Numpad5` **1 failed**(표본 확대 후).
 *    ⚠ 검수관이 처음 실증했을 때는 **0 failed** 였다(그때 표본은 43개, `F1` 이 밖).
 * ② **GS-F6 은 표본만 본다**(전수가 아니다 — 목록·근거는 그 검사 자리에).
 * ③ **GS-F6 은 「눌렀을 때 **즉시**」만 본다.** 판정이 `keydown` 직후 동기로 끝나므로
 *    `setTimeout(() => { held.add('up'); wake(); }, 0)` 는 **표본 안 키(`KeyQ`)라도**
 *    0 failed 다(검수관 실증: 동기 `up:false` → 60ms 뒤 `up:true`).
 *
 * ── 🔴 **그래서 지금 남는 사각은 셋이다**(전부 실측) ──────────────────────────────
 *   ⓐ **표본 밖 키코드 — 은닉과 무관하다.** `onDown` 에 `IntlYen`·`F13`·`ScrollLock`·
 *      `NumLock` 을 **그냥 하드코딩해도** 0 failed 다. `onDown` 경로는 `FLY_KEYS` 를
 *      안 만지므로 **GS-F5 와 애초에 무관**하고, GS-F6 은 표본만 본다.
 *      ⚠ 첫 판본이 이것을 *"**은닉 +** 표본 밖 키"* 로 적었고 **좁았다**(검수관 P-a).
 *      은닉은 GS-F5 를 피하는 수단인데, 이 경로는 GS-F5 를 **탈 일이 없다.**
 *   ⓑ **GS-F6 의 판정 시점 밖 — 비동기 지연 그리고 `keyup` 경로.**
 *      `setTimeout`/`queueMicrotask` 는 물론이고, **`onUp` 에서 `held.add('up')`** 도
 *      0 failed 다(키를 뗀 뒤 켜지고 안 꺼진다). 표본 루프가 `keydown`→판정→`keyup`→
 *      재바인딩 순서라 **keyup 으로 켜진 상태가 판정 없이 지워진다**(검수관 P-b).
 *      ⚠ 재바인딩 자체는 필요하다 — 없으면 고착 오염으로 **거짓 FAIL** 이 난다.
 *   ⓒ **동적 이름 접근**(`globalThis['FLY'+'_KEYS']`) — 정적 스캔 밖.
 *
 * ⚠ **이 검사들의 「거짓 FAIL 0」은 jsdom 범위에서만 참이다**(검수관 P-c). 실기기
 * 키보드 레이아웃(한글 IME · macOS)에서 `code` 가 다르게 오는 경우는 **이 환경에서
 * 못 쟀다** — 못 잰 것을 통과로 적지 않는다.
 *
 * 게이트를 여기까지만 넓힌 이유: ⓐⓑ 는 **적대적 형태**다(백로그 `G-FLY14` — 이 장치는
 * 적대적 우회가 아니라 **내가 습관으로 저지르는 것**을 막는다). 반면 검수관이 실증한
 * 「표본 밖 키 하드코딩」은 습관으로 저지를 수 있는 형태라 **표본을 91개로 넓혀 닫았다.**
 *
 * ⚠ **이 표는 두 번 거짓이었다.** 1판은 「다른 파일에서 병합 — 원리적으로 못 본다」였고
 * 원리가 아니라 **구현 선택**이었다(그래서 GS-F5 로 닫았다). 2판은 위 세 단서를 빼고
 * *"GS-F5 가 잡는다"* 로 **단정**했다. 같은 커밋에 *"닫을 수 있는 것을 「원리」로 적으면
 * 다음 사람이 확인을 생략한다"* 를 새겨 놓고 **그 기준을 내 표에는 적용하지 않았다.**
 *
 * (「다른 파일에서 병합」은 목록에 없다 — `FLY_KEYS` 는 **export 되지 않고** 다른 파일이
 * **코드로** 참조하지 않는다. 주석 인용은 `edit/input.ts` 에 셋 있다.)
 */
...
describe('★ 키 → 입력 — `createFlyInput`', () => {
  ...
  //
  // ⚠ **전수가 아니라 표본이다** — 아래 91개. 첫 판본은 43개(문자·숫자·화살표 + 셋)였고
  // 검수관이 그 **밖의 키로 실제 우회를 실증**했다(`F1` 을 은닉된 줄에서 하드코딩 → 게이트
  // 전체 0 failed). 그래서 기능키·숫자패드·문장부호·편집키를 더했다(권고 P-1). 넓힌 대가는
  // 배열 6줄이고 거짓 FAIL 위험은 0이다(대조군 실측).
  //
  // 🔴 **그래도 표본이다.** 남는 것: 국제 키(`IntlBackslash`·`IntlRo`·`IntlYen`),
  // `Lang1`~`Lang5`, 미디어·볼륨 키, `ScrollLock`·`Pause`·`PrintScreen`·`NumLock`.
  // **표본을 전수로 읽지 마라** — 백로그 `G-FLY14`.
  //
  // ⚠ 그리고 이 검사는 **판정 시점 밖**을 못 본다. 판정이 `keydown` 직후 **동기**로 끝나므로
  //   · `setTimeout`/`queueMicrotask` 로 `held` 를 채우는 형태
  //   · **`onUp` 에서 `held.add` 하는 형태** — 루프가 `keydown`→판정→`keyup`→재바인딩이라
  //     **keyup 으로 켜진 상태가 판정 없이 지워진다**(검수관 P-b, 재현 확인 0 failed)
  // 는 **표본 안 키라도** 못 잡는다. 닫으려면 `key('keyup', code)` **뒤에도** 판정을 한 번
  // 넣으면 되지만, 이번 회차에는 **안 넣었다** — 검수관이 «여기서 멈춘다» 를 경계로
  // 판정했고(7회차, 매 회차 새 우회가 하나씩 나왔다), 이것은 「아무도 안 본다」 항목의
  // 라벨 정밀도이지 잡던 것을 놓치는 변경이 아니다. 재론 조건은 `G-FLY14`.
  //
  ...
  it('🔴 **리터럴에 없는 키는 비행을 안 일으킨다** — 검수관 명세 GS-F6', async () => {
    ...
    const sample = [
      ...
      'Home', 'End', 'PageUp', 'PageDown', 'Insert', 'Delete', 'Backspace', 'CapsLock', 'ContextMenu',
    ].filter((c) => !fly.has(c));
    const guilty: string[] = [];
...
describe('🔴 키 배치가 바뀌면 **안내 문구를 함께 보게 한다** — 트립와이어 (검수관 명세 GS-F2)', () => {
  ...
  it('🔴 GS-F5 의 **전제 둘** — `Readonly` 를 유지하고 `export` 하지 않는다', async () => {
    ...
    // 이 식별자를 **코드로** 참조하지 않는다(주석 인용은 `edit/input.ts` 에 **셋** —
    // 첫 판본이 「한 곳」이라고 적었고 틀렸다, 검수관 권고 P-2. 세는 법:
    // `grep -rn FLY_KEYS frontend/ tests/`. **줄 번호는 여기 안 적는다 — 낡는다.**).
    // 그래서 「다른 파일에서 병합」은 **지금은 성립조차 안 한다** — 검사가 아니라 export
    // 부재가 막고 있다. 그 사실이 바뀌는 순간을 이 검사가 잡는다.
    const { readFileSync } = await import('node:fs');
    const src = readFileSync('frontend/js/world2/edit/fly-input.ts', 'utf8');
    const clean = src.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/[^\n]*/g, '');
