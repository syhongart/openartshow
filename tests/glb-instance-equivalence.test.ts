...
//
//   (가) 인스턴스 행렬을 아예 안 읽음 (원래 결함)      → 5 failed
//   (나) 노드 월드변환을 안 곱함 (로컬 행렬만)         → 2 failed
//   (다) 셀 판정을 행렬마다 안 하고 노드 하나로        → 1 failed
//   (라) 진단이 인스턴스 수를 안 곱함(`meshes`)        → 1 failed
//   (마) 삼각형만 안 곱함                              → 1 failed
//   (바) 그림자 데칼만 안 곱함                         → 1 failed
//
// ⚠ **(가)(나)는 한 번 틀리게 적혔다**(검수관 블로커 B1). 처음 실측할 때는 테스트가
// 5개였고 그때 값이 「4·1」로 맞았는데, 그 뒤 6번째 검사(「그림자 보정까지 거친 최종
// 배치」)를 추가하고 `holderOf` 로 픽스처를 고치면서 **두 케이스가 추가로 깨지게 됐다.**
// 표를 재실측하지 않고 옛 값을 옮겨 적어 「4·1」이 남았다 — 실측은 「5·2」다.
// 방향은 안전한 쪽(과소 보고)이었지만, **이 표 자체가 팀장 조건 1의 이행 증거**라
// 틀리면 제3자가 조건 이행을 확인할 수 없다. 게다가 같은 회차에 내가 신설한
// `DELEGATION.md` F-1 조항이 겨냥한 유형이 바로 이것이다(파생 수치 재실측 누락).
//
