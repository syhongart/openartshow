...
//
// 두 트리의 `decide/grass-mode.ts` 는 헤더 경로 주석과 세계별 상수(모드·마디·카드 잎 수) 독블록·값을 빼면
...
import { GRASS_MODE_DEFAULT as W2, GRASS_SEG_DEFAULT as S2, CARD_BLADES_DEFAULT as C2, GRASS_AA_DEFAULT as A2 } from '../frontend/js/world2/decide/grass-mode.js';
import { GRASS_MODE_DEFAULT as W8, GRASS_SEG_DEFAULT as S8, CARD_BLADES_DEFAULT as C8, GRASS_AA_DEFAULT as A8 } from '../frontend/js/world-glb/decide/grass-mode.js';
...
describe('잔디 기본 모드 — 세계별 감독 판정', () => {
  ...
  it('두 트리의 grass-mode.ts 는 주석을 걷어낸 코드가 세계별 3줄(모드·마디·카드 잎 수) 빼고 같다', () => {
    ...
    const PER_WORLD = ['GRASS_MODE_DEFAULT', 'GRASS_SEG_DEFAULT', 'CARD_BLADES_DEFAULT'];
