...
describe('GLB 월드 낮 빛 — 감독 판정값의 계약', () => {
  ...
  it('⭐ world2 와 갈려 있다 — 「동기화」로 되돌아가는 것을 막는다', () => {
    // 이 트리는 world2 전체 포크이고 **no-sync 가 정책**이다(`world8/README.md`).
    // 두 값이 같아지는 순간은 둘 중 하나다: 감독이 world2 도 올리라고 했거나,
    // 누가 「같은 상수인데 다르네」로 맞춰버렸거나. 후자는 판정을 지우는 것이다.
    expect(DAY_HEMI_I, `world2(${W2_HEMI_I})와 같아졌다 — 감독 판정을 확인하고 이 검사를 고쳐라`)
      .not.toBe(W2_HEMI_I);
    expect(DAY_HEMI_I).toBeGreaterThan(W2_HEMI_I);
  });
  ...
  it('⭐ world2 의 두 계약은 여기서 **의도적으로** 깨져 있다 — 기록', () => {
    // 이 검사는 「깨져 있음」을 못 박는다. 다음 사람이 world2 계약을 그대로 옮겨오면
    // 여기가 먼저 빨간불이 나서 **옮기기 전에** 이 주석을 읽게 된다.
    expect(DAY_SUN_I / DAY_HEMI_I, 'world2 계약은 >2 였다. 여기는 필라이트가 있어 1 근처다')
      .toBeLessThan(2);
