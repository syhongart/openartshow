...
describe('waveHeight — 타일이 이어진다', () => {
  ...
  it('★ 마루가 좁고 골이 넓다 — 위아래가 대칭이면 물결이 아니라 요철이다', () => {
    // ── 이 축이 없어서 뮤테이션이 뚫렸다 (실측 2026-07-31) ────────────────────
    // 앞 회차에서 "파동 합성을 덧셈으로 바꾸기" 뮤테이션이 **안 깨졌다.** `waveHeight`
    // 테스트가 *심리스인가* 와 *평평하지 않은가* 만 봤기 때문이다 — 무늬의 **모양**을
    // 재는 축이 하나도 없었다.
    //
    // 실제 수면은 중력 때문에 골이 넓고 완만하며 마루가 좁고 뾰족하다. 그 비대칭이
    // 물을 물로 보이게 하고, 대칭이면 대비를 아무리 올려도 "울퉁불퉁한 표면" 이다.
    //
    // 비대칭은 **분포의 치우침**으로 드러난다. 골(낮은 값)이 넓게 퍼져 있으면 표본
    // 평균이 중앙(0)보다 아래로 내려간다. `WAVE_SKEW` 를 1(대칭)로 되돌리면 평균이
    // 0 근처로 올라와 이 단언이 깨진다.
    const vals: number[] = [];
    for (let i = 0; i < 64; i++) {
      for (let j = 0; j < 64; j++) vals.push(waveHeight(i / 64, j / 64));
    }
    const mean = vals.reduce((a, b) => a + b, 0) / vals.length;
    expect(mean, '물결이 위아래 대칭이다 — 골이 넓어야 물처럼 보인다').toBeLessThan(-0.05);
    // 그렇다고 바닥에 붙어 있으면 안 된다 — 마루가 실제로 서 있어야 한다.
