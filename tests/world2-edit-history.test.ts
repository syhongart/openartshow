...
describe('★ ② 세 저장소가 각각 되돌아간다', () => {
  ...
  it('★ 축별만 바꿔도 라벨이 「크기」다 — 「조작」이면 무엇을 되돌리는지 모른다', () => {
    // 검수관 비블로커(2026-08-25). 균등 `s` 만 보던 판정이라 축별만 바꾸면 셋 다 false 로
    // 떨어져 `'조작'` 이 나왔다 — 되돌리기 목록에 뜨는 글자라 감독이 직접 본다.
    expect(poseLabel(poseOf(entry(1)), { x: 0, y: 0, z: 0, ry: 0, s: 1, sx: 2 })).toBe('크기');
    // 다른 축과 섞이면 여전히 「조작」이다 — 한 가지로 요약할 수 없다.
    expect(poseLabel(poseOf(entry(1)), { x: 5, y: 0, z: 0, ry: 0, s: 1, sz: 2 })).toBe('조작');
  });
