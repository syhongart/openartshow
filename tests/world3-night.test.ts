// world3 의 **밤 정합.** 포크가 데려오지 않은 게이트를 채운다.
//
// ── 왜 생겼나 (검수관 반려 B1·권고 P4, 2026-08-08) ─────────────────────────
// `world3/decide/night.ts` 에 이렇게 적혀 있었다:
//
//   *"⚠️ `sky.js` 의 값을 복제한 것이라 미러링이다. 저쪽이 바뀌면 여기가 조용히
//     어긋난다 — 그래서 `tests/world2-night.test.ts` 가 두 값이 같은지 검사한다."*
//
// **world3 에서 그 문장은 거짓이었다.** `world2-night.test.ts` 는
...
// 내리고 있었다는 점이다(*"world3 에서 그 문장은 거짓이었다 — 포크한 트리에는 그
...
// `world3/sky.js` 는 `LIGHT` 테이블을 export 하지 않고 `three` 를 import 한다. 팔레트
...
} from '../frontend/js/world3/decide/night.js';
import { BLOOM_THRESHOLD } from '../frontend/js/world3/features/postfx-params.js';
import { V, luma } from '../frontend/js/world3/parts/palette.js';

const SKY_SRC = readFileSync(
  join(process.cwd(), 'frontend/js/world3/sky.js'), 'utf8',
);

/**
 * `world3/sky.js` 의 `LIGHT[time][weather].fog` 를 소스에서 뽑는다.
 *
...
describe('가로등이 블룸 문턱을 넘는다 — 넘지 못하면 블룸이 돌아도 화면이 그대로다', () => {
  ...
  //
  // 미러링을 없애면서(`LAMP_LUMINANCE = luma(V.lanternGlow)`) 이 축에 **새 연결이
  // 생겼다**: 이제 팔레트에서 등불색을 어둡게 조정하면 블룸이 조용히 죽는다.
  // world2 는 `tests/world2-night.test.ts` 가 이 축을 지키는데 world3 에는 없었다.
  ...
  it('휘도가 팔레트에서 유도된다 — 손으로 베낀 값이면 조용히 어긋난다', () => {
    ...
    // 그래서 **팔레트를 기준으로** 대조한다.
    expect(LAMP_LUMINANCE).toBeCloseTo(luma(V.lanternGlow), 12);
  });
