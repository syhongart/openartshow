...
describe('한쪽 안(岸)만 놓는다 — 두 개가 겹치지 않게', () => {
  ...
  it('건너편에도 길이 이어진다 — 다리가 길 없는 곳으로 데려다주지 않는다', () => {
    // ── 검수관이 찾은 사각 (2026-07-30) ──────────────────────────────────
    // `place` 는 **자기 파셀에 남쪽 길이 있는가**만 본다. 건너편(남단) 파셀이 실제로
    // 북쪽 길을 갖는지는 검사하지 않는다. `roadDirs` 의 `north` 는 격자 경계 해시가
    // 정하고 슈퍼셀 안에서 확률적으로 꺼질 수 있어(`decide/grid.ts`), 이론적으로
    // **다리는 있는데 건너편이 길로 안 이어지는** 상태가 가능하다.
    //
    // 검수관 실측으로는 현재 지형에서 불일치 0건이지만, **강 곡선이나 격자 소금값이
    // 바뀌면 재발하는 구조적 사각**이다. 그때 화면에는 "다리를 건넜는데 길이 없다"로
    // 나타나고, 걸어가 보기 전에는 아무도 모른다.
    for (const { px, pz } of allBridges()) {
      const span = spanSouth(px, pz, o);
      const far = pz + 1 + span; // 건너편 뭍 파셀
      expect(
        roadDirs(px, far),
        `(${px},${pz}) 다리의 건너편 (${px},${far}) 에 북쪽 길이 없다`,
      ).toContain('north');
    }
  });
});
