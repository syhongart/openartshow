 ...
 *
 * ⚠ **재는 형태가 제품과 정확히 같지는 않다 — 근사다**(검수관 재확인 R4). 여기서는 어댑터
 * 둘(`a`·`b`)을 **동시에** 들고 번갈아 미는데, 제품에서는 선택이 하나라 실제 경로가
 * 「a 선택 → 밀기 → **b 를 새로 선택**(새 어댑터) → 확정」이다. 결과는 같지만 형태가 다르고,
 * **이번 회차가 배운 것이 정확히 그 종류의 어긋남이다**(하네스가 제품과 다르게 부르면
 * 검사는 그대로 통과한다). 여기서 근사를 쓰는 이유는 이 파일이 **어댑터 단위** 검사이고
 * 선택 전환은 `edit/state.ts` 소관이어서다 — 선택을 낀 형태는
 * `world2-edit-panel-wiring.test.ts` 가 패널을 통째로 태워서 본다.
 */
