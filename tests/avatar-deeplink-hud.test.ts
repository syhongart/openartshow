...
//
// ── 못 보는 것 ────────────────────────────────────────────────────────────
// 실브라우저 렌더·CSS·포커스, 실기기 터치, WebGPU.
//
// **CI 스모크는 이 경로를 한 번도 열지 않는다**(`smoke/config.mjs` 가 `/app/index.html`
// 을 파라미터 없이만 연다). 딥링크 세션의 콘솔 에러 0 은 **부팀장이 1회 수동 실측했다**
// (2026-08-08, 7세션 · 저장·닫기·ESC·배경클릭 + hash 보존 + 평소 미술관, 전부 0).
// **그것은 게이트가 아니다** — 다음 변경이 이 경로를 깨도 CI 는 모른다. 백로그 `G-DL1`.
