...
describe('배선 — 잘라낸 것이 실제로 씬까지 간다', () => {
  ...
  it('거절 판정을 **패널이 직접 하지 않는다** — 검사 닿는 곳으로 옮겼다', () => {
    ...
    //
    // ⚠⚠ **이 검사가 막는 것은 「그 문자열 그대로 되돌리기」 하나다**(검수관 권고 R2).
    // 한때 여기 *"인라인으로 다시 적는 것을 막는다 — GX-1 과 같은 형태다"* 라고 적혀
    // 있었고 **과장이었다**: `foreign === 0 && nodes.length === 0` 처럼 **순서만 바꿔**
    // 다시 적으면 0건 FAIL 이다(검수관 실측). GX-1 은 «전부 같은 식별자 하나를 넘긴다»
    // 는 **양성** 단언이라 우회가 어려운데, negative 정규식은 그 형태가 아니다.
    //
    // 「다시 적는 것 일반」과 「판정값이 실제로 소비되는가」는 **집행 쪽 통합 검사**의
    // 몫이다(검수관 GS-C, 다음 회차). 게이트 유효성 과장은 다음 사람이 확인을
    // 생략하게 만든다 — 이 회차에 「존재만 보는 검사」로 미끄러진 세 번째다.
    expect(PANEL, '판정을 여기서 그대로 되돌리면 두 곳이 갈린다')
