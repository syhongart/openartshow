...
import { artworkSize, normalizeSpace, ART_SCALE, FRAME_RULES, PART_TYPES, DEFAULT_SPACE } from '../frontend/js/space.js';
import { generateSpace, GEN_ART_SCALE } from '../frontend/js/space-generate.js';
...
describe('artworkSize — 배율', () => {
  ...
  it('상한은 임의값이 아니다 — clamp 와 맞물려 있다', () => {
...
describe('generateSpace — 배치와 렌더가 같은 배율을 쓴다', () => {
  ...
  it('배율을 올려도 같은 벽에서 액자가 겹치지 않는다', () => {
    ...
    for (const k of [1.0, 1.4, 2.0]) {
      ...
      for (const p of r.space.parts) {
        ...
        const w = artworkSize(p.ar, r.space.shell.artScale).W;
  ...
  it('액자가 벽 안에 머문다(천장·바닥을 뚫지 않는다)', () => {
    ...
    for (const p of r.space.parts) {
      if (p.t !== 'artwork') continue;
      const { H: h } = artworkSize(p.ar, r.space.shell.artScale);
      const cy = p.y ?? 1.6;
...
describe('space-assembler — 조립이 배율을 읽는다', () => {
  ...
  const check = (scale: number, withImage: boolean) => {
    ...
    const want = space.parts.filter((p) => p.t === 'artwork').map((p) => {
      const { W, H } = artworkSize(p.src ? p.ar : undefined, space.shell.artScale);
  ...
  for (const withImage of [true, false]) {
    ...
    it(`배율을 올리면 조립된 판이 실제로 커진다 — ${label}`, () => {
      ...
      expect(Math.max(...big.want)).toBeGreaterThan(Math.max(...small.want) * 1.3);
    });
  }
