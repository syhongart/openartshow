...
// 여기 초록이 「라이브에 파란 링크가 없다」를 뜻하지 않는다.
//
// ⚠ **그 한계 진술도 처음엔 부정확했다**(교차리뷰 P1, 2026-08-09). 원문은 뜻하는 바를
// 「파랑이 나올 자리를 막아 둔 페이지 목록이 안 줄었다」로 적었는데, 검수관이
// `a { color: inherit }` 를 **`a { color: #0000EE }`** 로 바꾸는 뮤테이션(M6)을 돌리니
// **10 passed** 였다. 자리를 막기는커녕 사고 색을 명시 선언했는데도 통과한다 —
// 이 검사는 `a` 셀렉터에 `color` **선언이 있는가**만 보고 **그 값은 안 본다.**
// 정확한 진술은 이것이다:
//   **「`a` 셀렉터에 `color` 선언이 있는 페이지 목록이 안 줄었다」**
// 값 축(선언값이 UA 기본 링크색 계열인지)은 별건으로 열려 있다.
//
// **뮤테이션 5종을 돌린 축에도 6번째가 있었다** — 뮤테이션 개수는 검출력의 상한이 아니다.
//
// ── 양방향으로 센다 ────────────────────────────────────────────────────────
...
describe('링크 기본 글자색 안전망 — UA 기본색이 나올 자리를 막는다', () => {
  ...
  const WITHOUT_SAFETY_NET = [
    ...
    'builder.html',
    'mypage.html',
    // 🔴 2026-08-23 `world.html` → `world2.html` (감독 지시 «월드 1로 승격»).
    // 둘 다 정적 `<a>` 가 **0개**이고 링크는 HUD 안에서 만들어진다 — `index.html` 과
    // 같은 성격이라 현황 배열에 그대로 들어온다.
    //
    // ⚠ **그래도 위험도는 달라졌다.** 이 배열의 취지는 *"괜찮다가 아니라 아직 안 깔았다"*
    // 인데, 여기 새로 들어온 항목은 **랜딩의 첫 링크**다. 색을 안 준 링크가 HUD 에 하나
    // 생기는 날 파랑이 뜨는 자리가 「실험 페이지」에서 「방문자 첫 화면」으로 옮겨 왔다.
    // 안전망을 까는 것은 이 승격 회차의 범위 밖이라 현황만 고정하되, **줄이는 방향의
    // 우선순위가 올라갔다**는 것을 여기 적어 둔다.
    'world2.html',
  ];
