...
/**
 * 합성 세계의 바닥 윗면 y(m). **0 이 아니어야 한다** — 아래 `syntheticGlb` 주석 참조.
 *
 * ⚠ **`DEFAULT_KNEE_Y`(0.5m) 미만이어야 한다**(검수관 권고 2026-09-22). 그 값이
 * 「바닥이 `groundY` 위로 이만큼까지는 여전히 걸을 수 있다」의 허용 오차이므로, 여기를
 * 그보다 크게 올리면 **walkability 판정 자체가 뒤집히고** 이 파일의 다른 검사들이 재는
 * 것이 조용히 바뀐다. 지금 0.25 가 안전한 것은 우연이 아니라 이 부등식 때문이고,
 * 그 사실이 어디에도 안 적혀 있으면 다음 사람이 「맨해튼 실측에 −0.61 도 있으니」 같은
 * 이유로 0.6 을 넣는다.
 */
