...
//   (나) 0 고정 카운터(`bootErrors`)를 되살림          → 1 failed  (정규식이 잡는다)
//   (자) 그 카운터를 실제로 `showBootChecklist` 에 넘김 → tsc `TS2345` (타입이 잡는다)
...
//   (아) 폭주 상한 제거                                → 1 failed
//
// 8/8 + (자). **(나)와 (자)는 서로 다른 방어가 잡는다** — 되살리기만 한 것은 정규식이,
// 실제로 넘긴 것은 타입이 잡는다(검수관 권고 P3 으로 갈라 적었다).
//
// ⚠ **뮤테이션을 쓸 때 함정 하나**(검수관이 재현 중 직접 겪었다): 지운 줄을 «주석»으로
// 바꾸면서 그 주석 안에 원본 문자열을 남기면, 소스 정규식이 그 주석에 매치돼 **거짓
// 통과**가 난다. 실제로 `// MUTATION: bootLog.dispose() removed` 로 바꿨더니 0 failed
// 가 나왔다. 지울 때는 완전히 다른 텍스트로 바꾼다.
//
// (가)(나)(바)가 이 파일의 존재 이유다 — **셋 다 `tests/glb-checklist.test.ts` 는
