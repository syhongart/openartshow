...
describe('슬롯 예산 — 여유 배수 1.0이 실제로 버티는가', () => {
  ...
  // 검수관이 짚은 사각. 위의 순간이동은 CELL*40 같은 **너무 큰** 점프라 순회 범위
  // (`reach = ceil(farExit)+1 = 4`셀) 밖으로 통째로 나가고, 그러면 전부 unload로 처리돼
  // 위험한 경로를 밟지 않는다. 정작 위험한 건 **범위 안에 남는 중간 크기 점프**다 —
  // `tierFor`가 한 프레임에 한 단계만 강등하므로, 밴드 폭(0.15~0.30셀)보다 크고 순회
  // 범위보다 작은 이동에서 "실제 거리보다 안쪽 tier에 잔류하는" 파셀이 생긴다.
  //
  // 현재 물리 상수로는 프레임당 0.062셀이라 도달 불가능하지만(parcel-builder.ts 주석),
  // 여기서는 그 여유를 무시하고 직접 만들어 본다.
  //
  // 다만 **이 케이스가 새 검출력을 주지는 않는다.** headroom을 낮춰가며 재보니 이 테스트는
  // 0.75까지도 안 깨지는 반면, 아래 "여러 방향으로 걸어다녀도"·"프레임 예산이 빠듯해도"·
  // "밀도 8"은 0.85에서 이미 깨진다. 점프는 한 번에 크게 옮겨 대량 unload가 먼저 일어나는
  // 반면, 연속 걷기는 승격과 강등이 계속 겹쳐 슬롯이 최대로 물려 있기 때문이다.
  // 즉 예산을 지키는 실질 게이트는 연속 걷기 쪽이고, 이 케이스는 상수가 바뀌었을 때를
  // 대비한 경계 표식이다 — "점프 사각을 메웠다"고 읽으면 안 된다.
  it('중간 크기 점프에서도 넘지 않는다 — 순회 범위 안에 남는 이동', () => {
    const h = harness();
    h.settle();
    // 0.2셀부터 3.5셀까지. 밴드 폭보다 크고 reach(4셀)보다 작은 구간을 훑는다.
    for (const cells of [0.2, 0.35, 0.6, 0.9, 1.2, 1.45, 1.8, 2.2, 2.7, 3.2, 3.5]) {
      for (const [ux, uz] of [[1, 0], [0.707, 0.707], [0, 1], [-0.707, 0.707]] as const) {
        for (let rep = 0; rep < 3; rep++) {
          h.dir.x = ux; h.dir.z = uz;
          h.pos.x += ux * cells * CELL;
          h.pos.z += uz * cells * CELL;
          h.step(0.05); // 예산도 함께 빠듯하게 — 두 조건이 겹칠 때가 최악이다
        }
      }
    }
    expect(h.violations).toEqual([]);
