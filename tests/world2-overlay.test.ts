...
//
// ── 뮤테이션 — ⚠️ 이 커밋 시점에는 **아직 안 돌렸다** ────────────────────────
// 19개가 통과하는 것은 검출력의 증거가 아니다. 아래 넷을 일부러 되살려 실제로 FAIL 이
// 나는지 확인하고, 결과를 **이 자리에 실측으로** 적는다(안 깨지는 것이 있으면 그 축은
// 장식이므로 테스트를 고친다):
//   ① `isSafeSrc` 의 `src.includes('..')` 줄 삭제
//   ② `SRC_RE` 의 선두 `^` 삭제 — `blob:…/assets/models/x.glb` 가 통과하는지
//   ③ `normalizeOverlay` 의 `if (!isSafeSrc(...)) continue` 삭제
//   ④ 스케일 클램프 `S_MIN` 을 0 으로
// ②가 특히 중요하다 — **정규식이 그대로 있는 채로** 열리는 구멍이라 코드를 읽어서는
// 안 보인다.
