...
// (**B8·B9·B10·B11**)이 전부 거기서 났다. 매번 산문이 틀렸다.
// 낡지 않게 하는 유일한 방법은 문장을 **단언으로 바꾸는 것**이다.
//
// ⚠ 여기 원래 *"산문만 고치는 왕복이 **다섯 번**"* 이라고 적었고 **틀렸다**(검수관 C1).
// 산문만 고친 왕복은 **2회**다 — `6283e43`(B9-R)은 실행코드를 13줄 추가한 회차라 거기 안
// 든다. "다섯" 은 **반려 건수**였는데 그것을 왕복 수로 재진술하며 어긋났다.
// **그래서 숫자를 다른 숫자로 고치지 않고 이름을 적었다**(검수관 처방) — 이 파일의 병이
// *"검증되지 않는 수를 산문에 적는 것"* 인데 수를 고치는 것은 같은 병이다. 이름은 다음
// 사람이 셀 수 있고, 세었더니 다르면 그건 **목록이 낡은 것이라 눈에 띈다.**
//
...
describe('게이트 커버리지 — 산문 대신 실행이 말한다', () => {
  it('check:refs 는 이 계약 파일을 대상으로 삼지 않는다 — 고아 모듈이라 도달 못 한다', async () => {
    ...
    // 옮기려던 구멍이 남아 있었던 것**이다. 앞선 반려들(B8·B9·B10·B11)이 *"주석이 코드보다
    // 강하다"* 였다면 이것은 **그 처방 자체에 난 구멍**이라 형태가 다르다 — 여기 원래
    // *"반려 사슬 다섯 건의 형태 그대로"* 라고 적었는데 틀렸다(검수관 C1).
    //
    // ⚠ 대조군으로 `lod.ts` 를 고른 이유(검수관 P-C): **하위 디렉터리이면서 8개 소스가
    ...
    //
    // ⚠ **아래 `not.toContain` 은 실패 모드가 반대라 더 나쁘다**(검수관 P-A). `overlay.ts` 가
    // 이동하고 import 경로가 함께 갱신되면 그 단언은 빨간불이 아니라 **영원히 조용히
    // 통과**한다 — 신호가 사라지는 형태다. 이 파일을 옮길 일이 생기면 **경로 문자열을 같이
    // 옮겼는지**를 반드시 확인하라.
