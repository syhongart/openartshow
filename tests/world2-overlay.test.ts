...
//
// ⚠ **`npm run gate` 6종을 "이 파일이 6개 축의 검사를 받았다" 로 읽지 마라.**
// `eslint.config.js` 가 `**/*.{js,mjs}` 라 **`.ts` 를 아예 안 본다**(의도된 설계 — `tsc`
// strict 가 대신 본다). 이 diff 를 실제로 보는 축은 **셋**이다:
//   `tsc`(strict) · `vitest` · **`check:refs`** (`scripts/smoke/check-refs.mjs` 가
//   `frontend/js` 아래 `.ts` 를 TS 프로그램으로 파싱해 미해결 참조를 본다).
//
// ⚠ 여기 원래 *"tsc 와 vitest 둘"* 이라고 적었고 **틀렸다**(검수관 B9). 틀린 방식이 더
// 나쁘다 — `check-refs.mjs` 의 헤더가 *"eslint 도 .ts 를 대상에서 제외한다"* 를 **자기 존재
// 근거로** 명시하고 있다. 즉 나는 그 사각을 지적하면서 **그 사각을 메우려고 만들어진 게이트를
// 못 셌다.** 커버리지를 손으로 세면 이렇게 틀린다 — 처방은 게이트가 자기 대상 규칙을 출력하게
// 하는 것이고(검수관 P24), 범위 확대라 태스크로 올렸다.
...
/**
 * 해석할 수 없는 `version` 값. **완전 목록의 SSOT 는 여기 한 곳이다**(검수관 P22).
 *
 * 원래 완전 목록이 **세 곳**에 있었다 — 이 파일 두 곳과 `overlay.ts` 주석 한 곳. 테스트
