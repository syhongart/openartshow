...
describe('🔴 태스크 #112 — 미술관 벽이 편집에 물려 있다 (배선 축, 검수관 명세 GS-A1)', () => {
  ...
  it('★ `glbCity` 기능이 `wallRoot` 를 계약에 노출한다 — 그 문이 3단의 출발점이다', () => {
    ...
    // ⚠ **축이 둘이다 — 「문이 있는가」와 「성공 경로에서 루트를 내주는가」.**
    // 처음엔 `wallRoot: () => root` 를 통째로 봤는데, 파셀 생사 회차(2026-08-19)에서 본문이
    // «보이는 채가 없으면 null» 로 커지며 이 검사가 빨간불이 됐다. 축을 나눈 것 자체는
    // 옳았다 — 그런데 나는 나누면서 **아래 GS-V2 를 안 넣었고**, 그 순간 「루트를 내주는가」
    // 축이 통째로 사라졌다.
    //
    // 🔴 **그리고 그 자리에 거짓 문장을 적었다** (검수관 블로커 B3, 2026-08-20). 그때
    // 문구는 *"본문이 무엇을 판정하는지는 `world2-glb-city.test.ts` 의 가시성 블록이
    // **행위로** 잰다"* 였는데 **거짓이다** — 그 블록에서 `wallRoot` 를 보는 단언은
    // `toContain('copies.some(...)')` **한 줄이고 정적 문자열**이며, 같은 커밋의
    // `world2-glb-city.test.ts` 가 **스스로 «정적 텍스트 축이다» 라고 적고 있다.**
    // 한 diff 안에서 두 문장이 서로를 부정했다.
    //
    // 실측(검수관): `wallRoot` 의 성공 분기를 `? root : null` → `? null : null` 로 바꿔도
    // **0 failed / 4094**. 미술관 벽 검출(#112)이 통째로 죽는데 전 스위트가 초록이었다.
    //
    // **배운 것**: 축을 나누는 것은 정당한 처방이지만, **나눈 뒤 각 축이 무엇을 잡는지
    // 뮤테이션으로 확인하지 않으면 나누는 것이 곧 지우는 것**이다. 그리고 그것을
    // 「행위로 잰다」로 적으면 다음 사람이 확인을 생략한다 — `main` unprotected 오기가
    // 7일을 잃은 그 형태다.
    expect(src, '🔴 구현이 그 문을 안 연다').toContain('wallRoot: () =>');
    // 🔴 **GS-V2** (검수관 명세, 블로커 B3) — 성공 경로가 실제로 `root` 를 내주는가.
    // 조건식까지 함께 못 박는다. ⚠ **실측하면 `? root : null` 도 지금은 1회뿐이라 안 걸린다**
    // (검수관 권고 R1) — 첫 판본은 *"다른 삼항에 걸린다"* 고 **단정**했고 그것은 세어 보지
    // 않고 쓴 문장이었다. 조건식을 붙이는 이유는 **미래의 충돌을 막기 위함**이지 지금
    // 충돌하기 때문이 아니다. 이 회차의 블로커가 정확히 「확인 안 한 것을 단정으로 적은
    // 것」이었으므로 여기에 적어 둔다.
    // ⚠ **거짓 FAIL 이 나는 경우**: 삼항을 `if` 로 풀거나 조건 순서를 바꾸면 의미가 같아도
    // 빨간불이 된다. 그때는 **약화가 아니라 갱신**이므로 문자열을 새 형태로 옮기고
    // **그 사실을 이 자리에 적는다**(«행위로 잰다» 로 얼버무리지 말 것 — 그것이 B3 이다).
    // ⚠ **못 잡는 것**: ⓐ 틀린 루트를 내주는 것(자식 하나·씬 전체 — 기존 사각 ⓑ 그대로)
    // ⓑ 실제 광선이 미술관 메시에 닿는지(jsdom 원리적 불가) ⓒ 죽은 코드에 같은 문자열이
    // 남은 형태 ⓓ WebGPU 실기기.
    expect(src, '🔴 `wallRoot` 가 성공 경로에서도 루트를 안 내준다 — 미술관 벽 검출이 죽는다')
      .toContain('copies.some((c) => c.node.visible) ? root : null');
