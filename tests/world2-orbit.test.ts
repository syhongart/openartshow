...
describe('내려다보는 각 — **음수가 아래**다', () => {
  ...
  it('★ 눈과 대상이 **같은 높이이고 수평거리도 0** 이면 유한하다', () => {
    // 나눗셈(`atan((eyeY-targetY)/r)`)으로 쓰면 여기서 `0/0 = NaN` 이 되고, 그 NaN 이
    // pitch 에 들어가면 카메라 회전이 통째로 깨진다 — 화면이 사라진다.
    //
    // ⚠ 이 축은 O-5 뮤테이션(`atan2` → `atan`)이 **0 failed** 로 나와서 생겼다.
    // 그 둘은 거의 등가다(`atan2(y,0) = ±π/2` 이고 `atan(±∞) = ±π/2`) — **한 점만
    // 빼고.** 「거의 등가」와 「등가」는 다른 일이고, 그 한 점이 여기다.
    expect(Number.isFinite(pitchTo(5, 5, 0)), '★ 0/0 이 NaN 으로 새어나왔다').toBe(true);
  });
