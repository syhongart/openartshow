...
describe('실제 docs/devlog-times.json — 오염 검사', () => {
  ...
  //
  // **남는 한계 — "불가능" 이 아니라 "아직 안 함" 이다** (검수관 정정, 2026-07-30):
  // `check:devlog-times` 는 full clone 을 요구하고 CI checkout 은 기본 shallow 다. 그래서
  // 지금은 CI 에서 돌지 않는다. 그러나 **구조적으로 불가능한 것이 아니다** — `ci.yml` 에
  // `fetch-depth: 0` 인 job 을 하나 더 두면 자동화된다(비용은 checkout 시간뿐이고, 막지
  // 않고 알리기만 해도 된다). 첫 판본은 이것을 "CI 에서 못 돈다" 고 단정했는데, 그렇게
  // 적으면 **닫을 수 있는 사각이 닫을 수 없는 것으로 굳는다.**
  //
  // 즉 현재 자동화된 것은 아래 불변식뿐이고, 값 대조는 **손으로 돌려야 한다**.
  // CI 승격은 팀장 상신 사안(검수관 권고).
