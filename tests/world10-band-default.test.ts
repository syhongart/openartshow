...
//   (ㄹ) 재론 조건의 게이트 — 허용 목록 **밖**에서 `DEFAULT_BANDS` 나 `NYC_BAND_SCALE` 을
//        코드로 읽는 자리가 생기면 여기서 빨간불이 난다(팀장 재론 조건: 그러면 상신한다).
//        배율 축은 팀장 사후 통지(2026-09-11)가 *"같은 급의 재론 트리거"* 로 못 박은 것이다.
//
...
// ════════════════════════════════════════════════════════════════════════════
// (ㄹ) 재론 조건의 게이트 — 허용 목록 밖에서 `DEFAULT_BANDS`·`NYC_BAND_SCALE` 을 읽는 자리
// ════════════════════════════════════════════════════════════════════════════
describe('(ㄹ) DEFAULT_BANDS·NYC_BAND_SCALE 을 코드로 읽는 자리가 허용 목록과 정확히 같다', () => {
  ...
  // ── R-3: **배율 자체**를 직접 읽는 자리 (팀장 사후 통지 2026-09-11) ──────────
  // 팀장: 배율의 직접 소비처가 셋째로 늘어나는 것은 *"다섯 소비처 밖에서 `DEFAULT_BANDS` 를
  // 읽는 자리와 같은 급의 재론 트리거"* 다. 지금 둘뿐인 이유는 나머지 소비처가 **이미 배율이
  // 걸린** `NYC_BANDS` 를 읽기 때문이다 — 거기에 배율을 또 곱하면 그 자리만 거리가 제곱으로
  // 어긋나고(0.5² = 0.25), 그 어긋남은 산술이 아니라 **화면에서만** 보인다. 위 (ㄴ) 의 통합
  // 단언도 이것을 못 잡는다 — 그 검사는 네 거리가 한 수인지를 보는데, 새 소비처는 그 넷
  // 밖에서 자기 거리를 만들기 때문이다. 그래서 「읽는 자리」 자체를 목록으로 고정한다.
  const SCALE_ALLOWED = new Set([
    // 정의 자리 — 배율은 여기서 만들어진다
    'frontend/js/world10/decide/nyc-bands.ts',
    // 유일한 예외 — `FOG_NEAR_CELLS` 만 밴드 **객체**가 아니라 기준 배수(`FOG_NEAR_REF_CELLS`)
    // 에서 유도되므로 배율을 직접 곱한다. 안 곱하면 near > far 로 안개가 뒤집힌다
    // (B-1 을 넘은 편집 ⓐ · 사후 추인 — `decide/fog.ts` 의 「밴드와 같은 배율을 탄다」 절).
    'frontend/js/world10/decide/fog.ts',
  ]);

  /**
   * 주석 줄과 **문자열 리터럴**을 걷어낸 «코드만».
   *
   * 문자열 안의 등장은 소비가 아니다(진단 문구·키 이름). 다만 리터럴은 **한 줄 안에서
   * 닫히는 것만** 걷는다 — 개행을 넘게 두면 짝 없는 아포스트로피 하나가 파일 뒷부분을
   * 통째로 먹어 검출력이 조용히 사라진다. fog.ts 의 import 줄이 이 필터를 통과해
   * 목록에 잡히는 것이 「코드를 안 먹는다」의 실물 증거다.
   */
  ...
  it('허용 목록 밖의 파일은 NYC_BAND_SCALE 을 코드로 읽지 않는다 [M-R3: decide/fly.ts 에 소비 한 줄을 넣으면 깨진다]', () => {
    const found: string[] = [];
    for (const abs of walk(join(ROOT, 'frontend/js/world10'))) {
      if (/\bNYC_BAND_SCALE\b/.test(codeOnly(abs))) found.push(relative(ROOT, abs));
    }
    expect([...found].sort(), '배율을 직접 읽는 자리가 늘었다 — 팀장 재론 조건이다(상신)')
      .toEqual([...SCALE_ALLOWED].sort());
  });
});
