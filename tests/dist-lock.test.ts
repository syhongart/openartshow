...
// ── 회귀 가드 — 클론에서 실측한 뮤테이션 결과를 CI 에서 영구 재확인한다 ─────
//
// ⚠ **이 describe 는 뮤테이션을 돌리지 않는다.** 이름을 바로잡는다(검수관 조건 2,
// 2026-08-10). 첫 판본은 이것을 *"뮤테이션 검출력"* 이라 불렀는데 **거짓이었다** —
// 진짜 뮤테이션은 **저장소 밖 클론에서 소스를 실제로 변조해** 돌렸고(executor 실측 →
// 검수관이 독립 재현, 수치 일치: **M1 6/8 · M2 3/8 FAIL**, 대조군 8/8 PASS), 여기 있는
// 것은 그 결과가 **깨지지 않았는지 CI 에서 다시 보는 가드**다.
//
// 이름을 안 고치면 다음 사람이 *"이 파일이 뮤테이션을 돌린다"* 로 읽는다 — 이 저장소가
// **"테스트 통과는 검출력의 증거가 아니다"** 라고 못 박은 그 형태를, 하필 검출력을
// 주장하는 파일이 스스로 재현한 셈이었다.
describe('G-LOCK — 회귀 가드 (뮤테이션 실측은 별도 클론에서 수행)', () => {
  it('M1: 락이 없으면 두 실행이 임계구역에 동시에 들어간다', () => {
    // ⚠ 아래 for 문은 **설명용이고 검증 로직이 아니다** — `withDistLock` 을 호출하지
    // 않으므로 구현을 어떻게 변조해도 이 부분은 항상 통과한다(검수관 실측). 락 없는
    // 판본에서 무엇이 성립하는지를 코드로 보여 줄 뿐이다.
    let inside = 0;
    let maxConcurrent = 0;
    ...
    // ↓ **여기부터가 실제 검증이다** — `withDistLock` 이 두 번째를 거부하는가.
    fs.mkdirSync(LOCK_DIR, { recursive: true });
    fs.writeFileSync(
      path.join(LOCK_DIR, 'owner.json'),
...
//  · **baseline(`assembleSite`)의 `$OUT` 경합**(검수관 조건 1a) — `ASSEMBLE_BASELINE_SH`
//    도 `rm -rf "$OUT"` 을 하므로 **baseline↔vite · baseline↔baseline** 을 동시에 돌리면
//    같은 형태의 경합이 난다. 이 락은 **`dist/` 조립만** 보호한다(실사고 2건이 전부
//    vite×vite 였고 baseline 은 `dist/` 를 아예 안 쓴다) — 즉 **원 사고("빌드 중간 상태
//    서빙")는 baseline 에 구조적으로 없지만, `$OUT` 을 서로 지우는 경합은 미보호다.**
//  · **TOCTOU 창**(검수관이 이번 리뷰에서 **새로 발견**, 조건 1b) — 획득이 두 단계다:
//    `mkdirSync(LOCK_DIR)` 로 자리를 잡고 → `owner.json` 에 소유자를 적는다. **그 사이**에
//    다른 프로세스가 `mkdirSync` 를 시도하면 `EEXIST` 인데 `owner.json` 이 아직 없어
//    *"기록 직전에 죽었다"* 로 **오판하고 락을 회수**한다. 그러면 **두 프로세스가 동시에
//    빌드한다** — 이 장치가 막으려던 바로 그 사고가 락 자체의 결함으로 성립하는 경로다.
//    블로커로 걸지 않은 이유: 창이 **동기 두 문장 사이**(마이크로초)인데 원 사고는 수 분짜리
//    빌드 두 개가 겹치는 조건이라 확률이 비교 불가하게 낮다. 원자적 쓰기로 좁히는 개선은
//    백로그 `G-LOCK` 후속이다. ⚠ **낮은 확률을 "없음" 으로 적지 않는다.**
const _limits = null;
export { _limits };
