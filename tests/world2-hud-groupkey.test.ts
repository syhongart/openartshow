...
// (`draw`와 `drawGroupKeys`)이 **같은 인덱스에서 같은 프레임인가**는 순수 함수 테스트로
// 절대 안 잡힌다. 링이 한 칸 어긋나도 판정은 조용히 돌아가고, 심지어 통과한다.
//
// 검수관이 이 diff를 보고 지목한 것이 정확히 그 구멍이었다 — "테스트가 전부 순수 함수만
// 겨냥한다. sky.js 실제 visible 토글 조건과 drawGroupKey()가 일치하는지 검증하는 통합 테스트는
...
describe('HUD 표본 수집 — draw와 하늘 상태의 짝', () => {
  it('전이 구간(drawGroupKey null) 표본은 draw에도 안 들어가고, 뺀 수가 리포트에 남는다', async () => {
    const p = parts();
    ...
    expect(line).toContain('상태 전이 중 3표본 제외');
    hud.dispose();
  });
  ...
  it('drawGroupKey()가 던져도 두 링이 어긋나지 않는다', async () => {
    const p = parts();
    const { src, state } = source();
    const hud = attachHud(p, src);
  ...
  it('drawGroupKey가 없는 소스면 옛 판정으로 떨어진다 — 하위호환', async () => {
    const p = parts();
    const { src, state } = source({ drawGroupKey: undefined });
    const hud = attachHud(p, src);
