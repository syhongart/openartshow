...
//
// 실측으로 확인한 것(2026-07-31): **three 는** 배포 번들에 `@license` 주석이
// 살아남는다(청크 3개 전부에 `Copyright 2010-2024 Three.js Authors` + SPDX).
// 그러니 이 페이지는 위반 해소가 아니라 **사람이 찾을 수 있는 고지**를 만드는 것이다 —
// 미니파이된 JS 안의 주석은 기계는 읽어도 방문자는 못 읽는다.
//
// **PeerJS 는 사정이 다르다.** `app/vendor/peerjs.min.js`(92,865 bytes)에는 선두 배너
// 주석이 아예 없고, `MIT` 의 첫 출현은 offset 65,612 — 번들에 인라인된 `package.json` 의
// `"license":"MIT"` 필드다. 그것은 저작권 고지가 아니라 메타데이터다(MIT 가 요구하는
// 것은 저작권 고지 **및 허가 고지 전문**이다).
//
// 즉 **PeerJS 의 고지는 이 페이지가 유일한 이행 수단이다.** 처음엔 "번들에 고지가
// 살아남는다(three + PeerJS)" 로 뭉뚱그려 적었는데, 검수관 지적을 받아 파일을 직접 열어
// 보니 PeerJS 는 그 문장이 성립하지 않았다. 참인 문장에서 성립하지 않는 결론을 뽑는
// 형태 — 이 저장소가 여러 번 데인 자리라 실측 그대로 적어 둔다.
//
// ── 왜 마크다운을 소스로 두는가 ────────────────────────────────────────────
