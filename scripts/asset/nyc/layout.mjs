...
/**
 * **격자 셀 규격** (팀장 판정 2026-09-06 「C·포크」 C2 — «격자 셀 → 결정적 seed 파셀 목록»).
 *
 * 지시서 `docs/NYC-GALLERY-WALK.md` 의 «50~70m 거리 한 블록» 이 스폰 셀이다. `SIZE` 는 그 범위
 * 안에서 **거리 내용물이 셀 안에 들어가는 최소값**으로 잡았다 — 내용물의 x 범위는 건물 서쪽 끝
 * `x=0` 부터 게이트 동쪽 끝 `GATE_X + 1.4 = 63.4` 까지이므로 64 가 그 값이다(여유 0.3m×2).
 *
 * `ANCHOR_X` 는 **셀 중심이 놓인 저작 좌표**다. 생성기는 지면 판을 이 중심 기준 ±`SIZE/2` 로
 * 자르고, 런타임(`world10/systems/nyc-parcels.ts`)은 플레이어 좌표에서 이 값을 빼 격자 좌표를
 * 얻는다. **두 곳에 같은 값이 있으므로 `tests/nyc-grid.test.ts` 가 대조한다**(값 미러링 방지 —
 * `PALETTE.curb` ↔ `world10-boot` `hemig` 와 같은 형태).
 *
 * ⚠ 값을 바꾸면 셀 GLB 를 다시 굽고(`--cell`) 런타임 상수도 함께 움직여야 한다. 그 대조가
 * 위 테스트다.
 */
export const CELL = Object.freeze({
  SIZE: 64,
  ANCHOR_X: (DIMS.GATE_X + 1.4) / 2,   // 31.7
  ANCHOR_Z: 0,
});
 ...
 *
 * @param bounds `{ x0, x1, zFar }` — **격자 셀 모드에서만 넘긴다.** 기본값은 «거리 한 장» 모드이고
 *   그 값을 바꾸면 `nyc-street.glb` 의 바이트가 달라진다(기존 캡처·증거와 비교가 깨진다).
 */
export function groundPlan(bounds = {}) {
  // ── 기본(«거리 한 장») ────────────────────────────────────────────────────
  ...
  //
  // ── 🔴 격자 셀 모드에서는 이 «넉넉함» 이 곧 결함이다 ──────────────────────
  // 셀을 격자로 이어 붙이면 **인접 셀의 지면 판이 서로를 통째로 덮는다** — 160×120m 판을 64m
  // 간격으로 놓으므로 x 로 96m·z 로 56m 가 겹치고, 겹친 두 판은 **윗면 y 가 같고 법선도 같다.**
  // 그것이 감독이 2026-09-06 에 지적한 «우글우글»(z-fighting) 의 정의 그대로이고, 같은 회차에
  // 496 쿼드쌍 → 0 으로 고친 결함이다. 그래서 셀 모드는 지면을 **셀 경계에 정확히** 자른다 —
  // 맞닿기만 하고 겹치지 않으므로 겹침 면적이 0 이다(`tests/nyc-coplanar.test.ts` 축).
  const { x0 = -40, x1 = 120, zFar = 60 } = bounds;
