...
export const ACCEPTED = [
  {
    ...
    //
    //   ✅ **실물 확인 (2026-08-14, run 31757052567 — 규격 유도가 실물과 일치했다)**.
    //   PR #182 승인 시점에는 부팀장도 검수관도 실물 바이트를 못 봤고 판정 근거가 근사
    //   픽스처였다(검수관 Q-5 가 이 기록을 요구했다). 실제 `peek` 출력:
    //
    //       APP11  JP.........}jumb....jumdc2pa.........8.q.c2pa....Wjumb...Gjumdc2
    //                          ↑12      ↑20 ↑24              ↑41
    //
    //   **`c2pa` 가 두 군데 나온다** — idx24(UUID 앞 4바이트)와 idx41(label). 즉 위 fail-open
    //   은 이론이 아니라 **실물에 그대로 존재한다.** 옛 규칙이었다면 idx24 에서 이미 참이
    //   되어 label 을 안 봤을 것이다. 크기는 6023 바이트이고 그중 게이트가 본 것은 45다.
    //   ⚠ 뒤쪽에 `jumb`/`jumd` 가 **또 나온다**(idx49~) — JUMBF 는 중첩 박스 구조다.
    //   오프셋을 고정하지 않으면 그 중첩분까지 판별에 섞인다는 뜻이기도 하다.
    //
