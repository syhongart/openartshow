...
export const RATES = {
  ...
  reviewer: {
    // 정규직 승격(감독 지시 2026-07-30) — 부팀장과 **동급 별축**이다: 부팀장은 구현
    // 실무, 검수관은 검증 실무. 지시를 받아 수행하는 구조라 팀장 상급으로는 두지
    // 않았다(감독 판단).
    //
    // 승격 전에는 시간당 범위형(30~50만)이었다. 정규직 산정은 `countEmploymentMonths`
    // × 월급 단일값이라 단위 자체를 바꿔야 했다 — 범위형을 그대로 두면 정규직 분기가
    // `rate.amount` 를 숫자로 기대해 조용히 틀린 값을 냈다.
    amount: 7750000,      // 월 775만
    unit: '월',
    grade: '대체추정',
    source: 'KOSA 「응용SW개발자」(부팀장 동급). 코드리뷰·릴리스 승인 특화 통계 없음',
  },
  legal: {
