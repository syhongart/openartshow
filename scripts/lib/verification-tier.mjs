...
//
// 그래서 등급이 줄이는 것은 **배포 전 로컬 스모크와 검수관 왕복**이지,
// 자기완결·CSP 검사 자체가 아니다.
// (⚠ 이 줄은 오래 *"로컬 executor 스모크"* 라고 적고 있었다 — §10-3 이 2026-08-10 에
//  그 위임 형태를 폐기했으므로 `TIERS.smoke` 주석과 함께 정정했다.)
//
 ...
 * 등급 정의. **번호가 작을수록 무겁다.**
 *
 * `smoke` — 배포 **전** 로컬 `smoke:vite` 를 **조기 스크리닝**으로 돌리는가(§10-3).
 *           `false` 여도 CI 스모크는 돈다(파일 머리 참조).
 *
 *           ⚠ **이 필드의 뜻이 2026-08-10 팀장 판정으로 바뀌었다.** 예전에는
 *           *"독립 executor 가 돌린다(구현자 본인 금지)"* 였고, 그 형태는 `smoke:vite`
 *           소요(10~11분 8초)가 Bash 상한(10분)을 넘어 **원리상 완주 불가**였다 —
 *           완주할 수 없는 일을 맡기니 «완주했다고 적는 압력» 이 생겼다(3회차 위임이
 *           실제로 그렇게 됐다). 지금은 **구현자 본인 실행을 허용**하되 그 PASS 를
 *           **배포 판정 근거로 기재하는 것을 금지**한다. **판정 주체는 CI** 다.
