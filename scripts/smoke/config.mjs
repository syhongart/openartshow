...
export const LIVE_PAGES = [
  ...
  //   ① **회귀는 world2 쪽에서 잡힌다.** 같은 코드이고 `tests/world8-independence.test.ts`
  //      가 그 동일성을 지킨다(주석 제외 코드 diff 0줄 — `features/sky.ts`·`sky.js`).
  ...
  // → **재론 조건**: world8 이 라이브로 승격되거나, world8 에서만 나는 날씨 결함이
  //    보고되는 회차. 그때는 켜기 전에 **뮤테이션으로 검출력부터 확인한다.**
  //
