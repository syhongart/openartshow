...
export const LIVE_PAGES = [
  ...
  // 성립하지 않는다 — 열면 곧장 로더가 돈다.
  //
  // ⚠ **이 자리에 한때 *"자산 요청까지 포함해 콘솔 에러 0 을 본다(경로가 틀리면 여기서
  // 잡힌다)"* 라고 적혀 있었고 실측보다 넓었다**(검수관 반려 B1). 그 주장이 겨냥한 실패
  // 모드는 **둘**인데 — ⓐ 경로가 틀림 ⓑ 자산이 안 옴 — 잡는 것은 **하나**다. 뮤테이션 2종:
  //
  //   GLB → 404          consoleErrors 3 · failedRequests 2 · 로딩 "error" → **FAIL** ✓
  //   GLB → 40초 지연     전 축 0 · 화면 빈 상태                        → **PASS** ✗
  //
  // ⚠ **위 표의 두 행은 실측 시점이 다르다**(2026-08-26 정정, 검수관 조건 3).
  // 404 행은 world8 이 **world2 전체 포크로 재구성된 뒤** 재측정한 값이다(검수관 실측).
  // 40초 지연 행은 **폐기된 페이지**(`js/glb-world.js` 를 쓰던 판본)에서 잰 것이고
  // **포크 후 재측정하지 않았다** — 그 판본의 증거였던 `#pick` 은 지금 페이지에
  // 존재하지도 않는다(로딩 UI 가 world2 의 `ui/loading.ts` 로 바뀌었다). 결론(조용한
  // 통과)은 유지될 가능성이 높지만 **그것은 추정이고 실측이 아니다.**
  //
  // 즉 **판정 창 안에 도착한 경우에만** 참이다. 안 온 것은 조용히 초록이다. world8 은
  // GLB 가 세계의 전부라 「PASS = 세계가 섰다」로 읽히기 쉬운데 그 등식이 성립하지 않는다.
  //
  // **이 항목이 못 잡는 것**:
  //   · **판정 창 밖으로 밀린 자산** — 위 실측. 느린 러너에서 이 형태로 조용히 통과한다.
  //   · **텍스처 누락** — 진단 훅은 지오메트리만 센다. CSP 로 텍스처를 전부 막아도
  //     메시 수 `28,707` 은 정상 출력됐다(검수관 실측, 폐기된 판본 기준).
  //   · **자산이 낡았는지** — world2 가 바뀌어도 이 GLB 는 안 따라온다(백로그 `G-W8A`).
  //   · **날씨 축** — 아래 `weatherProbe` 절 참조.
  //   · 렌더 결과의 유의미성 · WebGPU · 시각 회귀 — 다른 항목과 같다.
  //
  // ── `weatherProbe` 를 **안 단다** — 판단과 근거 (2026-08-26, 검수관 조건 3) ──
  // world2·world3·world5 는 전부 달고 있고, world8 은 포크라 **같은 하늘·날씨 코드**를
  // 돈다 — 일관성만 보면 다는 것이 자연스럽다. 그럼에도 안 다는 이유:
  //   ① world8 **고유 갈래는 GLB 세계**이고 날씨와 직교한다. 이 항목이 겨냥할 것은
  //      「고정 자산이 오는가」이지 「비가 오는가」가 아니다.
  //   ② **이번 회차에 이 축을 단독으로 확인하지 못했다.** 확인 안 한 축을 CI 에 넣으면
  //      그때부터 「돌고 있으니 괜찮다」로 읽힌다 — 이 저장소가 관측 게이트에 대해
  //      *"승격 없이 남으면 그때부터 장식이다"* 라고 적은 것과 같은 형태다.
  //
  // ⚠ **여기 «회귀는 world2 가 잡아 준다 — `tests/world8-independence.test.ts` 가 코드
  // 동일성을 지키므로» 라는 근거가 적혀 있었고 거짓이었다**(검수관 N4, 2026-08-26).
  // 그 파일의 `it()` 12개는 전부 **import 격리·어댑터 목록·three 순수성**이고 두 트리의
  // **내용을 비교하는 단언이 하나도 없다** — 가장 가까운 것이
  // `it('\`world2/\` 로 향하는 import 가 0 이다 — 포크의 존재 이유')` 인데 그것은
  // 「참조하지 않는다」이지 「같다」가 아니다. 코드 diff 0줄은 **오늘의 사실**일 뿐 검사가
  // 지키는 것이 아니다. 게다가 `frontend/js/world-glb/README.md:82` 는 **no-sync 가 기본**
  // 이라고 선언한다 — 두 트리는 **갈라지도록 설계돼 있다.** 즉 그 근거에는 **만료가
  // 예약돼 있었고** 만료 시점을 아무것도 알려주지 않았다.
  // **이것이 이 회차 세 번째 같은 유형이다**(그림자 근거 · 「차단 케이스 2건」 · 이것).
  // 셋 다 「게이트가 무엇을 보증하는가」에 대한 거짓 진술이고, 셋 다 앞 것을 정정하는
  // 커밋 안에서 났다. 처방 후보는 백로그 `G-GATE` 의 G-6 이다.
  //
  // → **재론 조건**(셋 중 하나):
  //    ⓐ world8 이 라이브로 승격되는 회차
  //    ⓑ world8 에서만 나는 날씨 결함이 보고되는 회차
  //    ⓒ **world8 트리의 하늘·날씨 파일이 world2 와 갈리는 첫 회차** — no-sync 가
  //       기본인 이상 그 순간이 「world2 가 잡아 준다」가 실제로 무너지는 지점이고,
  //       ⓐⓑ 로는 그것을 못 잡는다(검수관 조건 2).
  //    셋 중 무엇이든, 켜기 전에 **뮤테이션으로 검출력부터 확인한다.**
  //
  // ⚠⚠ **이 편입은 라이브 검증에도 자동으로 들어간다** — `verify-live.mjs` 가 이 배열을
  // 그대로 순회한다. 그 파일에는 2.79MB mp4 가 예산을 못 맞춰 배포마다 빨간불이 났던
  // 타임라인이 주석으로 박혀 있다. 여기서 위험이 낮다고 본 근거는 둘이고 **실측이다**:
  // ① gzip 후 **1,201,956 B**(mp4 는 압축이 안 된다 — 이 값은 `gzip -c` 기본 레벨의
  // 실측이고, 자산 재생성 후 다시 쟀다. 앞 판본의 `1,200,967` 은 **다른 자산**의 값이다)
  // ② GLB 요청이 문서 `load` 와 **동시**에
  ...
  //
  // ⚠ **43 MiB 는 world8(5.2MB)·world10 과 자릿수가 다르다.** 실측된 것과 안 된 것을
  // 갈라 적는다 — 이 행의 판정을 읽는 사람이 무엇을 믿어도 되는지 알아야 한다.
  //
  //   실측됨 — 파일 45,498,128 B(43.4 MiB) · **`gzip -9` 7,790,575 B(7.43 MiB)**
  //            GitHub Pages 의 **100MB 파일 상한** 대비 여유 있음(43.4 < 100).
  //   안 쟀음 — **`networkidle` 이 `PAGE_TIMEOUT_MS` 안에 성립하는가.** world8 의 gzip
  //            전송량이 약 1.2MB 이므로 이 페이지는 그 **6배 자릿수**다. 로컬 정적
  //            서버라 통과 가능성은 높지만 **확정이 아니다** — CI run 1회로 확인한다.
  //            거짓 FAIL 이 나면 그것은 자산 결함이 아니라 **이 행의 타임아웃 문제**다.
  //   안 쟀음 — **라이브에서의 같은 축.** `verify-live.mjs` 도 `LIVE_PAGES` 를 그대로
  //            순회하므로 배포본에서 방문자가 7.43 MiB 를 받는다. world8 행이 적어 둔
  //            「판정 창 안에 도착한 경우에만 참이다」가 여기서는 **더 크게** 적용된다.
  //
  // ⚠⚠ 이 자리는 초안에 *"자산이 저장소에 없어 404 가 정상"* 이라고 적혀 있었고 자산이
  // 커밋된 뒤에도 남아 **검수관 블로커 B1** 이 됐다(2026-09-16). 단순 상태 서술이 아니라
  // **판정 기준**이었던 것이 문제다 — 다음 사람이 빨간불을 정상으로 읽었을 것이다.
  //
  // 행 자체는 **처음부터** 넣는다(behind-flag 인데 검사가 0 이던 사고가 visit·lab-glb 에서
  // 이미 났고, `tests/verification-tier.test.ts` G1 이 편입을 강제한다).
