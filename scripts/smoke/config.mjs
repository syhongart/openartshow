...
export const LIVE_PAGES = [
  ...
  //
  //   GLB → 404          consoleErrors 2 · failedRequests 1 · HUD 빈 상태 → **FAIL** ✓
  //   GLB → 40초 지연     전 축 0 · HUD 빈 상태 · #pick 안 숨겨짐        → **PASS** ✗
  //
  ...
  //   · **텍스처 누락** — HUD 는 지오메트리만 센다. CSP 로 텍스처를 전부 막아도 HUD 는
  //     `28,707` 을 정상 출력했다(검수관 실측).
  ...
  // ① gzip 후 1,200,967 B(mp4 는 압축이 안 된다) ② GLB 요청이 문서 `load` 와 **동시**에
