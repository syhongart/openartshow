...
//   ⚠ **그 축은 2026-08-26 에 열렸다.** `scripts/asset/blender-edit.py` 가 `bpy`
//   (Blender as a Python Module) 로 **실제 블렌더에** 우리 GLB 를 읽히고 다시 쓴다.
//   그 산출물(`frontend/assets/worlds/world2-blender-edit.glb`)로 되읽기를 재고
//   **감독 신고가 재현되지 않음**을 실측했다(라벨 «✓ 28,704개 적용 · 추가된 물건 2개»
//   · 씬 `{groups:1,meshes:2,visible:2}` · 콘솔 0 · pageerror 0).
//   ⚠⚠ **이 문장이 3주 가까이 「한계」로 적혀 있었고 아무도 열어보지 않았다.** 한계
//   선언은 불가능 선언이 아니다 — `pip download bpy` 한 줄이면 됐다. 「못 잰다」고
//   적힌 것은 **적힌 시점의 환경**에 대한 진술이므로 주기적으로 다시 시험해야 한다.
//   그 교훈은 `docs/BOARD.md` 에 있다.
//   지금도 이 스크립트 자신은 블렌더를 안 쓴다(휠이 374MB 라 CI 에 못 넣는다) —
//   여기 흉내가 남아 있는 것은 **게이트 소요** 때문이고, 블렌더 축은 위 자산으로 본다.
