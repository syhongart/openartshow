...
//
// ── 거짓 FAIL 위험 (위와 성격이 반대다 — 과소 검출이 아니라 과잉 차단) ──────
//  · `.gitignore` 의 디렉터리 전용 패턴(`foo/`)과 **이름이 같은 파일**을 `git add foo`
//    로 넣으면, 실제 `git add` 는 성공하는데 이 검사는 FAIL 을 낸다. 근거는
//    `ignoredAmong` 주석에 있다 — 검사 시점에 그것이 파일인지 디렉터리인지 판별할
//    수단(로컬 존재 여부)을 **일부러 안 쓴다.**
//    막히면 `-f` 로 억제할 수 있다. 단 그때의 `-f` 는 git 에 필요해서가 아니라
//    **순전히 이 검사를 조용히 시키기 위한 것**이다 — 붙이기 전에 이름 충돌 자체를
//    의심하는 편이 낫다.
//
// ── 트리거 (중요) ───────────────────────────────────────────────────────────
