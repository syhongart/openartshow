...
//
// **`import()` 가 깨지는 경우 — 두 층이 겹쳐 있다.**
// 실측 2건(경로 오타 `./auth-modal-NOPE.js` · export 이름 불일치)이 **빌드 단계에서
// FAIL** 했고 `[G-CLICK]` 줄이 안 나왔다. 즉 **번들러가 한 겹 먼저 막는다.**
//
// ⚠ **그렇다고 이 게이트가 그 형태를 「못 잡는」 것은 아니다**(검수관 권고 2026-08-10 —
// 첫 판본이 그렇게 적었고 **과잉 진술**이었다). `subnav.js` 의 `open` 은
// `import(...).then((m) => m.openAuthModal(...)).catch(...)` 구조라 **`.then()` 안에서
// 던진 예외까지 같은 `.catch()` 가 받는다.** 빌드를 통과해 런타임에만 죽어도
// **오버레이가 안 뜨는 것은 동일**하므로 이 게이트가 DOM NG 로 잡는다.
// 안전 방향의 과소 주장이었지만 **부정확한 것은 고친다** — 다음 사람이 이 줄을 읽고
// "그 축은 여기서 못 본다" 로 판단하면 그것이 사각을 만든다.
//
// ⚠ **뮤테이션을 인정할 때는 `[G-CLICK]` 줄이 실제로 나왔는지 함께 본다** — 종료코드
// 1 만으로는 *"빌드가 먼저 죽었다"* 와 *"게이트가 도달해 NG 를 냈다"* 가 구별되지
// 않는다(직전 회차가 그 구분을 안 해 무효가 됐다). 이 판별법은 **`npm run
// measure:click` 단독 실행에만** 유효하다 — `smoke:vite` 경로는 `log: quiet` 를
// 넘겨 그 줄을 안 찍고, 판정은 `record()` 가 그대로 받는다(검수관 지적).
//
// ── 이 검사가 못 잡는 것 ───────────────────────────────────────────────────
...
//  · **모달이 열린 상태의 가로 넘침**(검수관 권고 2026-08-10). `checkVisibility` 는
//    존재·투명도만 본다. 지금은 넘칠 수 없다 — 검수관이 계산했다: 컨테이너가
//    `position:fixed; inset:0` 이라 **문서 흐름 밖**이고(그래서 `[5]` 넘침 검사가 재는
//    `scrollWidth` 에 애초에 안 잡힌다), 카드가 `width: min(94vw, 400px)` 이라
//    320px 에서 300.8px 다. **그러나 그 폭을 고정 px 로 바꾸는 사람에게 이 축의
//    부재를 알리는 것이 이 줄의 역할이다** — 지금 안전한 것과 검사가 있는 것은 다르다.
//    ⚠ `import()` 실패는 **이 목록에서 뺐다** — 위 절 참조. 첫 판본이 여기 적었고
//    거짓이었다(빌드가 먼저 잡는 경우가 많을 뿐, 런타임에 죽어도 DOM NG 로 잡힌다).
//
...
import { BASE_PATH, CHROMIUM_ARGS, CHROMIUM_EXECUTABLE, SITE_DIR, VIEWPORTS } from './config.mjs';
...
/**
 * 폭은 **`config.mjs` 의 `VIEWPORTS` 를 그대로 쓴다**(검수관 권고 2026-08-10).
 *
 * 첫 판본은 `[1280, 390]` 을 독립 상수로 적었다 — `1280` 은 우연히 같았고 **`390` 은
 * SSOT 의 `320`·`375` 어느 것과도 안 맞는 새 값**이었다(내가 손으로 실측할 때 쓴 폭을
 * 그대로 옮겼을 뿐 근거가 없었다). 같은 성격의 값을 두 곳에 적으면 한쪽만 고쳐도
 * 아무도 모른다 — 이 저장소가 색·수치·임계값에서 세 번 덴 형태다.
 *
 * `320` 이 늘어 조합이 4→6 이 되지만, 초소형 폭은 **모달 wrap 회귀**를 보는 축이라
 * 이 검사에도 의미가 있다(`VIEWPORTS` 주석이 그 이유를 적고 있다).
 */
const WIDTHS = VIEWPORTS;
