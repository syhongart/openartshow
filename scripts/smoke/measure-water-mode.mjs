...
// · **조작 축이 없다** — `[7][8][11]` 과 달리 여기서는 카메라를 돌리지도 걷지도 않는다.
//   부팅 후 `SETTLE_MS` 동안 가만히 둘 뿐이다. 매 프레임 도는 결함(`system.update` 안의
//   예외 등)은 조작과 무관하게 나므로 이 구성으로도 잡히지만, **조작 중에만 나는**
//   결함은 여기서 안 잡힌다. 그 축이 필요해지면 `[11]` 처럼 걷는 구성을 따로 세운다.
// · **오늘 이 저장소에는 `tsl` 가지를 실행하는 러너가 없다**(검수관 권고 2026-08-02).
//   초록불을 "TSL 경로가 매 프레임 검증됐다" 로 읽으면 안 된다 — 아래 판정이 `backend`
//   를 함께 적는 이유이고, `run.mjs` 의 리포트 문구도 그 사실을 그대로 옮긴다.
//
...
// ── 폴백 경고는 걸러낼 필요가 없다 (검수관 권고 1, 2026-08-02) ──────────────
// 처음에는 `ocean.ts` 의 경고 문구 앞부분을 여기 적어두고 에러 집계에서 뺐다. 값
// 미러링이었고, 검수관이 "방향은 안전하지만 유지보수 비용" 이라고 지적했다.
//
// **필터 자체가 불필요하다.** 폴백 통지는 `console.warn` 이라 playwright 에서
// `msg.type() === 'warning'` 이고, 아래 집계는 `'error'` 만 센다 — 애초에 안 걸린다.
// 문구를 지운 것이 아니라 **걸러야 할 이유가 없었다**는 것이 정확한 서술이다.
//
// 누가 그 통지를 `console.error` 로 바꾸면 이 게이트가 FAIL 한다. 그것이 맞다 —
// 정상 폴백이 에러를 뱉으면 그 자체가 고칠 것이다.
